...
def prepare_tabulardata(
    ...
    **Parameter**            **Description**
    ...
    distance_features       Optional list of :class:`~arcgis.features.FeatureLayer` objects.
                            ...
                            Field names added to the prepared dataset follow the pattern
                            DIST_<prefix>, where the total field name length is strictly
                            limited to 10 characters.

                            The prefix is derived from the input
                            distance feature layer name. The name is split on underscores
                            and only the first segment is used, then truncated to fit
                            within the allowed length. To ensure uniqueness, duplicate prefixes
                            are assigned numeric suffixes such as _1, _2, and so on.
                            When such a suffix is added, the base prefix is further shortened
                            to accommodate the character limit.
                            For example, the layer name “Hospitals” produces DIST_hospi on
                            first occurrence, DIST_hos_1 on the second,
                            and DIST_hos_2 on the third, ensuring both
                            uniqueness and compliance with the field length constraint.
    ---------------------   -------------------------------------------
...
def prepare_data(
    ...
    **Parameter**           **Description**
    ...
    n_temporal_dates        Required list of strings. The dates of that observations
                            ...
                            Applicable only for dataset_type='PSETAE'.
    ---------------------   -------------------------------------------
    num_workers             Optional int. Default ``0``. Number of subprocesses to
                            use for data loading. ``0`` means that the data will
