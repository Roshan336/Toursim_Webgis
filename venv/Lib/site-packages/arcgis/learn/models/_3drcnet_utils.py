...
class ConvNeXt(nn.Module):
    ...
    Args:
        ...
        depths (tuple(int)): Number of blocks at each stage. Default: [1, 2, 4, 2]
        dims (int): Feature dimension at each stage. Default: [32, 64, 128, 256]
        drop_path_rate (float): Stochastic depth rate. Default: 0.05.
        layer_scale_init_value (float): Init value for Layer Scale. Default: 1e-6.
...
class acc(Callback):
    ...
    def on_epoch_end(self, last_metrics, **kwargs):
        ...
        OA = np.array(pre_label == tar_label, np.float64).sum() / samples_num
