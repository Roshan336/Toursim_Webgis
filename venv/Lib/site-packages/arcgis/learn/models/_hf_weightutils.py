import warnings, logging, functools, sys, threading

# quiet HF xet + timm builder/hub chatter (keep your existing lines if you want)
logging.getLogger("huggingface_hub").setLevel(logging.ERROR)
logging.getLogger("huggingface_hub._xet").setLevel(logging.ERROR)
warnings.filterwarnings(
    "ignore", message=r"Xet Storage is enabled.*hf_xet", category=UserWarning
)
logging.getLogger("timm.models._builder").setLevel(logging.ERROR)
logging.getLogger("timm.models._hub").setLevel(logging.ERROR)

# 1) detect whether hf_hub actually downloaded (vs returned from cache)
_dl_flag = threading.local()
_dl_flag.did = False
try:
    from huggingface_hub import file_download as _hf_fd

    _orig_http_get = getattr(_hf_fd, "http_get", None)
    if _orig_http_get is not None:

        @functools.wraps(_orig_http_get)
        def _http_get_mark(*args, **kwargs):
            _dl_flag.did = True  # mark that bytes were streamed
            return _orig_http_get(*args, **kwargs)

        _hf_fd.http_get = _http_get_mark  # patch
except Exception:
    _orig_http_get = None  # graceful no-op if API differs

# 2) wrap timm's hf_hub_download to emit the PyTorch-style pink line only when _dl_flag.did is True
import timm
from timm.models import _hub as _timm_hub

if hasattr(_timm_hub, "hf_hub_download"):
    _orig_hf_hub_download = (
        _timm_hub.hf_hub_download
    )  # functools.partial in timm 1.0.14

    @functools.wraps(_orig_hf_hub_download)
    def _hf_hub_download_stderr(*args, **kwargs):
        # reset flag for this call
        if _orig_http_get is not None:
            _dl_flag.did = False

        # call original (returns local cached path)
        cached_path = _orig_hf_hub_download(*args, **kwargs)

        # only print if a real HTTP GET streamed bytes (i.e., not cache hit)
        if _orig_http_get is not None and getattr(_dl_flag, "did", False):
            # build pretty URL for the message
            repo_id = kwargs.get("repo_id") or (args[0] if len(args) >= 1 else None)
            filename = kwargs.get("filename") or (args[1] if len(args) >= 2 else None)
            revision = kwargs.get("revision") or (
                args[2] if len(args) >= 3 and isinstance(args[2], str) else None
            )
            try:
                ...
                url = (
                    f"hf://{repo_id}/{filename}" if (repo_id and filename) else "hf-hub"
                )

            # PyTorch-style pink message goes to stderr
            sys.stderr.write(f'Downloaded: "{url}" to {cached_path}\n')

        return cached_path

    _timm_hub.hf_hub_download = _hf_hub_download_stderr
...
hf_resnet_cfgs = {
    ...
    "resnet18_landsat_tm_toa_moco": _res_cfg(
        hf_hub_id="torchgeo/ssl4eo_landsat",
        hf_hub_filename="resnet18_landsat_tm_toa_moco-1c691b4f.pth",
        input_size=(7, 224, 224),
    ),
    "resnet18_landsat_tm_toa_simclr": _res_cfg(
        hf_hub_id="torchgeo/ssl4eo_landsat",
        hf_hub_filename="resnet18_landsat_tm_toa_simclr-d2d38ace.pth",
        input_size=(7, 224, 224),
    ),
    "resnet18_landsat_etm_toa_moco": _res_cfg(
        hf_hub_id="torchgeo/ssl4eo_landsat",
        hf_hub_filename="resnet18_landsat_etm_toa_moco-bb88689c.pth",
        input_size=(9, 224, 224),
    ),
    "resnet18_landsat_etm_toa_simclr": _res_cfg(
        hf_hub_id="torchgeo/ssl4eo_landsat",
        hf_hub_filename="resnet18_landsat_etm_toa_simclr-4d813f79.pth",
        input_size=(9, 224, 224),
    ),
    "resnet18_landsat_etm_sr_moco": _res_cfg(
        hf_hub_id="torchgeo/ssl4eo_landsat",
        hf_hub_filename="resnet18_landsat_etm_sr_moco-4f078acd.pth",
        input_size=(6, 224, 224),
    ),
    "resnet18_landsat_etm_sr_simclr": _res_cfg(
        hf_hub_id="torchgeo/ssl4eo_landsat",
        hf_hub_filename="resnet18_landsat_etm_sr_simclr-8e8543b4.pth",
        input_size=(6, 224, 224),
    ),
    "resnet18_landsat_oli_tirs_toa_moco": _res_cfg(
        hf_hub_id="torchgeo/ssl4eo_landsat",
        hf_hub_filename="resnet18_landsat_oli_tirs_toa_moco-a3002f51.pth",
        input_size=(11, 224, 224),
    ),
    "resnet18_landsat_oli_tirs_toa_simclr": _res_cfg(
        hf_hub_id="torchgeo/ssl4eo_landsat",
        hf_hub_filename="resnet18_landsat_oli_tirs_toa_simclr-b0635cc6.pth",
        input_size=(11, 224, 224),
    ),
    "resnet18_landsat_oli_sr_moco": _res_cfg(
        hf_hub_id="torchgeo/ssl4eo_landsat",
        hf_hub_filename="resnet18_landsat_oli_sr_moco-660e82ed.pth",
        input_size=(7, 224, 224),
    ),
    "resnet18_landsat_oli_sr_simclr": _res_cfg(
        hf_hub_id="torchgeo/ssl4eo_landsat",
        hf_hub_filename="resnet18_landsat_oli_sr_simclr-7bced5be.pth",
        input_size=(7, 224, 224),
    ),
    "resnet18_sentinel2_all_moco": _res_cfg(
        hf_hub_id="torchgeo/resnet18_sentinel2_all_moco",
        hf_hub_filename="resnet18_sentinel2_all_moco-59bfdff9.pth",
        input_size=(13, 224, 224),
    ),
    "resnet18_sentinel2_rgb_moco": _res_cfg(
        hf_hub_id="torchgeo/resnet18_sentinel2_rgb_moco",
        hf_hub_filename="resnet18_sentinel2_rgb_moco-e3a335e3.pth",
        input_size=(3, 224, 224),
    ),
    "resnet18_sentinel2_rgb_seco": _res_cfg(
        hf_hub_id="torchgeo/resnet18_sentinel2_rgb_seco",
        hf_hub_filename="resnet18_sentinel2_rgb_seco-cefca942.pth",
        input_size=(3, 224, 224),
    ),
    "resnet50_landsat_tm_toa_moco": _res_cfg(
        hf_hub_id="torchgeo/ssl4eo_landsat",
        hf_hub_filename="resnet50_landsat_tm_toa_moco-ba1ce753.pth",
        input_size=(7, 224, 224),
    ),
    "resnet50_landsat_tm_toa_simclr": _res_cfg(
        hf_hub_id="torchgeo/ssl4eo_landsat",
        hf_hub_filename="resnet50_landsat_tm_toa_simclr-a1c93432.pth",
        input_size=(7, 224, 224),
    ),
    "resnet50_landsat_etm_toa_moco": _res_cfg(
        hf_hub_id="torchgeo/ssl4eo_landsat",
        hf_hub_filename="resnet50_landsat_etm_toa_moco-e9a84d5a.pth",
        input_size=(9, 224, 224),
    ),
    "resnet50_landsat_etm_toa_simclr": _res_cfg(
        hf_hub_id="torchgeo/ssl4eo_landsat",
        hf_hub_filename="resnet50_landsat_etm_toa_simclr-70b5575f.pth",
        input_size=(9, 224, 224),
    ),
    "resnet50_landsat_etm_sr_moco": _res_cfg(
        hf_hub_id="torchgeo/ssl4eo_landsat",
        hf_hub_filename="resnet50_landsat_etm_sr_moco-1266cde3.pth",
        input_size=(6, 224, 224),
    ),
    "resnet50_landsat_etm_sr_simclr": _res_cfg(
        hf_hub_id="torchgeo/ssl4eo_landsat",
        hf_hub_filename="resnet50_landsat_etm_sr_simclr-e5d185d7.pth",
        input_size=(6, 224, 224),
    ),
    "resnet50_landsat_oli_tirs_toa_moco": _res_cfg(
        hf_hub_id="torchgeo/ssl4eo_landsat",
        hf_hub_filename="resnet50_landsat_oli_tirs_toa_moco-de7f5e0f.pth",
        input_size=(11, 224, 224),
    ),
    "resnet50_landsat_oli_tirs_toa_simclr": _res_cfg(
        hf_hub_id="torchgeo/ssl4eo_landsat",
        hf_hub_filename="resnet50_landsat_oli_tirs_toa_simclr-030cebfe.pth",
        input_size=(11, 224, 224),
    ),
    "resnet50_landsat_oli_sr_moco": _res_cfg(
        hf_hub_id="torchgeo/ssl4eo_landsat",
        hf_hub_filename="resnet50_landsat_oli_sr_moco-ff580dad.pth",
        input_size=(7, 224, 224),
    ),
    "resnet50_landsat_oli_sr_simclr": _res_cfg(
        hf_hub_id="torchgeo/ssl4eo_landsat",
        hf_hub_filename="resnet50_landsat_oli_sr_simclr-94f78913.pth",
        input_size=(7, 224, 224),
    ),
    "resnet50_sentinel2_all_moco": _res_cfg(
        hf_hub_id="torchgeo/resnet50_sentinel2_all_moco",
        hf_hub_filename="resnet50_sentinel2_all_moco-df8b932e.pth",
        input_size=(13, 224, 224),
    ),
    "resnet50_sentinel2_rgb_moco": _res_cfg(
        hf_hub_id="torchgeo/resnet50_sentinel2_rgb_moco",
        hf_hub_filename="resnet50_sentinel2_rgb_moco-2b57ba8b.pth",
        input_size=(3, 224, 224),
    ),
    "resnet50_sentinel2_rgb_seco": _res_cfg(
        hf_hub_id="torchgeo/resnet50_sentinel2_rgb_seco",
        hf_hub_filename="resnet50_sentinel2_rgb_seco-018bf397.pth",
        input_size=(3, 224, 224),
    ),
    "resnet50_sentinel2_all_dino": _res_cfg(
        hf_hub_id="torchgeo/resnet50_sentinel2_all_dino",
        hf_hub_filename="resnet50_sentinel2_all_dino-d6c330e9.pth",
        input_size=(13, 224, 224),
    ),
    "resnet50_sentinel2_mi_ms_satlas": _res_cfg(
        hf_hub_id="torchgeo/satlas",
        hf_hub_filename="sentinel2_resnet50_mi_ms-da5413d2.pth",
        input_size=(9, 224, 224),
    ),
    "resnet50_sentinel2_mi_rgb_satlas": _res_cfg(
        hf_hub_id="torchgeo/satlas",
        hf_hub_filename="sentinel2_resnet50_mi_rgb-e79bb7fe.pth",
        input_size=(3, 224, 224),
    ),
    "resnet50_sentinel2_si_ms_satlas": _res_cfg(
        hf_hub_id="torchgeo/satlas",
        hf_hub_filename="sentinel2_resnet50_si_ms-1f454cc6.pth",
        input_size=(9, 224, 224),
    ),
    "resnet50_sentinel2_si_rgb_satlas": _res_cfg(
        hf_hub_id="torchgeo/satlas",
        hf_hub_filename="sentinel2_resnet50_si_rgb-45fc6972.pth",
        input_size=(3, 224, 224),
    ),
    "resnet152_sentinel2_mi_ms_satlas": _res_cfg(
        hf_hub_id="torchgeo/satlas",
        hf_hub_filename="sentinel2_resnet152_mi_ms-fd35b4bb.pth",
        input_size=(9, 224, 224),
    ),
    "resnet152_sentinel2_mi_rgb_satlas": _res_cfg(
        hf_hub_id="torchgeo/satlas",
        hf_hub_filename="sentinel2_resnet152_mi_rgb-67563ac5.pth",
        input_size=(3, 224, 224),
    ),
    "resnet152_sentinel2_si_ms_satlas": _res_cfg(
        hf_hub_id="torchgeo/satlas",
        hf_hub_filename="sentinel2_resnet152_si_ms-4500c6cb.pth",
        input_size=(9, 224, 224),
    ),
    "resnet152_sentinel2_si_rgb_satlas": _res_cfg(
        hf_hub_id="torchgeo/satlas",
        hf_hub_filename="sentinel2_resnet152_si_rgb-f4d24c3c.pth",
        input_size=(3, 224, 224),
    ),
}
...
def _create_resnet(variant, pretrained=False, **kwargs):
    return build_model_with_cfg(
        ...
        variant,
        pretrained,
        pretrained_cfg=resnet_updated_cfgs[variant],
...
hf_vit_cfgs = {
    ...
    "vit_small_patch16_224_landsat_tm_toa_moco": _vit_cfg(
        hf_hub_id="torchgeo/ssl4eo_landsat",
        hf_hub_filename="vits16_landsat_tm_toa_moco-a1c967d8.pth",
        input_size=(7, 224, 224),
    ),
    "vit_small_patch16_224_landsat_tm_toa_simclr": _vit_cfg(
        hf_hub_id="torchgeo/ssl4eo_landsat",
        hf_hub_filename="vits16_landsat_tm_toa_simclr-7c2d9799.pth",
        input_size=(7, 224, 224),
    ),
    "vit_small_patch16_224_landsat_etm_toa_moco": _vit_cfg(
        hf_hub_id="torchgeo/ssl4eo_landsat",
        hf_hub_filename="vits16_landsat_etm_toa_moco-26d19bcf.pth",
        input_size=(9, 224, 224),
    ),
    "vit_small_patch16_224_landsat_etm_toa_simclr": _vit_cfg(
        hf_hub_id="torchgeo/ssl4eo_landsat",
        hf_hub_filename="vits16_landsat_etm_toa_simclr-34fb12cb.pth",
        input_size=(9, 224, 224),
    ),
    "vit_small_patch16_224_landsat_etm_sr_moco": _vit_cfg(
        hf_hub_id="torchgeo/ssl4eo_landsat",
        hf_hub_filename="vits16_landsat_etm_sr_moco-eaa4674e.pth",
        input_size=(6, 224, 224),
    ),
    "vit_small_patch16_224_landsat_etm_sr_simclr": _vit_cfg(
        hf_hub_id="torchgeo/ssl4eo_landsat",
        hf_hub_filename="vits16_landsat_etm_sr_simclr-a14c466a.pth",
        input_size=(6, 224, 224),
    ),
    "vit_small_patch16_224_landsat_oli_tirs_toa_moco": _vit_cfg(
        hf_hub_id="torchgeo/ssl4eo_landsat",
        hf_hub_filename="vits16_landsat_oli_tirs_toa_moco-c7c2cceb.pth",
        input_size=(11, 224, 224),
    ),
    "vit_small_patch16_224_landsat_oli_tirs_toa_simclr": _vit_cfg(
        hf_hub_id="torchgeo/ssl4eo_landsat",
        hf_hub_filename="vits16_landsat_oli_tirs_toa_simclr-ad43e9a4.pth",
        input_size=(11, 224, 224),
    ),
    "vit_small_patch16_224_landsat_oli_sr_moco": _vit_cfg(
        hf_hub_id="torchgeo/ssl4eo_landsat",
        hf_hub_filename="vits16_landsat_oli_sr_moco-c9b8898d.pth",
        input_size=(7, 224, 224),
    ),
    "vit_small_patch16_224_landsat_oli_sr_simclr": _vit_cfg(
        hf_hub_id="torchgeo/ssl4eo_landsat",
        hf_hub_filename="vits16_landsat_oli_sr_simclr-4e8f6102.pth",
        input_size=(7, 224, 224),
    ),
    "vit_small_patch16_224_sentinel2_all_dino": _vit_cfg(
        hf_hub_id="torchgeo/vit_small_patch16_224_sentinel2_all_dino",
        hf_hub_filename="vit_small_patch16_224_sentinel2_all_dino-36bcc127.pth",
        input_size=(13, 224, 224),
    ),
    "vit_small_patch16_224_sentinel2_all_moco": _vit_cfg(
        hf_hub_id="torchgeo/vit_small_patch16_224_sentinel2_all_moco",
        hf_hub_filename="vit_small_patch16_224_sentinel2_all_moco-67c9032d.pth",
        input_size=(13, 224, 224),
    ),
}
