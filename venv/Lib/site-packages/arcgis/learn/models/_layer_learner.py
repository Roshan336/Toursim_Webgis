...
try:
    from ._arcgis_model import ArcGISModel, _raise_fastai_import_error, _EmptyData
...
def _get_inference_data_stub(data, path, device):
    out_dims = max(1, len([data._dependent_variable]))
...
def _get_learner_object(data, layers, emb_szs, ps, emb_drop, pretrained_path):
    ...
    if data._is_empty:
        databunch = _get_inference_data_stub(data, data.path, data._device)
        databunch._is_empty = True

    else:
        databunch = data._databunch

    cont_variable = data._continuous_variables

    if hasattr(databunch, "get_emb_szs"):
    ...
    if pretrained_path:
        model_path = os.path.join(
            ...
            os.path.basename(pretrained_path).rsplit(".", 1)[0],
        )
...
class FullyConnectedNetwork(ArcGISModel):
    ...
    **Parameter**            **Description**
    ...
    def from_model(cls, emd_path, data=None):
        ...
        **Parameter**            **Description**
        ...
        if data is None:
            ...
            data._device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
            class_object = cls(
                data, layers=layers, emb_szs=emb_szs, pretrained_path=emd_path
            )
            class_object._data.emd = emd
            class_object._data.emd_path = emd_path
            return class_object

        model = cls(data, layers=layers, emb_szs=emb_szs, pretrained_path=str(emd_path))
    ...
    def _get_emd_params(self, save_inference_file):
        ...
        _emd_template["emb_szs"] = self._emb_szs
    ...
    def _predict_features(
        ...
        save_data_handle = None
        if hasattr(self.learn.data, "_is_empty"):
            if self.learn.data._is_empty:
                temp_df = processed_dataframe.copy()
                if not self._data._is_classification:
                    temp_df[self._data._dependent_variable] = temp_df[
                        self._data._dependent_variable
                    ].astype("float64")
                temp_databunch = TabularDataBunch.from_df(
                    path=self._data.path,
                    df=temp_df,
                    dep_var=self._data._dependent_variable,
                    cat_names=self._data._categorical_variables,
                    cont_names=self._data._continuous_variables,
                    valid_idx=np.random.choice(
                        temp_df.index, size=min(1000, len(temp_df) // 2), replace=False
                    ).tolist(),
                )
                save_data_handle = self.learn.data
                self.learn.data = temp_databunch

        dataframe["prediction_results"] = self._df_predict(processed_dataframe.copy())
        if save_data_handle:
            self.learn.data = save_data_handle
