...
class Block(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        from ._terramind_utils import GatedMlp
        ...
        if pretrained:
            ...
            if backbone_name == "prithvi":
                if kwargs.get("item_id") is not None:
                    pretrained_path = weight_download_url_clay(
                        kwargs.get("item_id"), model_name="prithvi_models"
                    )
                    pretrained_path = os.path.join(
                        pretrained_path, kwargs.get("weight_name")
                    )
                init_prithvi(self, pretrained_path)
            elif backbone_name == "clay_large":
                temp_path = weight_download_url_clay(
                    kwargs.get("item_id"), model_name="clay_model"
                )
