...
def _patch_mmcv_deform_print_log():
    try:
        import mmcv.ops.deform_conv as deform_conv_mod
    except Exception:
        return

    if getattr(deform_conv_mod, "_arcgis_patched_print_log", False):
        return

    original = deform_conv_mod.print_log

    def patched_print_log(msg, *args, **kwargs):
        if (
            isinstance(msg, str)
            and "DeformConv2dPack" in msg
            and "upgraded to version 2." in msg
        ):
            return
        return original(msg, *args, **kwargs)

    deform_conv_mod.print_log = patched_print_log
    deform_conv_mod._arcgis_patched_print_log = True
