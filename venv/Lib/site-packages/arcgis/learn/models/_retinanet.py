...
class RetinaNet(ArcGISModel):
    ...
    **Parameter**            **Description**
    ...
    backbone                Optional string. Backbone convolutional neural network
                            ...
                            Supported backbones: ResNet, ViT families and specified Timm
                            models(experimental support) from :func:`~arcgis.learn.RetinaNet.backbones`.
                            For ViT based terramind_base and terramind_large backbones,
                            supported modalities are RGB, Sentinel-2 L2A (12 bands),
                            Sentinel-2 L1C(13 bands), Sentinel-1 GRD (VV and VH) and DEM
                            for optimal utilization of the backbone.
    ---------------------   -------------------------------------------
    ...
    **Parameter**            **Description**
    ...
    def _freeze(self):
        if isinstance(self.learn.model.encoder[0].backbone, ViT):
            pretrained_layers, random_layers = self.learn.model.encoder[
                0
            ].backbone._freeze()
            random_layers.extend(flatten_model(self.learn.model.c5top5))
            random_layers.extend(flatten_model(self.learn.model.c5top6))
            random_layers.extend(flatten_model(self.learn.model.p6top7))
            random_layers.extend(flatten_model(self.learn.model.smoothers))
            random_layers.extend(flatten_model(self.learn.model.classifier))
            random_layers.extend(flatten_model(self.learn.model.box_regressor))
