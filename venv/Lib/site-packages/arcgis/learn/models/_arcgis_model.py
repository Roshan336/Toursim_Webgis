...
class DDPSafeEarlyStoppingCallback(EarlyStoppingCallback):
    def on_epoch_end(self, epoch, **kwargs):
        ...
        if torch.distributed.is_available() and torch.distributed.is_initialized():
            ...
            if torch.cuda.is_available():
                t = torch.tensor(
                    [1 if should_stop else 0], device=torch.cuda.current_device()
                )
            else:
                t = torch.tensor([1 if should_stop else 0])
...
class ArcGISModel(object):
    ...
    def _create_html(path_model):
        ...
        if emd_template.get("per_class_metrics"):
            html_table = pd.read_json(
                io.StringIO(emd_template.get("per_class_metrics"))
            ).to_html()
            model_analysis = f"""
    ...
    def load(self, name_or_path, **kwargs):
        ...
        **Parameter**           **Description**
        ...
        try:
            ...
            if self.custom_load(name, device=device, load_exception=e, **kwargs):
                pass
            else:
                raise e
        finally:
    ...
    def custom_load(self, name, device, **kwargs):
        from fastai.torch_core import get_model

        load_exception = kwargs.get("load_exception", None)
        err_msg = str(load_exception) if load_exception is not None else ""

        if getattr(self._backbone, "__name__", None) == "efficientnet_b2_pruned":
            source = self.learn.path / self.learn.model_dir / f"{name}.pth"
            ckpt = torch.load(source, map_location=device)
            new_ckpt = dict()
            for k, v in ckpt.items():
                if "backbone.3" in k:
                    k = k.replace("backbone.3", "backbone.2")
                if "backbone.4" in k:
                    k = k.replace("backbone.4", "backbone.3")
                if "backbone.5" in k:
                    k = k.replace("backbone.5", "backbone.4")
                new_ckpt[k] = v

            get_model(self.learn.model).load_state_dict(
                new_ckpt, strict=kwargs.get("strict", "True")
            )
            return True
        ...
        # for backward compatibility
            ...
            hasattr(self.learn.model, "backbone")
            and (
                getattr(self.learn.model.backbone, "_is_vitdet", False)
                or getattr(self.learn.model, "_is_vitdet", False)
            )
        ) or ("size mismatch" in err_msg and "pos_embed" in err_msg):
            from fastai.torch_core import get_model
            ...
            for key in pos_embed_keys:
                if key in state_dict and key in model_state:
                    ...
                    model_pos = model_state[key]

                    if ckpt_pos.shape != model_pos.shape:

                        def _grid(n):
                            if int(math.sqrt(n - 1)) ** 2 == (n - 1):
                                return int(math.sqrt(n - 1)), True
                            return int(math.sqrt(n)), False

                        ckpt_g, ckpt_cls = _grid(ckpt_pos.shape[1])
                        model_g, model_cls = _grid(model_pos.shape[1])
                        ...
                        grid = grid.reshape(1, ckpt_g, ckpt_g, -1).permute(0, 3, 1, 2)
                        grid = F.interpolate(
                            grid,
                            size=(model_g, model_g),
                            mode="bilinear",
                            align_corners=False,
                        )
                        grid = grid.permute(0, 2, 3, 1).reshape(
                            1, model_g * model_g, -1
                        )

                        if model_cls:
                            if cls_tok is None:
                                cls_tok = torch.zeros_like(model_pos[:, :1])
                            state_dict[key] = torch.cat([cls_tok, grid], dim=1)
                        else:
                            state_dict[key] = grid
