    ...
    **kwargs**
    ...
    def _predict_features(
        ...
        **kwargs,
        ...
        if "Embedding" in processed_dataframe.columns:
            processed_dataframe, expanded_embedding_cols = _expand_embedding_column(
                processed_dataframe, embedding_col="Embedding"
            )

        for column in processed_dataframe.columns:
            if column not in fields_needed:
                if "emb_" not in column and not column.startswith("embedding_"):
                    processed_dataframe = processed_dataframe.drop(column, axis=1)
        if self._data._embedding_variables is None:
            self._data._embedding_variables = []

        if any(col.startswith("embedding_") for col in processed_dataframe.columns):
            expanded_cols = [
                col
                for col in processed_dataframe.columns
                if col.startswith("embedding_")
            ]
            self._data._embedding_variables = expanded_cols
