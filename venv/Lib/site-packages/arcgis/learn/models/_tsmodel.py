...
try:
    from ._arcgis_model import ArcGISModel, _EmptyData, _raise_fastai_import_error
...
def _get_inference_data_stub(data, path, device, step):
    out_dims = max(1, len(data._dependent_variable) * step)
    stub = _EmptyData(Path(path), out_dims, torch.nn.MSELoss(), 1, train_ds=False)
    stub.features = len(
...
def _create_ts_learner(data_bunch, data_path, model_arch, seq_len, device, **kwargs):
    ...
    if model_arch.lower() == "lstm":
        model = model_arch_ob(data_bunch.features, data_bunch.c, device, **kwargs).to(
            device
        )
    elif model_arch.lower() == "timeseriestransformer":
        model = model_arch_ob(data_bunch.features, data_bunch.c, seq_len, **kwargs).to(
            device
        )
        ...
        model = model_arch_ob(config, **kwargs).to(device)
    else:
        if model_arch.lower() in ["resnet", "fcn"]:
            kwargs["device"] = device
        model = model_arch_ob(data_bunch.features, data_bunch.c, **kwargs).to(device)
        if model_arch.lower() in ["resnet", "fcn"]:
            del kwargs["device"]

    if model_arch.lower() == "timemoe":
        learn = Learner(
            data_bunch, model, loss_func=model.time_moe_loss, path=data_path
        )
    else:
        learn = Learner(data_bunch, model, path=data_path)

    learn.data = data_bunch
    learn.layer_groups = split_model_idx(learn.model, [1])
    return learn
    ...
    **Parameter**            **Description**
    ...
    def __init__(
        ...
        **kwargs,
        ...
        elif not kwargs.get("model_extension", False):
            data_bunch = _get_inference_data_stub(
                data, data.path, self._device, self.step
            )

        self.model_extension = False
        self.inference_model = None
        model_kwargs = kwargs.copy()
        model_kwargs.pop("pretrained_path", None)
        model_kwargs.pop("step", None)
        ...
        if kwargs.get("model_extension", False):
            ...
            if kwargs.get("pretrained_path"):
                ...
                self.load(kwargs.get("pretrained_path"))
    ...
    def _add_predict_rows(
        ...
        if delta is not None:
            tindex = self._generate_prediction_index(
                end_value, delta, number_of_predictions
            )
            orig_dataframe.loc[
                orig_dataframe.tail(number_of_predictions).index,
                index_field_name,
            ] = tindex
            orig_dataframe[index_field_name] = pd.to_datetime(
                orig_dataframe[index_field_name]
            )
        if len(datetime_dict):
            for key, value in datetime_dict.items():
                ...
                tindex = self._generate_prediction_index(
                    end_value_temp, new_delta, number_of_predictions
                )
                orig_dataframe.loc[
                    orig_dataframe.tail(number_of_predictions).index, key
                ] = tindex
                orig_dataframe[key] = pd.to_datetime(orig_dataframe[key])

        return orig_dataframe

    def _generate_prediction_index(self, end_value, delta, number_of_predictions):
        if number_of_predictions <= 0:
            return []
        return [
            end_value + (delta * step) for step in range(1, number_of_predictions + 1)
        ]
    ...
    def _apply_transform(self, processed_dataframe, processed_dataframe_transform):
        ...
        # have any NA label while the test data has because of nature of the prediction.
        for col in list(processed_dataframe.columns):
            ...
            if pd.api.types.is_datetime64_any_dtype(processed_dataframe[col]):
                processed_dataframe_transform[col] = pd.Series(
                    np.nan, index=processed_dataframe_transform.index, dtype="float64"
                )
            for transform in self._data._column_transforms_mapping.get(col, []):
                if isinstance(transform, LabelEncoder):
                    ...
                    if pd.api.types.is_datetime64_any_dtype(transformed_data):
                        transformed_data = transformed_data.to_numpy(
                            dtype="datetime64[ns]"
                        ).astype("int64")
                        transformed_data = transformed_data.astype("float64")
                        transformed_data[
                            pd.isna(processed_dataframe[col]).to_numpy()
                        ] = np.nan
                        transformed_data = transform.transform(
            ...
            processed_dataframe_transform.loc[
                processed_dataframe_transform.index[: len(transformed_data)], col
            ] = np.array(
                transformed_data,
                dtype=(
                    "float64"
                    if pd.api.types.is_datetime64_any_dtype(processed_dataframe[col])
                    else type(processed_dataframe[col].iloc[0])
                ),
            )
