    ...
    **Parameter**            **Description**
    ...
    def update(self, frame, **kwargs):
        ...
        **Parameter**            **Description**
        ...
        for i, track in enumerate(self.track_list):
            bbox_ = self.track_list[i].bbox
            self.track_list[i].bbox = [to_pyfloat(v) for v in bbox_]
        return self.track_list
            ...
            for j in range(0, 3):
                ...
                if image_counter == 0:
                    ...
                    for track in tracks:
                        ...
                        cv2.polylines(
                            image,
                            [track.location.astype(np.int32).reshape((-1, 1, 2))],
                            True,
                            (w, 255, h),
                            3,
                        )
    ...
    def compute_metrics(self, iou_thres=0.2):
        ...
        **Parameter**            **Description**
        ...
        if all_ious is None or np.size(all_ious) == 0:
            mean_iou = -1.0
        else:
            try:
        ...
        return {
            "mean_IOU": (
                mean_iou.item() if isinstance(mean_iou, np.generic) else float(mean_iou)
            ),
            "f_measure": (
                f_measure.item()
                if isinstance(f_measure, np.generic)
                else float(f_measure)
            ),
        }
