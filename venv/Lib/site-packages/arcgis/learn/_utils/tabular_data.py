...
def _decode_embedding_b64(s: str) -> np.ndarray:
    arr = np.frombuffer(base64.b64decode(s), dtype=np.float32).copy()
    arr[np.isnan(arr)] = 0.0
    return arr


def _expand_embedding_column(dataframe, embedding_col="Embedding"):

    if embedding_col not in dataframe.columns:
        return dataframe, []

    # Decode all embeddings and create a 2D array
    decoded_embeddings = dataframe[embedding_col].apply(
        lambda x: _decode_embedding_b64(x)
    )

    n_dims = len(decoded_embeddings.iloc[0])
    embedding_col_names = [f"embedding_{i+1}" for i in range(n_dims)]

    embedding_df = pd.DataFrame(
        decoded_embeddings.tolist(), columns=embedding_col_names, index=dataframe.index
    )
    dataframe = dataframe.drop(columns=[embedding_col])
    dataframe = pd.concat([dataframe, embedding_df], axis=1)

    return dataframe, embedding_col_names
...
class TabularDataObject(object):
        ...
        **kwargs,
        ...
        for col_name in dataframe.columns:
            try:
                ...
                current_col_max_len = _max_str_len_chunked(dataframe[col_name])
        ...
        if "Embedding" in continuous_variables:
            continuous_variables.remove("Embedding")
        if "Embedding" in dataframe.columns:
            dataframe, expanded_embedding_cols = _expand_embedding_column(
                dataframe, embedding_col="Embedding"
            )
            new_embd_cols.extend(expanded_embedding_cols)

        dataframe_columns = dataframe.columns
        if distance_feature_layers:
            all_prefix = defaultdict(int)
            prefix_list = []
            count = 1
            for distance_layer in distance_feature_layers:
                prefix, all_prefix = _prefix_builder(all_prefix, distance_layer, count)
                prefix_list.append(prefix)
                count += 1
            for prefix in prefix_list:
                if f"DIST_{prefix}" in dataframe_columns:
                    continuous_variables.append(f"DIST_{prefix}")
    ...
    def _sdf_gptool_workflow(
        ...
        if not is_table_obj:
            ...
            count = 1
            all_prefix = defaultdict(int)
            # fetch the column using arcpy
            field_names = [f.name for f in arcpy.ListFields(data_source)]
            for distance_layer in distance_feature:
                # fetch the layer name
                prefix, all_prefix = _prefix_builder(all_prefix, distance_layer, count)

                field_2 = f"DIST_{prefix}"
                if (
                    field_2 in field_names
                ):  # dropping this as this does not work on other machine
                    arcpy.management.DeleteField(data_source, [field_2])
                fields = arcpy.ValueTable(2)
                fields.addRow(["NEAR_DIST", field_2])
        ...
        if input_features is not None:
            ...
            if input_layer is not None and distance_layers:
                ...
                for distance_layer in distance_layers:
                    prefix, all_prefix = _prefix_builder(
                        all_prefix, distance_layer, count
                    )
...
def _prefix_builder(all_prefix, layer_name, count, max_length=10):
    try:
        import arcpy
    except:
        raise Exception("This method needs arcpy to be installed. Unable to continue")
    try:
        desc = arcpy.Describe(layer_name).name
    except:
        desc = str(count)
    desc = desc.rsplit(".", 1)[0]
    parts = desc.split("_")
    max_length = max_length - 5  # because first 5 Will be taken by DIST_
    base_prefix = "_".join(p[:max_length] for p in parts[:1])
    current_count = all_prefix.get(base_prefix, 0)
    if current_count == 0:
        prefix = base_prefix
    else:
        # shorten the base prefix if adding the count exceeds max_length
        if len(base_prefix) + len(str(current_count)) + 1 > max_length:
            base_prefix = base_prefix[: max_length - len(str(current_count)) - 1]
        prefix = f"{base_prefix}_{current_count}"
    all_prefix[base_prefix] = current_count + 1
    return prefix, all_prefix


def _max_str_len_chunked(series, chunk_size=1000):
    max_len = 0
    n = len(series)
    for i in range(0, n, chunk_size):
        chunk = series.iloc[i : i + chunk_size]
        chunk_max = chunk.astype(str).str.len().max()  # convert only chunk
        if pd.notna(chunk_max):
            max_len = max(max_len, int(chunk_max))
    return max_len
