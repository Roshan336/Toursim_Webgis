...
def aggregate_points(
    ...
    **Parameter**                            **Description**
    ...
    if estimate and not bin_type:
        return Exception("`bin_type` must be specified when estimate is True.")
    if bin_type and not bin_size and not bin_size_unit:
        return Exception(
            "`bin_size` and `bin_size_unit` must be specified when `bin_type` is specified."
        )
    kwargs = {
...
def summarize_within(
    ...
    **Parameter**                            **Description**
    ...
    sum_within_layer                        Required :class:`~arcgis.features.FeatureLayer` . The polygon features. Features, or
                                            ...
                                            This value can be `None` if the bin_size, bin_size_unit and bin_type are defined.
    -------------------------------------   ---------------------------------------------------------
    ...
    bin_type                                Required string. The type of bin used to calculate density.
                                            If sum_within_layer is not provided, then bin_type must be specified or it will default to "Square".
    ...
    """
    if sum_within_layer is None:
        sum_within_layer = ""
    if sum_within_layer is None and bin_type is None:
        # only set default if sum_within_layer is not given and user did not specify bin_type
        bin_type = "Square"
    if bin_type is None:
        bin_type = ""
    gis = _arcgis.env.active_gis if gis is None else gis
