...
def find_existing_locations(
    ...
    **Parameter**                           **Description**
    ------------------------------------    ------------------------------------------------------------------------------------------------------
    input_layers                            Required list of input layers that will be used in the *expressions* argument.
                                            Each layer in the list can be either:

                                            * a Feature Service :class:`layer <arcgis.features.FeatureLayer>` with an optional filter to select
                                              specific features
                                            * a :class:`feature collection <arcgis.features.FeatureCollection>`
    ...
    expressions                             Required list of dicts. There are two types of expressions, attribute and spatial.
                                            ...
                                            Options for symbols to use in *where* clause comparisons:
                                            ...
                                            .. note::
                                                ...
                                                * units is the units for distance. Units choice list:
    ...
    ------------------------------------    ------------------------------------------------------------------------------------------------------
    output_name                             Optional string or :class:`~arcgis.features.FeatureLayer` . Existing feature layer will cause the new
                                            *layer* to be appended to the :class:`Feature Service <arcgis.features.FeatureLayerCollection>`.

                                            * If *overwrite* is *True* in *context* argument, the new layer will overwrite existing *layer*.
                                            * If *output_name* is not provided, then a new :class:`~arcgis.features.FeatureCollection` is created.
    ------------------------------------    ------------------------------------------------------------------------------------------------------
    context                                 Optional dict. Additional settings such as processing extent and output spatial reference. For this
                                            operation, there are three settings.
                                            ...
                                            .. code-block:: python
                                                ...
                                                # Example Formatting for context argument

                                                result = find_existing_locations(
                                                            ...
                                                            context = {
                                                              "extent": {
                                                                "xmin": 3164569.408035,
    ...
    ------------------------------------    ------------------------------------------------------------------------------------------------------
    gis                                     Optional. The :class:`~arcgis.gis.GIS` in which this tool runs. If not specified, the active GIS is used.
    ------------------------------------    ------------------------------------------------------------------------------------------------------
    estimate                                Optional boolean. If True, returns a credits estimate as a float.

                                            .. note::
                                                **Deprecated** at 2.4.3. In the next release it will return an EstimateResult pydantic dataclass
                                                object.
    ------------------------------------    ------------------------------------------------------------------------------------------------------
    future                                  Optional boolean.

                                            * If *True*, a *future* object will be returned and the process will not wait for the task to complete
                                              before returning control.
                                            * If *False* (the *default*), control is returned after results are processed.
    ====================================    ======================================================================================================
...
def derive_new_locations(
    ...
    **Parameter**                               **Description**
    ...
    -------------------------------------       ------------------------------------------------------------------------------------------------------
    estimate                                    Optional boolean. If True, returns a credits estimate. Deprecated: currently returns a float; in the next release it will return an EstimateResult pydantic dataclass.
    -------------------------------------       ------------------------------------------------------------------------------------------------------
...
def find_similar_locations(
    ...
    **Parameter**                **Description**
    ...
    future                      Optional boolean.
                                ...
                                * If *True*, a ``Future``  object is returned. Call ``result()`` on that object to get the response.
                                * If *False*, an :class:`~arcgis.gis.Item` is returned if *output_name* is specified, otherwise, a Python
                                dicitionary. The default is *False*.
    -----------------------     -------------------------------------------------------------------------------------------
    ...
    :return:
        ...
        * *similar_result_layer* : :class:`~arcgis.gis.Item` if *output_name* is set, else :class:`~arcgis.features.FeatureCollection`
    ...
    .. code-block:: python
        ...
        # Access output item
        top_4_most_similar_locations["similar_result_layer"]
