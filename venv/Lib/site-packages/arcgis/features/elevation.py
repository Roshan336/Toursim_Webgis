...
def profile(
    ...
    .. note::
        In order for the service to return *z* and or *m* values in the
        :class:`~arcgis.features.FeatureSet` coordinate results,
        you must set the :class:`~arcgis.env.return_z` and/or
        :class:`~arcgis.env.return_m` environment variables to *True*.
    ...
    **Parameter**                             **Description**
    ...
    :return: Output Profile as a :class:`~arcgis.features.FeatureSet` object.
    ...
    .. code-block:: python
        ...
        flayer_item = gis.content.get("<item_id>")

        for idx, lyr in enumerate(l_item.layers):
            print(f"{idx:<3}{lyr.properties.name} geometry type: {lyr.properties.geometryType}")

       >>> 0  Hospitals geometry type: esriGeometryPoint
           1  ElectricTransmissionLines geometry type: esriGeometryPolyline
           2  ElectricTransmissionStations geometry type: esriGeometryPolygon

        line_lyr = l_item.layers[1]
        line_fset = line_lyr.query(
            where="POWER = 'minor_underground_cable'",
            return_z=True,
            return_m=True,
            as_df=False
        )
        ...
        prof = profile(
            input_line_features=line_fset,
            dem_resolution="FINEST",
            maximum_sample_distance=500,
            maximum_sample_distance_units="Meters"
        )

        print(prof.features[0])

        >>>
            {"geometry": {"hasZ": true, "hasM": true, "paths": [
                            [[19515335.33, -4545517.789999999, 39.3752999999997, 0],
                             [19515404.1299, -4545691.130100001, 39.19770000000426, 146.25329999999667],
                             ...
                             [19515558.060000002, -4546031.66, 39.927700000000186, 462.89569999999367]]
                            ], "spatialReference": {"wkid": 102100, "latestWkid": 3857}
                         },
             "attributes": {"OBJECTID": 1, "DEMRes": "24m",
                            "ProductName": "Airbus_WorldDEM4Ortho_24m",
                            "ProfileLength": 462.895717785818, "Shape_Length": 586.8552525033391
                           }
            }
    """
