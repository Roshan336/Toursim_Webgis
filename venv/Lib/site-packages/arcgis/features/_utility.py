    ...
    **Inputs**              **Description**
    ...
    def validate_topology(
        ...
        **Parameter**                                **Description**
        ...
        out_sr                                      Optional integer. The output spatial reference.
        ------------------------------------        --------------------------------------------------------------------
        validation_type                             This parameter defines the specific validate operation to be
                                                    performed. While `normal` is the default setting, ArcGIS Enterprise
                                                    10.9 introduced the `rebuild` and `forceRebuild` options as specialized
                                                    tools to repair inconsistencies and rebuild the network topology.

                                                    Both `rebuild` and `forceRebuild` perform a build to reconstruct the
                                                    specified portions of network topology, but they handle data
                                                    differently:

                                                    `normal`: Performs standard validation based on existing service
                                                    logic.

                                                    `rebuild`: Executes a repair using the dirty areas associated with the
                                                    provided `validateArea` or `validationSet` values.

                                                    `forceRebuild`: Ignores existing dirty areas entirely, applying the
                                                    reconstruction to all features within the specified `validateArea` or
                                                    `validationSet`.

                                                    Values: `normal` (default) | `rebuild` | `forceRebuild`
        ------------------------------------        --------------------------------------------------------------------
        future                                      Optional boolean. If `True`, the request is processed as an asynchronous
                                                    job and a URL is returned that points a location displaying the status
                                                    of the job.
