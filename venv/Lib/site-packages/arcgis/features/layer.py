...
class FeatureLayer(Layer):
    """
    The primary class for working with :class:`~arcgis.features.Feature` objects
    in a :class:`~arcgis.gis.GIS`.

    A :class:`user <arcgis.gis.User>` can create, import, export, analyze, edit,
    and visualize real-world entities in space as *features* accessed in
    *feature layers*.

    ``Feature layers`` can be added to and visualized using maps. They act as inputs
    to and outputs from *feature analysis* tools.

    Feature layers are created by adding content and publishing feature services to a
    *GIS*. The feature service is exposed as a broader :class:`~arcgis.gis.Item`
    resource in the :class:`~arcgis.gis.GIS`, and the *feature layers* are accessed
    using the *layers* attribute on *Item* objects of type feature layer.

    .. note::
        When leveraging Esri hosted content, organizations should review the
        `ArcGIS Online terms of use <https://doc.arcgis.com/en/arcgis-online/reference/terms-of-use.htm>`_,
        as well as the terms of use for the data layer to ensure they are in
        compliance with extracting data and/or making it available in other
        systems.
    """
    ...
    def fromitem(cls, item: Item, layer_id: int = None, id_property: int = None):
        """
        Creates a :class:`~arcgis.features.FeatureLayer` from an :class:`~arcgis.gis.Item`
        object, provided the *item* is of type *feature service*.

        ===============================     ====================================================================
        **Parameter**                       **Description**
        -------------------------------     --------------------------------------------------------------------
        item                                Required :class:`~arcgis.gis.Item` object of type *Feature Service*.

                                            .. note::
                                                A *feature service* is represented in the API as a
                                                :class:`~arcgis.features.FeatureLayerCollection`.
        -------------------------------     --------------------------------------------------------------------
        layer_id                            Optional Integer. The index position of the layer in either:

                                            * The *FeatureLayerCollection.layers* list or the
                                            * The *Item.layers* list.

                                            This is a zero-based value from a Python list.

                                            .. code-block:: python

                                                # Values for *layer_id* argument from Feature Layer item
                                                ...
                                                for idx,layer in enumerate(flyr_item.layers):
                                                    print(f"{layer.properties.name} -> index position {idx})

                                                >>> Fire_districts -> index position 0
                                                    Neighborhood_census2010 -> index position 1
        -------------------------------     --------------------------------------------------------------------
        id_property                         Optional Integer. The *id* property from the *Feature Layer*
                                            definition, which is accessed using the `FeatureLayer.properties`
                                            attribute.

                                            .. code-block:: python

                                                # Values for *id_property* argument from Feature Layer object
                                                from arcgis.gis import GIS
                                                from arcgis.features import FeatureLayer
                                                ...
                                                for flayer in flyr_item.layers:
                                                    flayer_definition = flayer.properties
                                                    print(f"{flayer_definition.id} -> {flayer_definition.name})

                                                >>> 8 -> Fire_Districts
                                                    12 -> Neighborhood_census2010
        ===============================     ====================================================================
        ...
        .. code-block:: python
            ...
            # Usage Example: Feature layer Item layers list (index and id values match)

            from arcgis.gis import GIS
            gis = GIS(profile="your_organization_profile")

            feat_lyr_items_list = gis.content.search(
                                          query="river *",
                                          item_type="Feature Layer")

            feat_lyr_item = feat_lyr_items_list[0]
            feat_lyr_item.type

            >>> 'Feature Service'

            print(f"Index ID Prop Layer Name")
            print(f"{'-' * 5} {'-' * 7} {'-' * 25}")
            for index, lyr in enumerate(feat_lyr_item.layers):
                print(f"{index:^6}{lyr.properties.id:^8}{lyr.properties.name}")

           >>>  Index ID Prop Layer Name
                ----- ------- -------------------------
                  0      0    Non_Populated_Places
                  1      1    Populated_Place_Names
                  2      2    Jackson River Wetlands
                  3      3    Jackson River Riparian Areas
                  4      4    City_Park_Boundary
                  5      5    Parcel Boundaries
                  6      6    Administrative_Boundary

            flyr = FeatureLayer.fromitem(item=feat_layer_item, layer_id=2)
            type(flyr)

            >>> <class 'arcgis.features.layer.FeatureLayer'>

            flyr.properties.name

            >>> 'Jackson River Wetlands'

            # Usage Example 2: Feature Layer Collection layers list (index and id differ)

            from arcgis.gis import GIS
            from arcgis.features import FeatureLayer

            gis = GIS("pro")
            flyr_item_list = gis.content.search(
                                query="owner:"+ gis.users.me.username,
                                item_type="Feature Layer")
            flyr_item = flyr_item_list[0]
            flyr_item.type

            >>> 'Feature Service'

            feat_lyr_coll = flyr_item.layers[0].container
            type(feat_lyr_coll)
            ...
            print(f"Index ID Prop Layer Name")
            print(f"{'-' * 5} {'-' * 7} {'-' * 25}")
            for index, lyr in enumerate(feat_lyr_coll.layers):
                print(f"{index:^6}{lyr.properties.id:^8}{lyr.properties.name}")

            >>> Index ID Prop Layer Name
                ----- ------- -------------------------
                  0      8    Fire_Districts
                  1      12   Neighborhood_census2010

            flyr8 = FeatureLayer.fromitem(item= flyr_item, id_property=8)
            flyr8.properties.name

            >>> 'Fire_Districts'
        """
        if layer_id is None and id_property is None:
            layer_id = 0
        if not layer_id is None:
            return FeatureLayerCollection.fromitem(item).layers[layer_id]
        if not id_property is None:
            _valid_ids = [lyr.properties.id for lyr in item.layers]
            if not id_property in _valid_ids:
                raise (
                    f"The id property {id_property} is not valid id of layers in Feature Service."
                )
            flyr_url = [
                flyr.url for flyr in item.layers if flyr.properties.id == id_property
            ][0]
            return FeatureLayer(flyr_url, item._gis)
    ...
    def _update_attachment(
        ...
        """
        Updates an existing attachment with a new file or keyword or both.
        ...
        **Parameter**          **Description**
        ...
        -----------------     --------------------------------------------------------------------
        file_path             Optional string. Path to new attachment. Required for Enterprise.
        -----------------     --------------------------------------------------------------------
        return_moment         Optional boolean. Specify whether the response will report the time
                              attachments were deleted. If True, the server will report the time
                              in the response's `editMoment` key. The default value is False.
        -----------------     --------------------------------------------------------------------
        keywords              Optional string. Keyword for attachment.
        =================     ====================================================================
        ...
        if (
            ...
            raise ValueError(
                "`file_path` must be provided when updating keywords in Enterprise"
            )
        if keywords is not None:
            params["keywords"] = keywords
        files = {"attachment": file_path} if file_path is not None else None
        if self._dynamic_layer is not None:
    ...
    def query_top_features(
        ...
        """
        Returns a :class:`~arcgis.features.FeatureSet` or
        :class:`spatially enabled dataframe <arcgis.features.GeoAccessor>`
        based on the top number of features ordered and grouped by the field
        name criteria entered as the *top_filter* dictionary argument.

        The operation also has many of the same parameters as the *Feature
        layer* :meth:`~arcgis.features.FeatureLayer.query` operation. However,
        unlike the layer query operation, it does not support parameters such as
        *outStatistics* and its related parameters, or *return_distinct_values*.

        The *Feature Layer* must have the *supportsTopFeaturesQuery* key set to
        True in the *advancedQueryCapabilities* property to support this
        operation. See the
        `Query Top Features <https://developers.arcgis.com/rest/services-reference/enterprise/query-top-features-feature-servicelayer/>`_
        REST API documentation for detailed explanation of operation processing.

        ================================     ====================================================================
        **Parameter**                         **Description**
        --------------------------------     --------------------------------------------------------------------
        top_filter                           Required Dict. Defines the aggregation of the data to return. The
                                             dictionary keys with description of value:

                                             * *groupByFields* - the field or fields used to aggregate results.
                                             * *topCount* - the number of features returned for each result group
                                             * *orderByFields* - the order in which features will be returned by
                                               specifying either *asc* (the *default*) or *desc* after the field
                                               name(s)

                                             .. code-block:: python

                                                 # Example argument formatting for top 5 counties grouped by
                                                 # state name in descending order of population

                                                 flyr_item = gis.content.get("<item_id>")
                                                 flyr = flyr_item.layers[0]

                                                 result = flyr.query_top_features(
                                                                top_filter={
                                                                    "groupByFields": "State_Name",
                                                                    "topCount": 5,
                                                                    "orderByFields": "Cnty_Population desc"
                                                                },
                                                                ...
                                                )
        --------------------------------     --------------------------------------------------------------------
        where	                             Optional String. A clause to filter results. SQL '92 WHERE
                                             clause syntax on the fields in the layer is supported for most data
                                             sources.
        --------------------------------     --------------------------------------------------------------------
        objectids	                         Optional List. Specific object IDs to query.
        --------------------------------     --------------------------------------------------------------------
        start_time                           Optional Datetime. The starting time to query for.

                                             .. note::
                                                 Only supported if *timeInfo* is in the Feature Layer properties
        --------------------------------     --------------------------------------------------------------------
        end_time                             Optional Datetime. The end date to query for.
                                             ...
                                             .. note::
                                                 Only supported if *timeInfo* is in the Feature Layer properties
        --------------------------------     --------------------------------------------------------------------
        geometry_filter                      Optional *arcgis.geometry.filter* object. Filter the results based
                                             on the spatial relationship with another geometry.
        --------------------------------     --------------------------------------------------------------------
        out_fields                           Optional String. The list of fields to include in the results.
        --------------------------------     --------------------------------------------------------------------
        ...
        return_centroid                      Optional Boolean. If *True*, the centroid of the geometry will be
                                             added to the output.
        --------------------------------     --------------------------------------------------------------------
        max_allowable_offset                 Optional float. Specifies the largest offset for generalizing
                                             geometries returned by the operation. This is set in the the units
                                             of the *out_sr* argument. If *out_sr* is not specified, the value is
                                             assumed to be in the Feature Layers's spatial reference.
        --------------------------------     --------------------------------------------------------------------
        out_sr                               Optional Integer. The spatial reference WKID value for results.
        --------------------------------     --------------------------------------------------------------------
        geometry_precision                   Optional Integer. Specifies the number of decimal places in the
                                             response geometries.

                                             .. note::
                                                 Applies **only** to X and Y values (not m or z-values).
        --------------------------------     --------------------------------------------------------------------
        return_ids_only                      Optional boolean.

                                             * If *True*, response only includes an array of object IDs
                                             * If *False*, the *default*, response is an object
        --------------------------------     --------------------------------------------------------------------
        return_extents_only                  Optional boolean.

                                             * If *True*, response only includes the extent of features returned
                                               in the results
                                             * If *False*, the *default*, an object is returned.

                                             .. note::
                                                 Feature Layer must have *supportsReturningQueryExtent* as *True*
                                                 in its properties to support this operation.
        --------------------------------     --------------------------------------------------------------------
        order_by_field                       Optional Str. Optional string. One or more field names on which the
                                             ...
                                             or descending order, respectively, following each field to control the
                                             ordering.

                                             .. code-block:: python

                                                 # Usage Example: Parameter formatting

                                                 flyr.query_top_features(
                                                    ...
                                                    order_by_field= "STATE_NAME ASC"
                                                    ...
                                                 )
        --------------------------------     --------------------------------------------------------------------
        return_z                             Optional boolean.

                                             * If *True*, if *Z* values are present, included in the results
                                             * If *False*, no *Z* values are returned.
        --------------------------------     --------------------------------------------------------------------
        return_m                             Optional boolean.

                                             * If *True*, if *M* values are present, included in the results
                                             * If *False*, no *M* values are returned.
        --------------------------------     --------------------------------------------------------------------
        result_type                          Optional String. Controls the number of features returned by the
                                             ...
                                             * *tile*
                                             ...
                                             .. note::
                                                 Feature Layer properties must have *supportQueryWithResultType*
                                                 set to *True* to support this parameter. See the
                                                 `Considerations for resultType <https://developers.arcgis.com/rest/services-reference/enterprise/query-top-features-feature-servicelayer/#considerations-for-resulttype->`_
                                                 documentation for details.
        --------------------------------     --------------------------------------------------------------------
        as_df                                Optional Boolean.

                                             * If *True*, the *default*, results returned as pandas *DataFrame*
                                               with a *spatial* property that returns a
                                               :class:`spatially enabled dataframe <arcgis.features.GeoAccessor>`
                                             * If *False*, results returned as a :class:`~arcgis.features.FeatureSet`
        ================================     ====================================================================


        :return:
            * *Default* is a pandas *DataFrame* containing a *spatial* property to access
              a :class:`spatially enabled dataframe <arcgis.features.GeoAccessor>`
            * If *as_df=False*, returns a :class:`~arcgis.features.FeatureSet`.
            * If *return_ids_only=True*, returns a list of values.

        .. code-block:: python

            # Usage Example: Medical Facilities with most licensed beds by facility type
            ...
            print("advancedQueryCapabilities" in flyr.properties)
            >>> True

            print("supportsTopFeaturesQuery" in flyr.properties["advancedQueryCapabilities"])
            >>> True

            res = flyr.query_top_features(
                        top_filter={
                            "groupByFields":"TYPE",
                            "topCount":2,
                            "orderByFields":"LICENSEDBE asc"
                        }
                    )
            ...
            >>>
                FID	ADDRESS	            LICENSEDBE ...     TYPE               SHAPE
            0	109	9622 INTERSTATE 12    819	     	SURGICAL HOSPITAL     {"x": -1028...}
            1	114	TWO ST.MARK'S CR      600         	SURGICAL HOSPITAL     {"x": -1027...}
            2	3	408 E BRADLEY ST       37	        GENERAL HOSPITAL      {"x": -1024...}
            3	2	1014 UNITY RD	       30	        GENERAL HOSPITAL      {"x": -1023...}
            4	64	385 N PALM Dr	      217	        PSYCHIATRIC HOSPITAL  {"x": -1027...}
            5	62	115 FINANCIAL CTR     124	        PSYCHIATRIC HOSPITAL  {"x": -1028...}
        ...
        if start_time and not end_time:
            time = "%s, null" % start_time
        elif end_time and not start_time:
            time = "null, %s" % end_time
        elif start_time and end_time:
            time = "%s, %s" % (start_time, end_time)
        params = _query.QueryParameters(
            where=where or "1=1",
            ...
            order_by_fields=order_by_field,
            return_extent_only=return_extents_only,
        )
        url = self._url + "/queryTopFeatures"
        return _query.Query(self, params, as_df=as_df, url=url).execute()
...
class Table(FeatureLayer):
    ...
    def fromitem(cls, item: Item, layer_id: int = None, id_property: int = None):
        """
        Returns a :class:`~arcgis.features.Table` object from an
        :class:`~arcgis.gis.Item` of type *Feature Service*.

        ===============================     ====================================================================
        **Parameter**                        **Description**
        -------------------------------     --------------------------------------------------------------------
        item                                Required :class:`~arcgis.gis.Item` object. *item* argument object
                                            must have a type property of *Feature Service*.

                                            .. code-block:: python

                                                # Usage Example: Reading *type* property of an item

                                                from arcgis.gis import GIS
                                                gis = GIS(profile="your_organization_profile")

                                                my_item_list = gis.content.search("*", "Feature Layer")
                                                my_flyr_item = my_item_list[0]
                                            ...
                                            .. note::
                                                A *feature service* item is represented in the API as a
                                                :class:`~arcgis.features.FeatureLayerCollection` object.
        -------------------------------     --------------------------------------------------------------------
        layer_id                            Optional Integer. The index position in either:

                                            * the *tables* property of the `~arcgis.features.FeautureLayerCollection`
                                            * or the *tables* property of the `~arcgis.gis.Item` object

                                            This is a zero-based indexed value from a Python list.
        -------------------------------     --------------------------------------------------------------------
        id_property                         Optional Integer. The *id* property for the *table* in the *Feature
                                            Layer's* definition, which is accessed using *properties* of a
                                            :class:`~arcgis.features.Table` object.

                                            .. code-block:: python

                                                # Get id property from a Table object

                                                from arcgis.gis import GIS
                                                gis = GIS(profile="your_organization_profile")

                                                tbl_item = [
                                                    t for t in gis.content.search(query="*", item_type="Feature Service")
                                                    if "Table" in t.typeKeywords
                                                ]
                                                tbl = tbl_item.tables[0]
                                                table_definition = tbl.properties
                                                table_definition.id
        ...
        .. code-block:: python
            ...
            # Usage Example: Return a table object using id property
            ...
            feat_svc_item = gis.content.get(<item_id>)
            feat_svc_item.type

            >>> `Feature Service`

            print(f"Index ID Prop Table Name")
            print(f"{'-' * 5} {'-' * 7} {'-' * 25}")
            for index, tbl in enumerate(feat_svc_item.tables):
                print(f"{index:^6}{tbl.properties.id:^8}{tbl.properties.name}")

            >>> Index ID Prop Table Name
                ----- ------- -------------------------
                  0      6    Conductor Information
                  1      9    PowerLine_accessories
                  2      4    PowerLine_location_desc

            accessory_tbl = Table.fromitem(
                                    item = feat_svc_item,
                                    id_property=9)
            type(accessory_tbl)
        ...
        if not id_property is None:
            ...
            _valid_tbl_ids = [tbl.properties.id for tbl in item.tables]
            if not id_property in _valid_tbl_ids:
                raise (
                    f"The table_id {id_property} is not valid id of tables in Feature Service."
                )
            tbl_url = [
                tbl.url for tbl in item.tables if tbl.properties.id == id_property
            ][0]
            return Service(url_or_item=tbl_url, server=item._gis)
    ...
    * by initializing an object using the feature service url
    ...
    def _populate_layers(self):
        ...
        for lyr in self.properties.layers:
            url: str = self.url + "/" + str(lyr.id)
            lyr = Service(
                url_or_item=url, server=self._gis, initialize=False, parent_url=self
            )
            layers.append(lyr)
    ...
    def query_domains(self, layers: Union[tuple, list[int]]):
        ...
        .. note::
            This operation is only available for ArcObjects and hosted feature services in ArcGIS Enterprise, and is
            not supported in ArcGIS Online. If the service supports this operation, then the `supportsQueryDomains`
            property in the service properties is True. If this value is False or not present, then the service
