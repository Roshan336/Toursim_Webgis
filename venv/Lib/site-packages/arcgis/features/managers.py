    ...
    **Inputs**              **Description**
    ...
    def count(
        ...
        **Parameters**          **Description**
        ...
        if data.get("attachmentGroups") is not None:
            count_values = [
    ...
    def update(
        ...
        **Parameter**        **Description**
        ...
        ---------------     --------------------------------------------------------------------
        file_path           Optional string for Arcgis Online, **required** for ArcGIS
                            Enterprise. The path of the file to be uploaded as the feature
                            attachment.
        ---------------     --------------------------------------------------------------------
        return_moment       Optional boolean. Specify whether the response will report the time
                            attachments were deleted. If *True*, the server will report the time
                            in the response's `editMoment` key. The default value is *False*.
        ---------------     --------------------------------------------------------------------
        keywords            Optional string. Keywords for each attachment.
        ===============     ====================================================================
...
class FeatureLayerCollectionManager(_GISResource):
    ...
    def swap_view(
        ...
        new_source: (
            ...
            | features.Table
            | list[features.FeatureLayer | features.Table]
            | Item
        ),
        future: bool = False,
        ...
        **Parameter**           **Description**
        ------------------     --------------------------------------------------------------------
        index                  Required int or list[int]. The index(es) of the layer on the view to replace.
        ------------------     --------------------------------------------------------------------
        new_source             Required FeatureLayer(s) or Table(s). The layer to replace the existing
                               source with. Each layer/table will be matched to the index passed based on position.
                               If an Item is passed, the layers and tables will be extracted as the new source.
        ------------------     --------------------------------------------------------------------
        ...
        """
        # Ensure it is a view layer
        if not self.properties.isView:
            raise ValueError("The feature service is not a view.")

        if isinstance(new_source, Item):
            new_source = [*(new_source.layers or []), *(new_source.tables or [])]

        # Make into lists if not already for processing
        if isinstance(index, int):
            index = [index]
        if not isinstance(new_source, list):
            new_source = [new_source]

        # Ensure same number of indexes and new sources
        if len(index) != len(new_source):
            raise ValueError(
                "The number of indexes must match the number of new sources provided."
            )

        return self._swap_view(
            view=self._fs, indexes=index, new_sources=new_source, future=future
        )
    ...
    def _swap_view(
        ...
        indexes: list[int],
        new_sources: list[features.FeatureLayer | features.Table],
        future: bool = False,
    ) -> dict | concurrent.futures.Future:
        """
        Swaps the Data Source Layer with a different parent layer.
        ...
        **Parameter**           **Description**
        ...
        delete_json: dict = {"layers": [], "tables": []}
        add_json: dict = {"layers": [], "tables": []}
        for index, new_source in zip(indexes, new_sources):
            lyr_type = new_source._lyr_dict.get("type", "FeatureLayer")
            # Determine if working with layer or table
            if lyr_type == "FeatureLayer":
                if not (0 <= index < len(view.layers)):
                    raise IndexError(
                        f"Layer index {index} out of range for FeatureLayer replacement."
                    )
                flc_lyr_info = view.layers[index]
            elif lyr_type == "Table":
                if not (0 <= index < len(view.tables)):
                    raise IndexError(
                        f"Table index {index} out of range for Table replacement."
                    )
                flc_lyr_info = view.tables[index]
            else:
                raise TypeError("new_source must be a FeatureLayer or Table.")
            ...
            # Set the URL with token if it exists
            if new_source._con.token:
                props["url"] = new_source.url + f"?token={new_source._con.token}"
            else:
                props["url"] = new_source.url

            # Construct the adminLayerInfo if it exists
            if flc_lyr_info.manager.properties["adminLayerInfo"].get(
                "viewLayerDefinition", {}
            ):
            ...
            # Determine if working with layer or table and prepare the delete and add json
            if lyr_type == "FeatureLayer":
                delete_json["layers"].append({"id": index})
                add_json["layers"].append(props)
            elif lyr_type == "Table":
                delete_json["tables"].append({"id": index})
                add_json["tables"].append(props)

        # Perform the delete and add; add is async if future=True and on ArcGIS Online
        view.manager.delete_from_definition(delete_json)

        try:
            if future and self._gis._is_arcgisonline:
                res = view.manager.add_to_definition(add_json, future=True)
            else:
                ...
                res = view.manager.add_to_definition(add_json, future=False)
        except Exception as e:
            view.manager.add_to_definition(delete_json)  # revert delete
            raise e
        return res
    ...
    def create_view(
        ...
        **Parameter**             **Description**
        ...
        preserve_layer_ids       Optional Boolean. Preserves the layer's `id` on it's definition when `True`.  The default is `True`.
                                 This must be set to True when using non-sequential layer ids.
        --------------------     --------------------------------------------------------------------
