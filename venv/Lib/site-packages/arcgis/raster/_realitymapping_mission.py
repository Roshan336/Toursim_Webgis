...
class Mission:
    ...
    **Parameter**                             **Description**
    ...
    def add_image(
        ...
        **Parameter**               **Description**
        ------------------          --------------------------------------------------------------------
        image_sources               Required. Must be either a single :class:`~arcgis.raster.imagesource.ImageSource`
                                    instance or a list of :class:`~arcgis.raster.imagesource.ImageSource`
                                    instances. 

                                    Each `ImageSource` encapsulates both the input data and its
                                    raster-type configuration (for example, ``productType``,
                                    ``processingTemplate`` and sensor-specific flags). Configure
                                    the `ImageSource` prior to calling ``add_image``. Example:

                                    ``src = ImageSource('UAV/UAS')``
                                    ``src.input_data = '\\\\path\\to\\data\\flight1'``
                                    ``src.raster_type.is_altitude_flight_height = True``

                                    To add multiple inputs, pass a list of `ImageSource` objects ``[src1, src2]``.
        ------------------          --------------------------------------------------------------------
        input_rasters               Deprecated. Use `ImageSource` object instead.
        ...
        ------------------          --------------------------------------------------------------------
        raster_type_name            Deprecated. Use `ImageSource` object to supply raster type name.

                                    Optional string. The name of the raster type to use for adding data to
                                    ...
                                    "QuickBird"
        ------------------          --------------------------------------------------------------------
        raster_type_params          Deprecated. Deprecated. Use `ImageSource` object to supply raster type parameters
    ...
    def get_processing_states(
        ...
        **Parameter**           **Description**
        ...
        return gis._tools.realitymapping.get_processing_states(
            image_collection=image_collection, future=future, context=context, **kwargs
        )
    ...
    def match_control_points(
        ...
        **Parameter**           **Description**
        ...
        op = gis._tools.realitymapping.match_control_points(
            image_collection=image_collection,
    ...
    def query_exif_info(
        ...
        """
        Reads the Exif header metadata from single or multiple images in a local
        path or shared data store. The Exif metadata is usually stored in drone
        image files. Some common Exif metadata information are GPS locations,
        camera model, focal length, and more.

        ==================      ====================================================================
        **Parameter**           **Description**
        ------------------      --------------------------------------------------------------------
        input_images            Required String/list of Strings.  The input images can be a:
                                ...
                                * list of folder paths
                                  ...
                                  .. note::
                                      The image file paths can also be a Server data store path.

                                .. code-block:: python


                                    >>> from arcgis.gis import GIS
                                    >>> rgis = GIS(profile="your_realitymapping_profile")
                                    ...

                                    # Formatting Usage example 1:

                                    # Path to image file:
                                    >>> res = mission_obj.query_exif_info(
                                                input_images="\\server_name\drone\imagefolder\image_file.jpg"
                                                gis=rgis
                                    )

                                    # Formatting Usage example 2:

                                    # Path to images in folder datastore:
                                    >>> res = mission_obj.query_exif_info(
                                                input_images=[
                                                    "/fileShares/drones/SampleEXIF/DJI_0002.JPG",
                                                    "/fileShares/drones/SampleEXIF/YUN_0040.jpg"
                                                ],
                                                gis=rgis
                                        )
                                    # Path to list of datastore folders:
                                    >>> res = mission_obj.query_exif_info(
                                                input_images=[
                                                    "/cloudStores/S3DataStore/yvwd13",
                                                    "/cloudStores/S3DataStore/BogotaFarm"
                                                ],
                                                gis=rgis
                                        )
        ------------------      --------------------------------------------------------------------
