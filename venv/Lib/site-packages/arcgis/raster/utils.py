...
def read_camera_gps_info(
    file_path: str,
    gps_fields: Optional[List[str]] = None,
    camera_fields: Optional[List[str]] = None,
    srs_field: Optional[str] = None,
) -> Dict[str, Any]:
    """

    Reads gps, camera and spatial reference info from a CSV.

    ==================     ====================================================================
    **Parameter**          **Description**
    ------------------     --------------------------------------------------------------------
    file_path              Path to the gps or camera file to be read.
    ------------------     --------------------------------------------------------------------
    gps_fields             Ordered list of column names expected in the input CSV.
                           The list should be provided in the following order::
                               ["image", "latitude", "longitude", "altitude", "omega", "phi", "kappa"]

                           These values represent the headers that the function will look for
                           in the provided CSV file. The order of the list is important and
                           determines how each column is interpreted.

                           - image      : Image name or path (**required**)
                           - latitude   : Latitude of the image center (decimal degrees) (**required**)
                           - longitude  : Longitude of the image center (decimal degrees) (**required**)
                           - altitude   : Altitude of the sensor (**required**)
                           - omega      : Rotational angle of the camera's X-axis in degrees
                           - phi        : Rotational angle of the camera's Y-axis in degrees
                           - kappa      : Rotational angle of the camera's Z-axis in degrees

                           If the list is missing any required field or the order is incorrect,
                           the function may fail to interpret the CSV.
    ------------------     --------------------------------------------------------------------
    camera_fields          Ordered list of column names expected in the input CSV.
                           The list should be provided in the following order::

                               ["make", "model", "columns", "rows", "pixel_size", "focal_length"]

                           These values represent the headers that the function will look for
                           in the provided CSV file. The order of the list is important and
                           determines how each column is interpreted.

                           - make            : Camera manufacturer (**required**)
                           - model           : Camera model (**required**)
                           - columns         : Number of image columns / width in pixels (**required**)
                           - rows            : Number of image rows / height in pixels (**required**)
                           - pixel_size      : Sensor pixel size. (**required**)
                           - focal_length    : Camera focal length, measured in microns. (**required**)

                           The function will look for these values as headers in the provided CSV.
                           Missing required fields or an incorrect order will prevent proper
                           interpretation of the CSV.
    ------------------     --------------------------------------------------------------------
    srs_field              String, field in the file containing the spatial reference of the images.
    ==================     ====================================================================

    :return: Dictionary containing spatial reference, camera, gps information.
    ...
    cam_records = {}
    srs_records = []
    cam_fields = ["make", "model", "columns", "rows", "pixel_size", "focal_length"]
    srs_index = None
    with open(file_path, newline="", encoding="utf-8-sig") as f:
        ...
        headers_list = []
        for h in headers:
            headers_list.append(h.strip().lower())

        gps_indices = []
        camera_indices = []
        if gps_fields:
            for gps_col in gps_fields:
                if gps_col.lower() not in headers_list:
                    raise ValueError(f"Unable to find headers in csv: {gps_col}")
                else:
                    gps_indices.append(headers_list.index(gps_col.lower()))

        if camera_fields:
            for camera_col in camera_fields:
                if camera_col.lower() not in headers_list:
                    raise ValueError(f"Unable to find headers in csv: {camera_col}")
                else:
                    camera_indices.append(headers_list.index(camera_col.lower()))

        if srs_field:
            if srs_field.lower() in headers_list:
                srs_index = headers_list.index(srs_field.lower())
            else:
                raise ValueError(f"Unable to find headers in csv: {srs_field}")
            ...
            if camera_indices:
                for i in camera_indices:
                    try:
                        ...
                        val = row[i]
                    cam_record[cam_fields[cam_fields_iter]] = val
                    cam_id = cam_id + str(val)
                    cam_fields_iter += 1
                if cam_id not in cam_records:
                    cam_records[cam_id] = cam_record

            if srs_index:
                parsed_srs = _util._parse_srs(row[srs_index])
                if parsed_srs not in srs_records:
                    srs_records.append(parsed_srs)

        if len(cam_records) == 1:
            cam_records = list(cam_records.values())[0]

        if len(srs_records) == 1:
            srs_records = srs_records[0]

        output_list = [v for v in (gps_records, cam_records, srs_records) if v]
        function_output = namedtuple(
            "FunctionOutput", ["gps_info", "camera_info", "srs_info"]
        )

        if len(output_list) > 1:
            return function_output(gps_records, cam_records, srs_records)
