    ...
    **Parameter**                         **Description**
    ...
    raster_type_name                     Optional string. The name of the raster type to use for adding data to
                                         ...
                                         Choice list:
                                             ...
                                             | "SPOT 7", "Superview-1", "Tiled Imagery Layer", "UAV/UAS", "WorldView-1",
                                             | "WorldView-2", "WorldView-3", "WorldView-4", "ZY3-SASMAC", "ZY3-CRESDA"
                                             | ]
...
def segment(
    ...
    output_name: Optional[str] = None,
    context: Optional[dict[str, Any]] = None,
    maximum_segment_size_in_pixels: float = -1,
    *,
    gis: Optional[GIS] = None,
    future: bool = False,
    estimate: Optional[bool] = False,
    ...
    **Parameter**                         **Description**
    ...
    context                              context contains additional settings that affect task execution.
                                         ...
                                         - Resampling Method (resamplingMethod): The output raster will be
                                           ...
                                           Example:
                                                ...
                                                {'resamplingMethod': "Nearest"}
    --------------------------------     --------------------------------------------------------------------
    maximum_segment_size_in_pixels       Optional float. Default is -1.
                                         The maximum size of a segment. Segments that are larger than the specified size will be divided.
                                         Use this parameter to prevent artifacts in the output raster resulting from large segments.
                                         Units are in pixels.

                                         Parameter available in ArcGIS Image Server 12.0 and higher.
    --------------------------------     --------------------------------------------------------------------
    gis                                  Keyword only parameter. Optional :class:`~arcgis.gis.GIS` object. If not specified, the currently active connection
                                         is used.
    --------------------------------     --------------------------------------------------------------------
    ...
    return gis._tools.rasteranalysis.segment(
        ...
        context=context,
        maximum_segment_size_in_pixels=maximum_segment_size_in_pixels,
        future=future,
        estimate=estimate,
        **kwargs,
    )
    ...
    **Parameter**                         **Description**
    ...
    raster_type_name                     Required string. The name of the raster type to use for adding data to \
                                         ...
                                         Choice list:
                                             ...
                                             | "GRIB", "GeoEye-1", "HDF", "IKONOS", "ISAT", "Jilin-1", "KOMPSAT-2", "KOMPSAT-3", 
                                             | "Landsat 1-5 MSS", "Landsat 4-5 TM", "Landsat 7 ETM+", "Landsat 8", "Landsat 9",
                                             | "MOD", "Match-AT", "NetCDF", "PAR", "PlanetScope", "Pleiades NEO", "Pleiades-1",
                                             | "QuickBird", "RapidEye", "Raster Dataset", "SPOT 5", "SPOT 6", "SPOT 7", "Sentinel-2",
                                             | "SkySat", "Superview-1", "UAV/UAS", "WorldView-1", "WorldView-2", "WorldView-3",
                                             ...
                                             - Parallel Processing Factor (parallelProcessingFactor): controls
                                               Raster Processing (CPU) service instances.
                                                ...
                                                processing instances:
                                                    ...
                                                    {"parallelProcessingFactor": "60%"}
    ------------------------------------     --------------------------------------------------------------------
    remove_continuum                         Optional Boolean. Specifies whether the spectra will be normalized from an image or reference data.

                                             - True : The spectra will be normalized from an image or reference data.

                                             - False : The spectra will not be normalized from an image or reference data. This is the default.
...
def manage_multidimensional_raster(
    ...
    truncation_unit: str = "MONTH",
    start_end_keyword: str = "START",
    dimension_definition: str = "BY_VALUES",
    min_value: str = None,
    max_value: str = None,
    *,
    gis: Optional[GIS] = None,
    ...
    **Parameter**                             **Description**
    ...
    manage_mode                              Optional string. Specifies the type of modification that will be performed
                                             ...
                                             - TRUNCATE_DIMENSION_VALUES - Modify the dimension values by truncating date values, or by selecting the start and end values when the dimension values are ranges.

                                             - REMOVE_SLICES - Remove a slice, or a range of slices from the multidimensional raster.
    ------------------------------------     --------------------------------------------------------------------
    ...
    dimension_name                           Optional string. The name of the dimension to be added to the dataset.
                                             This is required if manage_mode is set to ADD_DIMENSION or TRUNCATE_DIMENSION_VALUES or REMOVE_SLICES.
    ------------------------------------     --------------------------------------------------------------------
    dimension_value                          Optional string. The value of the dimension to be added.
                                             This is required if manage_mode is set to ADD_DIMENSION or REMOVE_SLICES when dimension_definition parameter is set to "BY_VALUES".
    ------------------------------------     --------------------------------------------------------------------
    ...
    update_transpose                         Optional boolean. Specifies whether the transpose will be rebuilt for
                                             ...
                                             Parameter available in ArcGIS Image Server 11.5 and higher.
    ------------------------------------     --------------------------------------------------------------------
    truncation_unit                          Optional string. Specifies the date and time truncation unit that will be used.
                                             This parameter is required when the manage_mode parameter is set to
                                             TRUNCATE_DIMENSION_VALUES and the dimension_name parameter is set to STDTIME.
                                             The accepted values for this parameter are listed below:

                                             - SECOND - The specified time unit of truncation will be second.
                                             - MINUTE - The specified time unit of truncation will be minute.
                                             - HOUR - The specified time unit of truncation will be hour.
                                             - DAY - The specified time unit of truncation will be day.
                                             - MONTH - The specified time unit of truncation will be month.
                                             - YEAR - The specified time unit of truncation will be year.

                                             Example:

                                                MONTH

                                             Parameter available in ArcGIS RasterAnalysis Server 12.0 and higher.
    ------------------------------------     --------------------------------------------------------------------
    start_end_keyword                        Optional string. Specifies how the dimension value will be truncated.
                                             This parameter is honoured when the manage_mode parameter is set to TRUNCATE_DIMENSION_VALUES.
                                             The accepted values for this parameter are listed below:

                                             - START - The dimension value will be truncated on the start value.
                                             - END - The dimension value will be truncated on the end value.
                                             - BOTH - The dimension value will be truncated on both the start and end values.
                                             ...
                                             Parameter available in ArcGIS RasterAnalysis Server 12.0 and higher.
    ------------------------------------     --------------------------------------------------------------------
    dimension_definition                     Optional string. Specifies how the dimension will be sliced.

                                             - BY_VALUES - When set as BY_VALUES, the dimension will be sliced using a list of dimension values.  This is the default.
                                             - BY_RANGES - When set as BY_RANGES the dimension will be sliced using a range.
                                             ...
                                             Parameter available in ArcGIS RasterAnalysis Server 12.0 and higher.
    ------------------------------------     --------------------------------------------------------------------
    min_value                                Optional string. The minimum value of the range of slices that will be removed.
                                             This parameter is honoured when the dimension_definition parameter is set to BY_RANGES.
                                             ...
                                             Parameter available in ArcGIS RasterAnalysis Server 12.0 and higher.
    ------------------------------------     --------------------------------------------------------------------
    max_value                                Optional string. The maximum value of the range of slices that will be removed.
                                             This parameter is honoured when the dimension_definition parameter is set to BY_RANGES.
    ...
    return gis._tools.rasteranalysis.manage_multidimensional_raster(
        ...
        truncation_unit=truncation_unit,
        start_end_keyword=start_end_keyword,
        dimension_definition=dimension_definition,
        min_value=min_value,
        max_value=max_value,
        future=future,
...
def calculate_image_service_field(
    image_collection: Union[Item, str],
    field: str,
    ...
    **kwargs,
    ...
    """
    Function calculates values for a field in an image service using a specified expression. 
    This service tool can be used to derive AquisitionDate, SensorType, BandName and
    others information from the raster name. We can also use this tool to add information
    to the image collection like download location of publicly hosted data.

    .. note::
        This function is only available for ArcGIS Enterprise 12.0 and higher.

    .. note::
        Requirement to execute this method:
            - The input should be a Hosted image collection
            - The field to be calculated should already exist
            - Confirm the field is editable
            - Confirm that the datatype of the field and calculated values matches.
    ...
    **Parameter**                             **Description**
    ...
    image_collection                         Required. The input image collection.The image_collection can be a
                                             ...
                                             The image_collection must exist.
    ------------------------------------     --------------------------------------------------------------------
    field                                    Required string. The field that will be updated with the new calculation.

                                             Syntax: A String (field name) that needs to be calculated. 
                                            
                                             Example: 
                                                "AquisitionDate", "SensorType", "CloudCover", "URL"
    ------------------------------------     --------------------------------------------------------------------
    expression                               Required dictionary. A simple calculation expression to create a value
                                             that will populate the selected rows. 

                                             Syntax: A dictionary representing the expression to be applied.
                                            
                                             Example: 
                                                | {"expression": "CURRENT_TIMESTAMP()"}
                                                | {"expression": "DateOnly()"}
    ------------------------------------     --------------------------------------------------------------------
    expression_type                          Required String. Specifies the type of expression that will be used. 
                                             ...
                                             | To learn more about Arcade expressions, see `ArcGIS Arcade <https://developers.arcgis.com/arcade/guide/>`__ in the Developer help.
                                             | To learn more about SQL expressions, see `Calculate field values <https://doc.arcgis.com/en/arcgis-online/manage-data/calculate-fields.htm>`__.
    ------------------------------------     --------------------------------------------------------------------
    context                                  context contains additional settings that affect task execution.
                                             
                                             Set mosaic dataset property tool

                                             This function has the following settings:

                                             - allowedMosaicMethods — Specifies the rules for displaying \
                                               overlapping imagery.

                                             - defaultMosaicMethods — Specifies the mosaic method that will be \
                                               used for views. The default mosaic method must be set in the list \
                                               of values used for the Allowed Mosaic Methods parameter or be set \
                                               in the mosaic dataset's Allowed Mosaic Methods property.

                                             - orderField — The field that will be used when ordering rasters \
                                               using the By attribute value of the Default Mosaic Methods parameter. \
                                               The list of available fields are defined as those in the attribute \
                                               table that are of type metadata and are integer. If the field is a \
                                               numeric or date field, the Order Base parameter must be set. This \
                                               parameter is not needed if the By attribute value is not part of the \
                                               Allowed Mosaic Methods list.

                                             - orderBase — Sorts the rasters based on their difference from this \
                                               value in the field selected in the Order Field parameter. This \
                                               parameter is required only if the By attribute value is specified \
                                               for the Allowed Mosaic Methods parameter.

                                             - sortingOrder — Specifies whether the rasters will be sorted in an \
                                               ascending or a descending order. This parameter is required only if \
                                               the By attribute value is specified for the Allowed Mosaic Methods \
                                               parameter.

                                             - allowedTransmissionField — The fields in the attribute table that \
                                               clients can view.

                                             - useTime — Specifies whether the mosaic dataset will be time aware. \
                                               If time is activated, the start and end fields and the time format \
                                               must be specified.

                                             - startTimeField — The field in the attribute table that shows the \
                                               start time.

                                             - endTimeField — The field in the attribute table that shows the end \
                                               time.

                                             - timeFormat — Specifies the time format that will be used for the \
                                               mosaic dataset for parameters such as Start Time Field and End Time \
                                               Field.

                                             - timeInterval — The duration of each time step interval. The time \
                                               step interval defines the granularity of the temporal data. The \
                                               unit of time is specified in the Time Interval Units parameter.

                                             - timeIntervalUnits — Specifies the measurement unit that will be \
                                               used for the time interval.

                                             Example:

                                                | {
                                                |   "allowedTransmissionField": [
                                                |     "Name",
                                                |     "MinPS",
                                                |     "MaxPS",
                                                |     "LowPS",
    ...
    .. code-block:: python
        ...
        image_col = gis.content.search("my_image_collection")[0].layers[0]

        calc_field_op = calculate_image_service_field(
                        image_collection=icol,
                        field="AquisitionDate",
                        expression={"expression": "CURRENT_TIMESTAMP()"},
                        expression_type="SQL",
                        context={"allowedTransmissionField": ["Name", "AquisitionDate", "SensorType"]},
                        gis=gis)
    """
    gis = _arcgis.env.active_gis if gis is None else gis
    return gis._tools.rasteranalysis.calculate_image_service_field(
        image_collection=image_collection,
        field=field,
...
def build_multidimensional_info(
    image_collection: Union[Item, str],
    variable_field: Optional[str] = None,
    dimension_fields: Optional[list[dict[str, Any]]] = None,
    variable_info: Optional[list[dict[str, Any]]] = None,
    delete_multidimensional_info: bool = False,
    *,
    gis: Optional["GIS"] = None,
    future: Optional[bool] = False,
    estimate: Optional[bool] = False,
    ...
    """
    Function generates multidimensional metadata in the image collection (mosaic dataset), including information regarding variables and dimensions.
    The Build Multidimensional Info tool creates multidimensional information for a collection that contains images collected at multiple times,
    depths, or heights.
    ...
    .. note::
        This tool creates the following two text fields in the image collection:
            - Variable - Stores the names of the variables. For example, this field will store the string Temperature \
                        for all the images with temperature data, and Salinity for all the images containing salinity data.
            - Dimensions - Stores the names of the dimensions. For example, if your temperature data has a corresponding \
                        Date dimension field representing the day it was captured, and your salinity data has a Depth \
                        dimension field representing the depth at which it was measured, the Dimensions field for that \
                        variable would be Date, Depth. The two fields, Date and Depth, must exist and be populated in the mosaic dataset.
    ...
    **Parameter**                             **Description**
    ...
    image_collection                         Required. The input image collection.The image_collection can be a
                                             ...
                                             The image_collection must exist.
    ------------------------------------     --------------------------------------------------------------------
    variable_field                           Optional String. The field in the image collection that stores the variable names and is
                                             used to populate a new field named Variable. If all rasters in the image
                                             collection represent the same variable, type the name of the variable, for example, Temperature.

                                             If the Variable field does not exist, an existing field or string value must be specified.
                                             If the Variable field exists, the tool will update the multidimensional information only.

                                             Example:  "ProductName"
    ------------------------------------     --------------------------------------------------------------------
    dimension_fields                         Optional List of dictionaries. The fields in the image collection that store the dimension
                                             information and are used to populate a new field named Dimensions.
                                             ...
                                             Example:
                                                ...
                                                |   "units": "<units of the field>"}]
    ------------------------------------     --------------------------------------------------------------------
    variable_info                            Optional List of dictionaries. Specify additional information about the Variable field.
                                             ...
                                             Example:
                                                ...
                                                |   "units": "<units of the variable>"}]
    ------------------------------------     --------------------------------------------------------------------
    delete_multidimensional_info             Optional boolean. Specifies whether existing multidimensional information will be deleted.

                                                - True - If multidimensional information exists in the image collection, it will be deleted.
                                                - False - If multidimensional information exists in the image collection, it will not be deleted. This is the default.
    ------------------------------------     --------------------------------------------------------------------
    gis                                      Optional :class:`~arcgis.gis.GIS` object. If not specified, the currently active connection
                                             is used.
    ------------------------------------     --------------------------------------------------------------------
    ...
    .. code-block:: python
        ...
        image_col = gis.content.search("my_image_collection")[0]

        result = build_multidimensional_info(
                    image_collection=image_col,
                    variable_field="ProductName",
                    dimension_fields=[{"dimension":"Date","description":"Date","units":"Days"}],
                    variable_info=[{"variable":"test","description":"","units":""}],
                    delete_multidimensional_info=False
                )
    """
    gis = _arcgis.env.active_gis if gis is None else gis
    return gis._tools.rasteranalysis.build_multidimensional_info(
        image_collection=image_collection,
        variable_field=variable_field,
        dimension_fields=dimension_fields,
        variable_info=variable_info,
        delete_multidimensional_info=delete_multidimensional_info,
        future=future,
        estimate=estimate,
    )
...
def detect_image_anomalies(
    ...
    background_region: Optional[FeatureLayer] = None,
    ...
    **kwargs,
    ...
    """
    Processes a multiband or hyperspectral image and creates an anomaly score raster.
    An anomaly score raster is a single band raster, with values between 0 and 1.
    Function available in ArcGIS Image Server 12.1 and higher.
    ...
    **Parameter**                             **Description**
    ...
    input_raster                             Required :class:`~arcgis.raster.ImageryLayer` object. The input raster.
                                             Portal Item can be passed.
    ------------------------------------     --------------------------------------------------------------------
    input_spectral_profile                   Required String. The anomaly detection method to use.

                                             Choice list:

                                             - RXD: The RXD method will be used to extract pixels that are significantly
                                               different from the background pixel values. This is the default.

                                             - UTD: The UTD method will be used to extract background pixels from the input image.

                                             - KMEANS: The KMEANS method will be used to extract pixels that significantly deviate from the
                                               established clusters in the data using K-means clustering algorithms.
    ------------------------------------     --------------------------------------------------------------------
    num_clusters                             Optional Integer. Number of clusters to use for KMEANS method.
    ------------------------------------     --------------------------------------------------------------------
    background_region                        Optional feature layer. A polygon feature layer that will define the region to
                                             be used to calculate background statistics when the method parameter is set to RXD or UTD.
    ------------------------------------     --------------------------------------------------------------------
    recompute_stats                          Optional Boolean. Specifies whether statistics will be recomputed for the
                                             output score raster when the method parameter is set to RXD or UTD.
                                             The RXD and UTD options require accurate statistics in which the skip factor must be 1.
    ...
    context                                  Context contains additional settings that affect task execution.
                                                ...
                                                Example:
                                                    {'mask': {'url': '<image_service_url>'}}
    ------------------------------------     --------------------------------------------------------------------
    gis                                      Optional :class:`~arcgis.gis.GIS` object. If not specified, the currently active connection is used.
    ------------------------------------     --------------------------------------------------------------------
    ...
    .. code-block:: python
        ...
        # Usage Example 1: Detect anomalies using RXD method
        anomaly_output = detect_image_anomalies(
            input_raster=input_layer,
            method="RXD",
            output_name="anomaly_detection",
            gis=gis
        )

        # Usage Example 2: Detect anomalies using KMEANS method with 5 clusters
        anomaly_output = detect_image_anomalies(
            input_raster=input_layer,
            method="KMEANS",
            num_clusters=5,
            output_name="anomaly_kmeans",
            gis=gis
        )
    """

    gis = _arcgis.env.active_gis if gis is None else gis
    return gis._tools.rasteranalysis.detect_image_anomalies(
        input_raster=input_raster,
        output_name=output_name,
        method=method,
...
def resample_library_spectra(
    input_library_spectra,
    output_name: str = None,
    target_sensor: str = "CUSTOM_TARGET_IMAGE_OR_SPECTRA",
    custom_target_image_or_spectra: str = None,
    resampling_method: str = "BAND_AVERAGING",
    context: Optional[dict[str, Any]] = None,
    *,
    gis: Optional[GIS] = None,
    future: bool = False,
    ...
    **kwargs,
    ...
    """
    Resamples library spectra to match the spectral characteristics of a target sensor or custom image/spectra.
    Function available in ArcGIS Image Server 12.1 and higher.

    ====================================     ====================================================================
    **Parameter**                             **Description**
    ------------------------------------     --------------------------------------------------------------------
    input_library_spectra                    Required string. The input Esri spectral library item or path to the shared local spectral library file.
    ------------------------------------     --------------------------------------------------------------------
    output_name                              Required string. The name of the output Esri spectral library item.
    ------------------------------------     --------------------------------------------------------------------
    target_sensor                            Required string. Specifies the target spectra, or a custom spectra library file or image, that contains the target spectra.
                                             The resampling will be based on the selected sensor or custom input.
                                             ...
                                             - CUSTOM_TARGET_IMAGE_OR_SPECTRA — A custom spectral library file or image that contains the
                                               target spectra will be used. This is the default.
    ------------------------------------     --------------------------------------------------------------------
    custom_target_image_or_spectra           Required Portal Item or Imagery Layer object when target_sensor parameter value is CUSTOM_TARGET_IMAGE_OR_SPECTRA.
                                             A custom spectral library file item or imagery layer with wavelength information
                                             that contains the target spectra.
    ------------------------------------     --------------------------------------------------------------------
    resampling_method                        Optional string. Specifies the resampling method that will be used for the output library spectra.

                                             - BAND_AVERAGING—A simple average of the input spectra based on all band values that fall
                                               within each target band’s wavelength range will be used. This is the default.
                                             - GAUSSIAN_CONVOLUTION—A Gaussian model will be used to represent the spectral response
                                               function of the sensor, and the input band values will be weighted for each wavelength range.
    ------------------------------------     --------------------------------------------------------------------
    context                                  Optional dict. Additional settings that affect task execution.
    ------------------------------------     --------------------------------------------------------------------
    gis                                      Optional :class:`~arcgis.gis.GIS` object. The GIS on which this tool runs.
    ...
    :return: output_library_spectra : str

    .. code-block:: python

        # Usage Example: Resample library spectra to match Sentinel-2A sensor
        resampled_spectra = resample_library_spectra(
            input_library_spectra=<ecd_item>,
            output_name="resampled_spectra",
            target_sensor="SENTINEL-2A",
            resampling_method="BAND_AVERAGING",
            gis=gis,
    ...
    gis = _arcgis.env.active_gis if gis is None else gis
    return gis._tools.rasteranalysis.resample_library_spectra(
        input_library_spectra=input_library_spectra,
        output_name=output_name,
...
def detect_target_using_spectra(
    input_raster,
    input_spectra,
    output_name: str = None,
    method: str = "SAM",
    ...
    **kwargs,
    ...
    """
    Detects the presence of a target in an input raster using a specified spectral matching method.
    Function available in ArcGIS Image Server 12.1 and higher.

    ====================================     ====================================================================
    **Parameter**                             **Description**
    ------------------------------------     --------------------------------------------------------------------
    input_raster                             Required. The input raster dataset or imagery layer to analyze.
    ------------------------------------     --------------------------------------------------------------------
    input_spectra                            Required. A spectral signature file. The input can be a spectral library file portal item
                                             that contains spectral signatures, or a feature layer created from the spectral collection workflow.
    ------------------------------------     --------------------------------------------------------------------
    output_name                              Required string. The name of the output score raster.
    ------------------------------------     --------------------------------------------------------------------
    method                                   Optional. Specifies the method that will be used to compute the matching scores from the input image and the spectral signature file.
                                             All scores are normalized from 0 to 1, in which a higher score is a better match.

                                             Syntax: A string of one of the following keywords.

                                             - SAM — The Spectral Angle Mapper method will be used to compute the matching scores. (default)
                                             - SID — The Spectral Information Divergence method will be used to compute the matching scores.
                                             ...
                                             Example:
                                                method = "CEM"
    ------------------------------------     --------------------------------------------------------------------
    remove_continuum                         Optional. A boolean value to specify whether the spectra will be normalized from an image or reference data.

                                             When set as True, the spectra will be normalized from an image or reference data.
                                             When False, the spectra will not be normalized from an image or reference data. The default value is False.
    ------------------------------------     --------------------------------------------------------------------
    context                                  Optional dict. Additional settings that affect task execution.
    ------------------------------------     --------------------------------------------------------------------
    gis                                      Optional :class:`~arcgis.gis.GIS` object. The GIS on which this tool runs. If not specified, the active GIS is used.
    ------------------------------------     --------------------------------------------------------------------
    future                                   Optional boolean. If True, the result will be a GPJob object and results will be returned asynchronously.
    ------------------------------------     --------------------------------------------------------------------
    estimate                                 Optional boolean. If True, the number of credits needed to run the operation will be returned as a float.
    ...
    :return: output_score_raster : Imagery Layer Item

    .. code-block:: python

        # Usage Example: Detect a target using the Spectral Angle Mapper (SAM) method
        score_raster = detect_target_using_spectra(
            input_raster=input_layer,
            input_spectra=<esl portal item>,
            output_name="target_detection_score",
            method="SAM",
            gis=gis,
            folder="target_detection_results"
        )
    """

    gis = _arcgis.env.active_gis if gis is None else gis
    return gis._tools.rasteranalysis.detect_target_using_spectra(
        input_raster=input_raster,
