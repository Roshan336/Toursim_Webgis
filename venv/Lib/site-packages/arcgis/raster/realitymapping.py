...
from arcgis.raster.imagesource import ImageSource
...
class ScenarioType(Enum):
    ...
    | SATELLITE      | "Satellite"      |
    +----------------+------------------+
    | AERIAL_NADIR   | "AerialNadir"    |
    +----------------+------------------+
    | AERIAL_OBLIQUE | "AerialOblique"  |
    +----------------+------------------+
    """
...
def _create_project(
    ...
    **Parameter**          **Description**
    ...
    sensor_scenario_map = {
        ...
        SensorType.SATELLITE.value: ScenarioType.SATELLITE.value,
        SensorType.AERIAL_SCANNED.value: "",
        SensorType.AERIAL_DIGITAL.value: ScenarioType.AERIAL_NADIR.value,
    }

    if sensor_type == SensorType.AERIAL_DIGITAL.value:
        scenario_type = (
            scenario_type.value
        ...
        if scenario_type not in [
            ...
            raise RuntimeError(
                "Invalid scenario type for Aerial Digital sensor. Supported values are 'AerialNadir', 'AerialOblique'. Please refer to arcgis.raster.realitymapping.ScenarioType for more information."
            )
    else:
        scenario_type = sensor_scenario_map[sensor_type]
    ...
    if tags:
        if isinstance(tags, str):
            tags = [tags]
        project_definition["tags"] = tags
    if summary:
        project_definition["summary"] = summary
    ...
    if user_list:
        project_definition["users"] = user_list
...
def compute_spatial_reference_factory_code(latitude: float, longitude: float):
    """
    Computes spatial reference factory code. This value may be used as out_sr
    value in create image collection function
...
class Project:
    ...
    **Parameter**                            **Description**
    ------------------------------------     --------------------------------------------------------------------
    project                                  Required string. The name of the project item to be created.
                                             To open an existing project, use the :meth:`~arcgis.raster.realitymapping.Project.from_item` method.
    ...
    scenario_type                            Optional ScenarioType or string. The type of scenario for the imagery.
                                             ...
                                             supported values.
    ------------------------------------     --------------------------------------------------------------------
    tags                                     Optional list of strings. Tags to be associated with the
                                             Reality Mapping Project portal item.
    ------------------------------------     --------------------------------------------------------------------
    summary                                  Optional string. A summary for the portal item.
    ------------------------------------     --------------------------------------------------------------------
    ...
    ------------------------------------     --------------------------------------------------------------------
    users                                    Optional list of :class:`~arcgis.gis.User` objects or strings.
                                             If using a list of strings, the string must be the usernames of the users.

                                             If the user possesses the required reality mapping privileges,
                                             they will be added as a group manager, else they will be added as a member.
    ------------------------------------     --------------------------------------------------------------------
    ...
    def __init__(
        ...
        scenario_type: Optional[Union[Enum, str]] = None,
        tags: Optional[list[str]] = None,
        summary: Optional[str] = None,
        ...
        **kwargs,
        ...
        elif not isinstance(project, Item):
            raise ValueError(
                "project must be a string or an instance of arcgis.gis.Item of type Reality Mapping Project."
            )
    ...
    @classmethod
    def from_item(cls, item: Item, gis: Optional[GIS] = None):
        """
        Creates a Project object from an existing Reality Mapping Project portal Item.

        ==================      ====================================================================
        **Parameter**           **Description**
        ------------------      --------------------------------------------------------------------
        item                    Required :class:`~arcgis.gis.Item` . The Reality Mapping Project Item.
        ------------------      --------------------------------------------------------------------
        gis                     Optional :class:`~arcgis.gis.GIS` . The GIS connection to use.
                                If not specified, the active GIS is used.
        ==================      ====================================================================
        ...
        .. code-block:: python
            ...
            my_project_item = gis.content.search("project_1", item_type="Reality Mapping Project")[0]
            project = Project.from_item(item=my_project_item, gis=my_gis)

        """
        if not isinstance(item, Item):
            raise ValueError("item must be an instance of arcgis.gis.Item")
        return cls(project=item, gis=gis)
    ...
    def create_mission(
        ...
        **kwargs,
        ...
        images sources, and settings.

        ====================================    ====================================================================
        **Parameter**                           **Description**
        ------------------------------------    --------------------------------------------------------------------
        image_sources                           Required the list of :class:`~arcgis.raster.imagesource.ImageSource` 
                                                instances to be added to the mission being created.

                                                - This parameter replaces the deprecated ``image_list``, ``raster_type_name`` 
                                                and ``raster_type_params`` arguments
        ------------------------------------    --------------------------------------------------------------------
        image_list (Deprecated)                 Deprecated: Use the ``image_sources`` parameter instead.

                                                Required, the list of input images to be added to
        ...
        scenario_type                           Optional ScenarioType or string. The type of scenario for the imagery.
                                                ...
                                                supported values.
        ------------------------------------    --------------------------------------------------------------------
        raster_type_name (Deprecated)           Deprecated: This parameter is deprecated; pass raster type information
                                                via the ``image_sources`` objects (see :class:`~arcgis.raster.imagesource.ImageSource`).
                                                
                                                Optional string. The name of the raster type used.
                                                Refer to :meth:`~arcgis.raster.analytics.create_image_collection` \
                                                for supported raster types.
        ------------------------------------    --------------------------------------------------------------------
        raster_type_params (Deprecated)         Deprecated: This parameter is deprecated; pass raster type parameter information
                                                via the ``image_sources`` objects (see :class:`~arcgis.raster.imagesource.ImageSource`).
        ...
        .. code-block:: python
            ...
            # Example : Create a mission with Drone imagery

            src = ImageSource('UAV/UAS')
            src.input_data = '\\\\path\\to\\data\\flight1'
            src.raster_type.is_altitude_flight_height = True
        ...
        for i, src in enumerate(sources):
            if src.input_data is None:
                raise RuntimeError(f"input_data is None for image_sources[{i}]")

        if any(
            k in kwargs
            for k in ("raster_type_name", "raster_type_params", "image_list")
        ):
            raise RuntimeError(
                "raster_type_name, raster_type_params, and image_list are deprecated. Please use image_sources parameter instead"
            )
        ...
        input_rasters, image_collection, raster_type, context, output_service = (
            _ra._sanitize_inputs(
                ...
                input_rasters=image_sources,
                raster_type_name=image_sources,
                raster_type_params=image_sources,
                out_sr=out_sr,
                context=context,
                folder=self._folder,
                **kwargs,
        ...
        mission_def["settings"] = settings or {}
        # if context is None:
        ...
        mission_def["settings"]["sensorType"] = sensor_type
        mission_def["settings"]["scenarioType"] = scenario_type
