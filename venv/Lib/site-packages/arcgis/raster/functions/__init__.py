...
from .._util import _get_geometry_from_feature_layer
...
def apply(raster: Union[Raster, ImageryLayer], fn_name, **kwargs):
    ...
    **Parameter**                         **Description**
    ...
    template_dict_ra = copy.deepcopy(template_dict)
    if variable_name is not None:
        template_dict_ra["rasterFunctionArguments"][variable_name] = raster_ra
    else:
        template_dict_ra["rasterFunctionArguments"]["Raster"] = raster_ra

    function_chain_ra = {
        "rasterFunction": "Identity",
        "rasterFunctionArguments": {
            "Raster": {
                "renderingRule": template_dict_ra,
...
def linear_spectral_unmixing(
    ...
    **Parameter**                         **Description**
    ...
    if remove_continuum is not None:
        template_dict["rasterFunctionArguments"]["ContinuumRemoval"] = remove_continuum
...
class RFT:
    ...
    def to_json(self, gis: Optional[GIS] = None):
        ...
        **Parameter**          **Description**
        ...
        # create the _GPResource
        gptool = _arcgis.geoprocessing._support._GPResource(url, gis)
        params = {}
        ...
        if gis._is_arcgisonline:
            resp = gptool.session.get(
                job_values["outputRasterFunction"]["url"],
                params={
                    "f": "json",
                },
            )
        ...
        def _raster_function_traversal(
            ...
            if "value" in raster_dict.keys():  # Handling Scalar rasters
                if raster_dict["value"] is None:
                    ...
                    if (
                        not self._is_public_flag
                        or ispublic
                        or raster_dict.get("isPublic", False)
                    ):
                        raster_name = _python_variable_name(raster_dict["name"])
                        raster_dict.update({"name": raster_name})
                        key_value_dict.update({raster_name: raster_dict["value"]})
