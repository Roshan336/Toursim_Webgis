...
class WMTSLayer(BaseOGC):
    ...
    **Parameter**        **Description**
    ...
    # ----------------------------------------------------------------------
    def __init__(
        self, url, version="1.0.0", gis=None, is_google_compatible=False, **kwargs
    ):
    ...
    def _get_operational_layer_config(url: str, properties: dict, idx=None) -> dict:
        ...
        # Get the first tile_matrix not tagged as GoogleMapsCompatible
        if tile_matrix is None:
            tile_matrix_identifier = [
                t["TileMatrixSet"]
                for t in layer["TileMatrixSetLink"]
                if "GoogleMapsCompatible" not in t["TileMatrixSet"]
            ][0]
