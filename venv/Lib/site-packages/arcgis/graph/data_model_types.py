...
class GraphDataModel(BaseModel):
    ...
    domains: list[GraphDomain] = Field(
        default=[], description="The list of domains in the data model."
    )
...
class GraphDomain(BaseModel):
    ...
    name: str = Field(..., description="The given name of the domain.")

    field_type: Optional[esriFieldType] = Field(
        default=None, description="The field type associated with the domain."
    )
    merge_policy_type: Optional[esriMergePolicyType] = Field(
        default="esriMergePolicyType_UNSPECIFIED",
        description="The merge policy type for the domain.",
    )
    split_policy_type: Optional[esriSplitPolicyType] = Field(
        default="esriSplitPolicyType_UNSPECIFIED",
        description="The split policy type for the domain.",
    )


class CodedValue(BaseModel):
    name: str = Field(..., description="The name of the coded value.")
    code: Any = Field(..., description="The code of the coded value.")


class CodedValueDomain(GraphDomain):
    """
    ...
    coded_values: list[CodedValue] = Field(
        default=[], description="The list of CodedValues in the domain."
    )

    @classmethod
    def _from_kg_domain(cls, domain_object: Any) -> Any:
        data = {
            "name": domain_object.name,
            "field_type": domain_object.field_type.name,
            "merge_policy_type": domain_object.merge_policy_type.name,
            "split_policy_type": domain_object.split_policy_type.name,
        }

        coded_values = []
        for cv in domain_object.coded_value_domain.get_coded_values():
            coded_values.append(CodedValue(name=cv.name, code=cv.code))
        data["coded_values"] = coded_values

        return cls(**data)


class RangeDomain(GraphDomain):
    """
    ...
    range: list[Any] = Field(
        default=[], description="The range of values in the domain."
    )
    ...
    def _from_kg_domain(cls, domain_object: Any) -> Any:
        data = {
            "name": domain_object.name,
            "range": domain_object.range_domain.get_range(),
            "field_type": domain_object.field_type.name,
            "merge_policy_type": domain_object.merge_policy_type.name,
            "split_policy_type": domain_object.split_policy_type.name,
        }
...
class GraphDomainMask(BaseModel):
    """
    Allows users to define what should be updated when performing a
    :meth:`~arcgis.graph.KnowledgeGraph.domain_update()`.
    ...
    update_name: bool = Field(
        default=False,
        description="Boolean signifying whether to update the name in the domain.",
    )
    update_domain: bool = Field(
        default=False,
        description="Boolean signifying whether to update the domain.",
    )
    update_field_type: bool = Field(
        default=False,
        description="Boolean signifying whether to update the field type associated with the domain.",
    )
    update_merge_policy: bool = Field(
        default=False,
        description="Boolean signifying whether to update the merge policy type for the domain.",
    )
    update_split_policy: bool = Field(
        default=False,
        description="Boolean signifying whether to update the split policy type for the domain.",
    )


class GraphDomainUpdate(BaseModel):
    """
    ...
    name: str = Field(..., description="The name of the domain to update.")
    mask: GraphDomainMask = Field(
        ...,
        description="The mask indicating the properties in the domain to update.",
    )
    domain: Union[CodedValueDomain, RangeDomain] = Field(
        ...,
        description="The domain properties to update, as determined by the mask.",
    )
