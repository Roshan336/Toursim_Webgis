...
class KnowledgeGraph:
    ...
    **Parameter**           **Description**
    ...
    def query_data_model(self, as_dict: bool = True) -> Union[dict, GraphDataModel]:
        ...
        **Parameter**           **Description**
        ...
        if as_dict:
            ...
            dm = self._datamodel
            pydantic_dm = GraphDataModel.model_validate(dm.to_value_object())
            domains = []
            for kg_domain in dm.domains:
                if kg_domain.type == "codedValue":
                    domain = CodedValueDomain._from_kg_domain(kg_domain)
                elif kg_domain.type == "range":
                    domain = RangeDomain._from_kg_domain(kg_domain)
                domains.append(domain)
            pydantic_dm.domains = domains
            return pydantic_dm
    ...
    def export(self, timeout: int = 600) -> ExportResponse:
        """
        ...
        **Parameter**       **Description**
        ----------------    ---------------------------------------------------------------
        timeout             Optional integer. The maximum time, in seconds, to wait for the
                            export operation to complete. The default is 600 seconds.
        ================    ===============================================================

        :return:
            :class:`~arcgis.graph.response_types.ExportResponse`
        ...
        self._validate_import()
        url = self._url + "/dataTransfer/export"
        params = {
            "f": "pbf",
            "token": self._gis._con.token,
        }
        headers = {"Content-Type": "application/octet-stream"}
        r_enc = _kgparser.ExportServiceDefinitionRequest()
        r_enc.export_data_format = _kgparser.ExportDataFormat.ExportDataFormatGeoParquet
        service_name = self._url.rstrip("/").split("/")[-2]
        r_enc.service_definition_name = service_name

        enc = _kgparser.ExportRequestEncoder()
        enc.set_export_service_definition(r_enc)
        enc.encode()
        enc_result = enc.get_encoding_result()
        error = enc_result.error
        if error.error_code != 0:
        ...
        dec = _kgparser.ExportResponseDecoder()
        dec.decode(response_content)
        if dec.has_error():
            raise Exception(dec.get_response_error().error_message)
        async_status_url = dec.get_async_status_url()
        step = 5
        result_url = None
        while result_url is None and timeout > 0:
            check = self._checkAsyncStatus(async_status_url)
            if check != False:
                result_url = check
            else:
                time.sleep(step)
        ...
        if result_url is None:
            raise Exception("Export operation timed out.")

        result_params = {"token": self._gis._con.token}
        result_response = session.get(
            url=result_url,
            params=result_params,
            headers=headers,
        )
        self._validate_response(result_response)
        result_content = result_response.content
        result_dec = _kgparser.AsyncResultResponseDecoder()
        result_dec.decode(result_content)
        result = result_dec.get_export_service_definition_result()
        e = result.get_error()
        if e.error_code != 0:
            e_response = Error(error_code=e.error_code, error_message=e.error_message)
        else:
            e_response = None
        wc = result.get_warnings_count()
        warning_list = []
        if wc > 0:
            for i in range(wc):
                warning = result.get_warning_at(i)
                warning_list.append(
                    Error(
        ...
        sc = result.get_statistics_count()
        stat_list = []
        if sc > 0:
            for i in range(sc):
                type_name = result.get_statistics_key_at(i)
                stat = result.get_statistics_value_at(i)
                stat_list.append(
                    DataTransferStats(
                        type_name=type_name,
        ...
        return ExportResponse(
            submission_time=result_dec.get_submission_time(),
            completion_time=result_dec.get_completion_time(),
            error=e_response,
            warnings=warning_list,
            service_definition_name=result.get_service_definition_name(),
            export_data_format=result.get_export_data_format().name,
            download_url=result.get_service_definition_download_url(),
            statistics=stat_list,
        )

    def add_domain(
        self, domain: Union[CodedValueDomain, RangeDomain]
    ) -> AddDomainResponse:
        """
        ...
        **Parameter**        **Description**
        ----------------    ---------------------------------------------------------------
        domain              Required :class:`~arcgis.graph.data_model_types.GraphDomain`.
                            The domain to be added to the data model.
        ================    ===============================================================
        ...
        """
        self._validate_import()
        url = self._url + "/dataModel/domains/add"
        params = {
            "f": "pbf",
            "token": self._gis._con.token,
        }
        headers = {"Content-Type": "application/octet-stream"}
        kg_domain = self._construct_domain(domain)
        enc = _kgparser.GraphDomainAddRequestEncoder()
        enc.domain = kg_domain
        enc.encode()
        enc_result = enc.get_encoding_result()
        error = enc_result.error
        if error.error_code != 0:
        ...
        dec = _kgparser.GraphDomainAddResponseDecoder()
        dec.decode(response_content)
        dec_error = dec.get_response_error()
        if dec_error.error_code != 0:
            return AddDomainResponse(
                error=Error(
                    error_code=dec_error.error_code,
                    error_message=dec_error.error_message,
                ),
            )
        else:
            return AddDomainResponse()

    def update_domain(self, domain_update: GraphDomainUpdate) -> UpdateDomainResponse:
        """
        ...
        **Parameter**        **Description**
        ----------------    ---------------------------------------------------------------
        domain_update       Required
                            :class:`~arcgis.graph.data_model_types.GraphDomainUpdate`.
                            Contains the name of the domain, updated domain, and mask
                            dictating which properties get updated.
        ================    ===============================================================
        ...
        """
        self._validate_import()
        url = self._url + "/dataModel/domains/" + domain_update.name + "/update"
        params = {
            "f": "pbf",
            "token": self._gis._con.token,
        }
        headers = {"Content-Type": "application/octet-stream"}
        kg_domain = self._construct_domain(domain_update.domain)
        enc = _kgparser.GraphDomainUpdateRequestEncoder()
        for k, v in domain_update.mask.model_dump(by_alias=True).items():
            setattr(enc, k, v)
        enc.domain = kg_domain
        enc.encode()
        enc_result = enc.get_encoding_result()
        ...
        self._validate_response(response)
        response_content = response.content
        dec = _kgparser.GraphDomainUpdateResponseDecoder()
        dec.decode(response_content)
        dec_error = dec.get_response_error()
        if dec_error.error_code != 0:
    ...
    def delete_domain(self, domain_name: str) -> DeleteDomainResponse:
        """
        ...
        **Parameter**        **Description**
        ----------------    ---------------------------------------------------------------
        domain_name         Required string. The name of the domain to be deleted.
        ================    ===============================================================
        ...
        """
        self._validate_import()
        url = self._url + "/dataModel/domains/" + domain_name + "/delete"
        params = {
            "f": "pbf",
            "token": self._gis._con.token,
        }
        ...
        dec = _kgparser.GraphDomainDeleteResponseDecoder()
        dec.decode(response_content)
        dec_error = dec.get_response_error()
        if dec_error.error_code != 0:
    ...
    def _construct_domain(self, domain: Union[CodedValueDomain, RangeDomain]):
        """
        Helper to construct an _arcgisknowledge.Domain() object from a GraphDomain object
        """

        kg_domain = _kgparser.Domain()
        kg_domain.name = domain.name

        if isinstance(domain, CodedValueDomain):
            kg_domain.type = "coded_value"
            kg_cvd = _kgparser.CodedValueDomain()
            for coded_value in domain.coded_values:
                cv = _kgparser.CodedValue()
                cv.name = coded_value.name
                cv.code = coded_value.code
                kg_cvd.add_coded_value(cv)
            kg_domain.coded_value_domain = kg_cvd

        if isinstance(domain, RangeDomain):
            kg_domain.type = "range"
            kg_rd = _kgparser.RangeDomain()
            for range_value in domain.range:
                kg_rd.add_value(range_value)
            kg_domain.range_domain = kg_rd

        kg_domain.merge_policy_type = getattr(
            _kgparser.MergePolicyType, domain.merge_policy_type
        )
        kg_domain.split_policy_type = getattr(
            _kgparser.SplitPolicyType, domain.split_policy_type
        )
        if domain.field_type:
            kg_domain.field_type = getattr(_kgparser.esriFieldType, domain.field_type)

        return kg_domain

    def _checkAsyncStatus(self, async_status_url: str) -> Union[bool, str]:
        params = {"token": self._gis._con.token}
        session = self._gis._con._session
        headers = {"Content-Type": "application/octet-stream"}

        async_status_response = session.post(
            async_status_url, params=params, headers=headers
        )
        async_content = async_status_response.content

        dec = _kgparser.AsyncStatusResponseDecoder()
        dec.decode(async_content)
        status_type = dec.get_async_status_type()
        if status_type == _kgparser.AsyncStatusType.AsyncStatusCompleted:
            result_url = dec.get_results_url()
            return result_url
        else:
            return False


def _create_new_kg(kg_name, gis, service_properties: dict = None):
    if not service_properties:
        service_properties = {
            "capabilities": "Query",
            "jsonProperties": {"supportsProvenance": False},
        }
    service_properties["name"] = kg_name
    service_properties["type"] = "KnowledgeGraph"
    kg_item = gis.content.create_service(
        name=kg_name, service_type="KnowledgeGraph", create_params=service_properties
    )
    return kg_item.url
