...
def _analysis_job(gptool, task, params):
    ...
    resp = gptool.session.post(submit_url, data=_format_payload(params)).json()
    # print(resp)
...
def _analysis_job_status(gptool, task_url, job_info):
    ...
    if "jobId" in job_info:
        ...
        try:
            ...
            job_response = gptool.session.get(
                job_url, params=_format_payload(params)
            ).json()
            ...
            if "jobStatus" in job_response:
                while not job_response.get("jobStatus") == "esriJobSucceeded":
                    ...
                    if job_response.get("jobStatus") == "esriJobFailed":
                        if "messages" in job_response:
                            raise Exception(
                                f"Job failed: {job_response.get('messages', '')} "
                            )
                        else:
            ...
            params = {"f": "json"}
            job_info = gptool.session.get(
                url=cancel_url, params=_format_payload(params)
            ).json()
    ...
    if "results" in job_info:
        ...
        for key in list(results.keys()):
            ...
            if "paramUrl" in param_value:
                ...
                param_result = gptool.session.get(
                    result_url, params=_format_payload(params)
                ).json()
...
def _future_op(
    ...
    if gptool.properties.resultMapServerName != "":
        job_id = job_info.get("jobId")
        result_layer_url = (
            gptool.url.replace("/GPServer", "/MapServer") + "/jobs/" + job_id
        )
...
class _GPResource:
    ...
    def __init__(self, url: str, gis: GIS | arcgis.auth.EsriSession | None = None):
        if isinstance(gis, GIS):
            self.session = gis.session
            self._gis = gis
        elif isinstance(gis, arcgis.auth.EsriSession):
            self.session = gis
            self._gis = None
        elif gis is None:
            self.session = arcgis.auth.EsriSession()
            self._gis = None
        else:
            raise ValueError("The `gis` must be of type EsriSession or GIS.")
        self.url = url
        self._token = None
    ...
    def properties(self) -> dict:
        ...
        if self._properties is None:
            self._properties = arcgis._impl.common._mixins.PropertyMap(
                self.session.get(
                    url=self.url,
                    params={
                        "f": "json",
...
def _execute_gp_tool(
    ...
    elif gis and hasattr(gis, "_session"):
        session = getattr(gis, "_session")
    elif gis and hasattr(gis, "session"):
        session = gis.session
    else:
        session = arcgis.auth.EsriSession()
        gis = None
    ...
    #     print(param_name + " = " + str(param_value))
    from arcgis.gis._impl._con._connection import Connection as _Connection

    if isinstance(gis, _Connection):
        gis = gis._session
    gptool = _GPResource(url, gis=gis or session)

    if estimate and "RasterAnalysisTools" in url:
        if gis and isinstance(gis, GIS) and gis._is_arcgisonline == False:
    ...
    if use_async:
        ...
        if add_token and submit_url.lower().find("arcgis.com") == -1:
            ...
            job_info = session.post(submit_url, data=_format_payload(gp_params)).json()

        else:
            job_info = gptool.session.post(
                submit_url, data=_format_payload(gp_params)
            ).json()
        ...
        if future:
            ...
            gpjob = GPJob(
                ...
                jobid=job_id,
                task_url=task_url,
                gis=gptool._gis or gptool.session,
        ...
        resp = gptool.session.post(exec_url, data=_format_payload(gp_params)).json()

        output_dict = {}

        if "results" not in resp:
            raise Exception(f"Tool execution failed with response: {resp}")
