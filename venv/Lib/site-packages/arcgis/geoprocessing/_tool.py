...
def _generate_fn(task, tbx):
    ...
    taskprops_call: requests.Response = tbx.session.post(taskurl, data={"f": "json"})
    taskprops: dict = taskprops_call.json()
...
def import_toolbox(url_or_item, gis=None, verbose=False):
    ...
    **Parameter**      **Description**
    ...
    if gis is None:
        gis = EsriSession()
    elif isinstance(gis, arcgis.gis._impl._con.Connection):
        gis = gis._session
...
from arcgis.geoprocessing._common import _format_payload
    ...
    try:
        ...
        try:
            if str(e).lower().find("token required") > -1:
                ...
                if gis is None:
                    gis = arcgis.env.active_gis or EsriSession()
                tbx = import_toolbox(url_or_item=url_or_item, gis=gis, verbose=verbose)
            return tbx
        except Exception as e:
            raise
    use_async = True
...
class _AsyncResource:
    ...
    _gis: arcgis.gis.GIS | None = None
    _session: EsriSession | None = None
    url: str

    def __init__(self, url: str, gis: arcgis.gis.GIS | EsriSession | None = None):
        """initializer"""
        ...
        if isinstance(gis, arcgis.gis.GIS):
            ...
            raise ValueError(f"Invalid `gis`, got type: {type(gis)}")
    ...
    def session(self) -> EsriSession:
        """returns the current EsriSession object"""
    ...
    def properties(self):
        ...
        if self._properties is None:
            ...
            self._properties = PropertyMap(resp.json())
        return self._properties

    @property
    def _token(self) -> str | None:
        if isinstance(self._session.auth, arcgis.auth.EsriBuiltInAuth):
            return self._session.auth.token
        elif isinstance(self._session.auth, arcgis.auth.EsriGenTokenAuth):
            return self._session.auth.token()
        elif isinstance(self._session.auth, arcgis.auth.ArcGISProAuth):
            return self._session.auth.token
        elif isinstance(self._session.auth, arcgis.auth.EsriOAuth2Auth):
            return self._session.auth.token
        elif isinstance(self._session.auth, arcgis.auth.EsriUserTokenAuth):
            return self._session.auth.token
        elif isinstance(self._session.auth, arcgis.auth.EsriNotebookAuth):
            return self._session.auth.token
        elif isinstance(self._session.auth, arcgis.auth.EsriAPIKeyAuth):
            return self._session.auth.token
        return
    ...
    def _analysis_job(self, task, params):
        ...
        resp_call: requests.Response = self.session.post(submit_url, data=params)
        resp_call.raise_for_status()
        resp: dict = resp_call.json()
        # print(resp)
    ...
    def _analysis_job_status(self, task_url, job_info):
        ...
        if "jobId" in job_info:
            ...
            params = {"f": "json"}
            job_call: requests.Response = self.session.get(url=job_url, params=params)
            job_response: dict = job_call.json()
            ...
            if "jobStatus" in job_response:
                while not job_response.get("jobStatus") == "esriJobSucceeded":
                    ...
                    job_call: requests.Response = self.session.get(
                        url=job_url, params=params
                    )
                    job_response: dict = job_call.json()
                    # print(job_response)
                    messages = (
        ...
        if "results" in job_info:
            ...
            for key in list(results.keys()):
                ...
                if "paramUrl" in param_value:
                    ...
                    params = {"f": "json"}
                    params_resp: requests.Response = self.session.post(
                        url=result_url, data=params
                    )
                    param_result = params_resp.json()
...
class Toolbox(_AsyncResource):
    ...
    def __init__(self, url: str, gis: arcgis.gis.GIS | EsriSession | None = None):
        ...
        if isinstance(gis, arcgis.auth.EsriSession):
            self._session = gis
            self._gis = None
        elif isinstance(gis, arcgis.gis.GIS):
            self.session = gis.session
            self._gis = gis
        else:
            raise ValueError(
                f"Invalid `gis` input, must be a GIS or EsriSession, got a {type(gis)}"
            )
        try:
        ...
        for task in self.properties.tasks:
            ...
            try:
                task_call: requests.Response = self.session.post(
                    url=taskurl, data={"f": "json"}
                )
                taskprops = task_call.json()
    ...
    def _execute(self, params):
        ...
        if self.properties.executionType == "esriExecutionTypeSynchronous":
            resp = self.session.post(url, gp_params)
                ...
                if ret_type in [
                    ...
                    if jsondict is not None:
                        if (
                            ...
                            result_obj.session = self.session
            ...
            submit_url = "{}/submitJob".format(task_url)
            job_info = self.session.post(submit_url, data=gp_params).json()
            ...
            try:
                ...
                params = {"f": "json"}
                job_info = self.session.get(url=cancel_url, params=params).json()
                isCan = True
            if isCan:
                job_info = super()._analysis_job_status(task_url, job_info)
