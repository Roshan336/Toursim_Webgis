...
class ProfileManager(object):
    """
    This class allows for control and management of profiles. Profiles allow for
    storing credentials to a web deployment securely in the local operating system where
    the profile is created. When a profile is created, the url, profile name and date are
    stored in the user's home directory in a file named *.arcgisprofile*. The password is
    encrypted and stored within the operating system's credential store and retrieved when
    the profile name is used to connect.

    This class is not meant to be initized directly, but instead can be initialized
    by importing the *login_profiles* object from the GIS module.

    .. code-block:: python

        # Usage Example: Initialize a ProfileManager with login_profiles:

        from argis.gis import login_profiles
        type(login_profiles)

        >>> <class 'arcgis.gis._impl._profile.ProfileManager'>

    The properties and methods of the :class:`~arcgis.gis._impl._profile.ProfileManager` are
    available on the *login_profiles* object. See the
    :meth:`~arcgis.gis._impl._profile.ProfileManager.create` method for syntax example on
    creating a profile.  After a profile is created, the  profile name can be used as the
    *profile* argument on the machine it was created to
    initialize a :class:`~arcgis.gis.GIS` object rather than explicitly providing other credentials.
    ...
    def list(self, as_df: bool = False):
        """
        This method retrieves the profile names from the *.arcgisprofile*
        configuration file on the machine running the method.
    ...
    def get(self, profile: str):
        """
        This method retrieves the profile information for a given profile name.

        ================  ================================================================
        **Parameter**     **Description**
        ----------------  ----------------------------------------------------------------
        profile           Required String. The profile name to retrieve information about.
        ================  ================================================================
        ...
        .. code-block:: python
            ...
            # Usage Example: Getting specific profile information

            from arcgis.gis import login_profiles
            login_profiles.get("my_profile_name")

            >>> {'date_modified': datetime.datetime(2025, 4, 23, 11, 30, 52, 514940),
    ...
    def delete(self, profile: str):
        """
        This method deletes a profile permanently from the *.arcgisprofile* file.
        ...
        **Parameter**     **Description**
        ...
        .. code-block:: python
            ...
            # Usage Example: Removing a profile from the .arcgisprofile

            from arcgis.gis import login_profiles
            login_profiles.delete("profile_name")
    ...
    def update(
        ...
        **Parameter**     **Description**
        ...
        .. code-block:: python
            ...
            login_profiles.update(profile = "profile_name1",
                                username = "User12345",
                                key_file = "new_key_file")
    ...
    def create(
        ...
        """
        Adds a new entry into the *.arcgisprofile* file in the user's home
        directory on the machine where the method is run. The *profile* argument
        can subsequently be used to intialize a :class:`~arcgis.gis.GIS` object.
        ...
        **Parameter**     **Description**
        ...
        .. code-block:: python
            ...
            # Usage Example: Create a standard profile for built-in authentication
            ...
            # Usage Example: Subsequent connections on the machine where profile was created

            from arcgis.gis import GIS

            gis = GIS(profile="my_profile_name")

        .. note::
            Alternatively, to use the :class:`~arcgis.gis.GIS` object to create profiles, see
            the  *profile* parameter documentation and *Usage Example #12*, as well as the
            `Storing your credentials locally <https://developers.arcgis.com/python/latest/guide/working-with-different-authentication-schemes/#storing-your-credentials-locally>`_
            section of the *Working with different authentication schemes* for details.
        """
        try:
            config = configparser.ConfigParser()
            if self._cfg_exists:
    ...
    def save_as(self, profile: str, gis: GIS):
        ...
        This method saves and adds the provided :class:`~arcgis.gis.GIS` object to the profile.
        ...
        **Parameter**     **Description**
        ...
        .. code-block:: python
            ...
            # Usage Example: Adding GIS properties to a profile

            from arcgis.gis import login_profiles
            gis = GIS("pro")

            login_profiles.save_as(profile="My_Profile_Name", gis=gis)
