...
class SharingGroupManager:
    ...
    **Parameter**      **Description**
        ...
        **Parameter**      **Description**
        ...
        if do_update:
            ...
            if "results" in resp and not "error" in resp["results"][0]:
                ...
                "notSharedWith" in resp
                and hasattr(group, "id")
                and not group.id in resp["notSharedWith"]
            ):
                return True
            elif "notSharedWith" in resp and not group in resp["notSharedWith"]:
                return True
            else:
                return False
        return False
    ...
    # ---------------------------------------------------------------------
    def remove(self, group: arcgis.gis.Group | str) -> bool:
        ...
        **Parameter**      **Description**
        ...
        if isinstance(group, str) and group in g:
            ...
            groups: str = ",".join(g)
            do_update = True
        elif isinstance(group, str) and group == self._gis.users.me.favGroupId == group:
            groups: str = ",".join(g)
            do_update = True
        if do_update:
