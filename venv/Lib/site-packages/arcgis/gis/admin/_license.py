    ...
    **Parameter**        **Description**
    ...
    def all(self) -> list[License]:
        ...
        if "purchases" in properties:
            ...
            for purchase in purchases:
                try:
                    if purchase["provision"]["endDate"] == -1:
                        licenses.append(License(gis=self._gis, info=purchase))
                    elif purchase["provision"][
                        "endDate"
    ...
    def stale_entitlement_usage(
        ...
        *,
        ...
        """
        Build a summary table of users who have been assigned entitlements (licenses/extensions)
        but have never used them or have not used them within a threshold period.
        This method iterates through all licenses in the organization, retrieves their usage reports,
        and identifies users who are considered "stale" based on the provided criteria.

        ====================        ====================================================
        **Parameter**               **Description**
        --------------------        ----------------------------------------------------
        inactive_days               Optional integer, default 365.
                                    Users whose lastUsed is older than now - inactive_days
                                    are considered stale.
        --------------------        ----------------------------------------------------
        include_never_used          Optional boolean, default True. If True, includes
                                    users who have never used the license to the list of
                                    stale users.
        --------------------        ----------------------------------------------------
        verbose                     Optional boolean, default False. If True, prints
                                    skipped licenses and basic progress.
        ====================        ====================================================
        ...
        -------
            ...
            If no stale users are found, returns an empty DataFrame with the expected columns.
        """
        one_year_ago = _dt.datetime.now() - _dt.timedelta(days=inactive_days)

        def _is_stale(last_used: Any) -> bool:
            # Treat missing values as "never used" if enabled
            if last_used is None:
                return include_never_used

            # pandas / numpy missing handling
            try:
            ...
            ts = pd.to_datetime(last_used, errors="coerce")
            if pd.isna(ts):
                return include_never_used

            return ts.to_pydatetime() < one_year_ago
        ...
        try:
            ...
            raise RuntimeError(
                f"Could not list licenses from gis.admin.license.all(): {exc}"
            ) from exc

        for lic in licenses:
            # Fetch license-specific usage report as a DataFrame
            try:
                report_df = lic.report  # pandas DataFrame
            except Exception as exc:
                if verbose:
                    print(f"Skipping license; could not retrieve report: {exc}")
                continue

            if report_df is None or getattr(report_df, "empty", True):
                continue

            # Make a readable label for the license
            lic_key = (
                getattr(lic, "name", None)
                or getattr(lic, "product_name", None)
                or repr(lic)
            )

            # Expecting columns like "Entitlement" and "Users"
            for _, row in report_df.iterrows():
                entitlement = row.get("Entitlement")

                # "Users" is typically a list of dicts: {"user": "...", "lastUsed": "..."}
                users = row.get("Users") or []
                if not isinstance(users, list):
                    # In case some environments return weird structures
                    continue
                ...
                for u in users:
                    ...
                    if not username:
                        continue
                    if _is_stale(u.get("lastUsed")):
                        records.append(
                            {
                                "License": str(lic_key),
                                "Entitlement": (
                                    str(entitlement) if entitlement is not None else ""
                                ),
                                "User": str(username),
                            }
                        )
        ...
        # Shape the result
        base_cols = ["License", "Entitlement", "Stale_User_Count", "Users_List"]
        ...
        grouped = (
            df.groupby(["License", "Entitlement"], sort=False)
            .agg(
                Stale_User_Count=("User", "nunique"),
                Users_List=("User", lambda x: ", ".join(sorted(set(x)))),
            )
            .reset_index()
        )
        return grouped
