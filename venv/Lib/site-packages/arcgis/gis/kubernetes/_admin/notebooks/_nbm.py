...
import requests
from arcgis.gis.nb._nbm import RuntimeConfiguration
...
class KubeRuntimeManager:
    """
    The runtimes resource provides tools to work with registered notebook
    runtimes.

    There is one runtime included in each container image, which is applied
    to a notebook deployment at launch time. Notebook authors open ArcGIS
    Notebooks in their individual notebook deployment. The runtime included
    with the container image makes a precise collection of Python modules
    available to each notebook in the notebook deployment.

    By default, there are two runtimes: ArcGIS Notebook Python 3 Standard
    and ArcGIS Notebook Python 3 Advanced. These runtimes are loaded with
    their corresponding container images during ArcGIS Enterprise on
    Kubernetes deployment by enabling the NotebookServer enterprise
    function.
    """
    ...
    # ----------------------------------------------------------------------
    def __str__(self):
        return "< KubeRuntimeManager @ {url} >".format(url=self._url)
    ...
    # ----------------------------------------------------------------------
    @property
    def runtimes(self) -> Iterable[KubeRuntime]:
        """
    ...
    # ----------------------------------------------------------------------
    def _add(self, configuration: RuntimeConfiguration) -> KubeRuntime | dict[str, Any]:
        """
        ...
        **This is a private method and can change at any release without
        warning**
        ...
        params: dict = configuration.model_dumps(by_alias=True)
        for key in params.keys():
            if params[key] is None:
                params[key] = ""
            elif isinstance(params[key], (bool, dict, list)):
                params[key] = json.dumps(params[key])

        manifest_file: str | None = params.pop("manifestFile", None)
        self._refresh()
        if manifest_file and os.path.isfile(manifest_file):
            with open(manifest_file) as reader:
                ...
                if "notebookRuntime" in data:
                    ...
                    return KubeRuntime(url=runtime_url, gis=self._gis)
                return data
...
class KubeNotebookManager(object):
    ...
    @property
    def runtime_manager(self) -> KubeRuntimeManager:
        """returns the manager to work with the runtimes"""
        url: str = self._url + "/runtimes"
        return KubeRuntimeManager(url=url, gis=self._gis)
