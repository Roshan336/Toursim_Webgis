...
class NotebookFolder:
    ...
    def __init__(self, folder_name: str, da: NotebookDataAccess):
        ...
        self._gis_version = da._gis.version
    ...
    def _upload_single_file(self, file_path: str) -> bool:
        ...
        # Check duplicate file in AGOL workspace before put, since AGOL will hang if try to upload an existing file.
        if self._is_agol:
        ...
        if (resp.status_code in (405, 409) and self._gis_version >= [12, 1]) or (
            resp.status_code == 201 and self._gis_version < [12, 1]
        ):
            raise ValueError(f"File {filename} already exists in the workspace.")
        return True
