...
class SurveyManager:
    ...
    def create(
        ...
        *Inputs*       *Description*
        ...
        ------------   ------------------------------------------------
        folder         Optional string or :class:`arcgis.gis._impl._content_manager.Folder`. If string, the name of the folder to store the survey form item in your ArcGIS content.
        ------------   ------------------------------------------------
        ...
        elif isinstance(folder, arcgis.gis._impl._content_manager.Folder):
            folder_obj = folder
            folder = folder.properties["id"]
        else:
...
class Survey:
    ...
    # ----------------------------------------------------------------------
    def upgrade_survey(self) -> bool:
        """
        Enable an existing form (:class:`~arcgis.apps.survey123.Survey`) for download in the 26.1 and later Survey123 field app.

        :Returns: bool
        """

        type_keywords = self._si.typeKeywords
        if "S123-26" not in type_keywords:
            type_keywords.append("S123-26")

        return self._si.update({"typeKeywords": ", ".join(type_keywords)})
