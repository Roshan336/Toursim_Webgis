...
class Collection(object):
    ...
    **Parameter**        **Description**
    ...
    def __init__(
        ...
        self._gis = gis
        if self._gis._portal.is_logged_in is False:
    ...
    def _create_new_collection(self) -> None:
        ...
        collection_node = utils.create_unique_id()
        ...
        utils.add_resource(self, resource_name=draft, text=text, access="private")
    ...
    def __str__(self) -> str:
        ...
        return f"Collection(url={self._url})"
    ...
    def _get_url(self) -> str:
        ...
        if self._gis._is_arcgisonline:
            # Online
            self._url = (
                "https://storymaps.arcgis.com/collections/{collectionid}/edit".format(
                    collectionid=self._itemid
                )
            )
        else:
            # Enterprise
            self._url = "https://{portal}/apps/storymaps/collections/{collectionid}/edit".format(
    ...
    # ----------------------------------------------------------------------
    def delete_collection(self, permanent: bool = False) -> bool:
        """
        Deletes the collection item.

        .. note::
            If `permanent` is True, the item will be permanently deleted and cannot be recovered.
        """
        # deletes the item
    ...
    def _get_content_type(self, obj) -> str:
        ...
        """
        if isinstance(obj, story_content.Cover):
            return "Cover"
        elif isinstance(obj, story_content.CollectionNavigation):
            return "Collection Navigation"
        elif isinstance(obj, story_content.Image):
            return "Image"
    ...
    def _get_collection_map(self) -> dict:
        # iterate through nodes to find the one that is of type 'collection-map'
        for _, node_data in self._properties["nodes"].items():
            if node_data.get("type") == "collection-map":
                return node_data
        return None

    def _get_location(self, node_data) -> list:
        feature_id = node_data.get("location", {}).get("featureId")
        geometries = self._get_collection_map().get("data", {}).get("geometries", {})
        if feature_id:
            # get the feature_id info and create the lat, long list
            feature_id_dict = geometries.get(feature_id)
            lat = feature_id_dict.get("nodes", {})[0].get("lat")
            long = feature_id_dict.get("nodes", {})[0].get("long")
            if lat and long:
                return [lat, long]
    ...
    def _iterate_content(self) -> object:
        ...
        for child in ui["children"]:
            node = utils.assign_node_class(self, child)
            yield {
        ...
        for item in ui["data"]["items"]:
            visible = not item.get("config", {}).get(
                "isHidden", False
            )  # defaults to True if key missing
            location = None
            if "location" in item:
                location = self._get_location(item)

            if "nodeId" in item:
                node = utils.assign_node_class(self, item["nodeId"])
                yield {
                    "type": self._get_content_type(node),
                    "instance": node,
                    "visibility": visible,
    ...
    # ----------------------------------------------------------------------
    @property
    def content_info(self) -> pd.DataFrame:
        """
    ...
    def update_content_info(
        ...
        **Parameter**        **Description**
        ...
        ---------------     --------------------------------------------------------------------
        visible             Optional boolean. If True, the item is visible. If False, the item is hidden.
                            If a list of indices is passed, all items will be set to the same specified visibility.
        ---------------     --------------------------------------------------------------------
        location            Optional list of [latitude, longitude]. The location to set for the item.
                            The collection must have an active map to be able to set locations.
        ===============     ====================================================================
        ...
        for i, _ in enumerate(items):
            if i in index:
                if visible in [True, False]:
                    ...
                    if (
                        "config"
                        not in self._properties["nodes"][ui_node]["data"]["items"][i]
                    ):
                ...
                if location is not None:
                    ...
                    geometries = (
                        self._get_collection_map().get("data", {}).get("geometries", {})
                    )
                    if not collection_map:
                        raise warnings.warn(
                            "Collection does not have an active map to set locations."
                        )
                    # update the location of the item. If a location already exists, update it
                    feature_id = items[i].get("location", {}).get("featureId")
                    if feature_id and feature_id in geometries:
                        # Update existing location
                        geometries[feature_id]["nodes"][0]["lat"] = location[0]
                        geometries[feature_id]["nodes"][0]["long"] = location[1]
                    else:
                        # Create a new feature_id and location entry
                        new_feature_id = utils.create_unique_id()
                        items[i]["location"] = {"featureId": new_feature_id}
                        geometries[new_feature_id] = {
                            "id": new_feature_id,
                            "nodes": [{"lat": location[0], "long": location[1]}],
                        }
    ...
    def remove(self, index: int) -> bool:
        ...
        **Parameter**        **Description**
        ...
        if isinstance(
            item, (story_content.Video, story_content.Image, story_content.Embed)
        ):
    ...
    def add(
        ...
        **Parameter**        **Description**
        ...
        if isinstance(
            item, (story_content.Image, story_content.Video, story_content.Embed)
        ):
            ...
            resource_node = utils.create_unique_id(resource=True)
            # The item is a portal item or file resource
            ...
            if isinstance(item, arcgis.gis.Item):
                ...
                utils.add_resource(self, file=item, resource_name=name)
    ...
    def _add_custom_properties(self, title, thumbnail):
        ...
        if thumbnail:
            ...
            utils.add_resource(self, file=thumbnail, resource_name=name)
            resources = self._item.resources.list()
            for resource in resources:
                if resource["resource"] == name:
                    self._properties["resources"][resource_node] = {
