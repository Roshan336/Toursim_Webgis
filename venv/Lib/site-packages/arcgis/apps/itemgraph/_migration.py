    ...
    if item.type in JSON_BASED_TYPES:
        ...
        if "View Service" in item.typeKeywords:
            ...
            for layer in item.layers:
                layer_props = dict(layer.manager.properties)
                layer_props_file_path = os.path.join(
                    data_folder, f"layer_{layer_props['id']}_props.json"
                )
            ...
            if needs_export:
                try:
                    i_gis = item._gis
                    if (
                        service_format == "Service Definition"
                        and not i_gis._is_agol
                        and i_gis.properties["enterpriseVersion"] >= "11.3"
                    ):
                        serv_url = i_gis.hosting_servers[0].url
                        pt = import_toolbox(
                            serv_url + "/System/PublishingTools/GPServer", gis=i_gis
                        )
                        service_name = item.url.split("/")[-2]
                        folder_name = item.url.split("/")[-3]
                        sd_res = pt.export_service(
                            service_name=service_name,
                            service_type="FeatureServer",
                            service_folder=folder_name,
                            gis=i_gis,
                        )
                        download_path = sd_res.download(data_folder)
                        item_dict["data_item_type"] = "Service Definition"
                    else:
            ...
            # grab the files necessary to update layer-level properties
            try:
                flc_props = dict(item.layers[0].container.manager.properties)
                ...
                # this means we likely don't have the right auth to access the service
                # and this service will be a by-ref one
                pass
        ...
        if (
            ...
            if (
                ...
                for file in os.listdir(data_folder):
                    if file.endswith(".zip") or file.endswith(".sd"):
            ...
            if service_item:
                ...
                if new_item:
                    update_params = pub_params.copy()
                    del_list = ["name", "url"]
                    for k, v in update_params.items():
                        if v == None or v == "":
                            del_list.append(k)
                    for param in del_list:
                        update_params.pop(param)
                    new_item.update(item_properties=update_params)

                    # update some stuff on the layer level
                    try:
                        flc_props_path = os.path.join(data_folder, "flc_props.json")
                        with open(flc_props_path, "r") as flc_props_file:
                            flc_props = json.load(flc_props_file)

                        og_layers = flc_props.get("layers", []) + flc_props.get(
                            "tables", []
                        )
                        for new_layer in new_item.layers + new_item.tables:
                            new_props = dict(new_layer.manager.properties)
                            for props in og_layers:
                                if new_props["id"] == props["id"]:
                                    update_properties = {}
                                    for key in [
                                        "drawingInfo",
                                        "maxRecordCount",
                                        ...
                                        if (
                                            key in props
                                            and props[key] != new_props[key]
                                        ):
                                            update_properties[key] = props[key]
                                    new_fields = new_props["fields"]
                                    for i in range(len(new_fields)):
                                        if "description" in props["fields"][i]:
                                            new_fields[i]["description"] = props[
                                                "fields"
                                            ][i]["description"]
                                    update_properties["fields"] = new_fields
                                    new_layer.manager.update_definition(
                                        update_properties
                                    )
                                    break
                    except:
                        # unable to update layer-level properties, raise a warning
                        warnings.warn(
                            f"Unable to update layer-level properties for item {new_item.id}. Field descriptions, drawingInfo, description, or maxRecordCount may not have been updated.",
                            RuntimeWarning,
                        )
                        pass
