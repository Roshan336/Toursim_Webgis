...
from ._ge import GeoEnrichment
from ._helper import service_properties
from ._utils import pep8ify
...
class NamedArea(object):
    ...
    def _childlevels(self):
        dset = [
            ...
            if dset["datasetID"] == self._country.default_dataset
        ][0]
...
class Country(object):
    """
    The Country object enables access to Business Analyst data and methods for a specific country thorough a specified
    source. A source can either be in an environment with ArcGIS Pro with Business Analyst and local
    data (``local``) or a Web GIS (``GIS``) object instance with configured GeoEnrichment service.
    The Web GIS can reference either an instance of ArcGIS Enterprise with Business Analyst or ArcGIS Online.
    Available countries can be discovered using :func:`~arcgis.geoenrichment.get_countries`.

    .. note::

        If the gis is not explicitly set, the ``Country`` object will first try to use
        ``local`` source, ArcGIS Pro with the Business Analyst extension. If this is not available,
        ``BusinessAnalyst`` will try to use a ``GIS`` object instance already in the session. If
        neither is available, and a ``gis`` is not set, this will invoke an error.

    .. warning::

        GeoEnrichment using ArcGIS Online *does* cost credits.
    """

    @property
    def source(self) -> Union[str, GIS]:
        """
        Source being used. Expected value is the *local* keyword or an
        instantiated :class:`~arcgis.gis.GIS` object.
        """
        return self._source
    ...
    def source(self, in_source: Optional[Union[str, GIS]] = None) -> None:
        ...
        if isinstance(self._source, GIS):
            ...
            self._base_url = get_helper_service_url(self._source, "geoenrichment")
    ...
    def get(
        ...
        """
        Get a reference to a particular country, given its:

        * *name*
        * `two letter abbreviation <https://www.iso.org/obp/ui/#search>`_
        * `three letter ISO3 code <https://www.iso.org/obp/ui/#search>`_
        ...
        **Parameter**           **Description**
        ...
        gis                    Optional :class:`~arcgis.gis.GIS` instance. This
                               ...
                               .. note::
                                   If not explicitly specified, it tries to use a *local*
                                   source, ArcGIS Pro with the Business Analyst extension.
                                   If this is not available, it will try to use a
                                   *GIS* object already in the session. If neither is available,
                                   and no active *gis* is set, this will raise an error.
        ---------------------  --------------------------------------------------------
        hierarchy              Optional string. Explicitly specifying the hierarchy
                               (year) to query in the case of multiple years of data
                               being available.

                               .. note::
                                   Only available with a Web GIS source. Argument is
                                   ignored if used with a *local* GIS source.
        ---------------------  --------------------------------------------------------
        ...
        derivative_variables   Optional boolean. Support derivative variables.
                               Return a data collection with these additional variables:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(self.source, GIS):
            # Online
            self.api_provider = GeoEnrichment(
                gis=self.source,
                country=iso3,
                hierarchy=hierarchy,
                derivative_variables=derivative_variables,
            ...
            # Local
            self.api_provider = _business_analyst.BusinessAnalyst(
        ...
        if isinstance(self.source, GIS):
            ...
            hlp_svcs = self.source.properties["helperServices"]
            ...
            if gis._is_hosted_nb_home:
                res = self.source._private_service_url(self._base_url)
                prv_url = (
                    res["privateServiceUrl"]
                    if "privateServiceUrl" in res
                    else res["serviceUrl"]
            ...
            self.default_dataset = self.properties.default_dataset

    def __repr__(self):
        if self.source == "local":
            repr = (
                f"<{type(self).__name__} - {self.properties.name} {self.properties.year} "
                f"({self.source.__repr__()})>"
            )
        else:
            repr = f"<{type(self).__name__} - {self.properties.name} ({self.source.__repr__()})>"
        return repr
    ...
    def properties(self):
        ...
        """
        return self.api_provider.properties
    ...
    def levels(self) -> pd.DataFrame:
        ...
        """
        return self.api_provider.geography_levels

    def _levels_gis(self):
        """GIS levels implementation."""
        dset = [d for d in self._geog_levels if d["datasetID"] == self.default_dataset][
            0
        ]
        lvls = dset["levels"]
        return lvls
    ...
    def data_collections(self):
        ...
        """
        return self.api_provider.data_collections
    ...
    def hierarchies(self):
        ...
        """
        return self.api_provider.hierarchies

    @property
    def hierarchy(self):
        """Get/Set the current hierarchy used. This will affect the enrichment variables"""
        return self.hierarchies[self.hierarchies["ID"] == self.api_provider.hierarchy]
    ...
    def enrich_variables(self):
        ...
        analysis.
        """
        return self.api_provider.enrich_variables
    ...
    def _get_enrich_variables_from_iterable(
        ...
        **Parameter**                     **Description**
        ...
        """
        return self.api_provider.get_enrich_variables_from_iterable(enrich_variables)
    ...
    def enrich(
        ...
        **Parameter**                     **Description**
        ...
        enrich_res = self.api_provider.enrich(
            study_areas=study_areas,
    ...
    def travel_modes(self):
        ...
        """
        return self.api_provider.travel_modes
...
def get_countries(gis: Optional[GIS] = None, as_df: bool = True):
    ...
    **Parameter**           **Description**
    ...
    if isinstance(gis, GIS):
        out_res = GeoEnrichment(gis).countries
    else:
        out_res = _business_analyst.BusinessAnalyst().countries
...
def _preproces_data_colletions_and_analysis_variables(
    ...
    **kwargs,
    ...
    if data_cols is not None:
        ...
        dc_vars = src.api_provider.enrich_variables[
            src.api_provider.enrich_variables["data_collection"].isin(data_cols)
        ]
    else:
        dc_vars = None
    ...
    if enrich_vars is not None and not isinstance(enrich_vars, pd.DataFrame):
        av_vars = src.api_provider.get_enrich_variables_from_iterable(
            enrich_vars, **kwargs
        )
    else:
        av_vars = None
