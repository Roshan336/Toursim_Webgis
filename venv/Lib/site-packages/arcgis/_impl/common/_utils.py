...
from arcgis._impl.common import _mixins
from arcgis._impl.common import _isd
...
def _prepare_parameters(params: dict[str, Any]) -> dict[str, Any]:
    for k, v in params.items():
        if isinstance(v, (dict, list, tuple, bool)):
            params[k] = json.dumps(v, default=_date_handler)
        elif isinstance(v, _mixins.PropertyMap):
            params[k] = json.dumps(dict(v), default=_date_handler)
        elif isinstance(v, _isd.InsensitiveDict):
            params[k] = v.json
    return params
