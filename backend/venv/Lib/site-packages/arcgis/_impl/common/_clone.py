...
class _DeepCloner:
    ...
    def __init__(
        ...
        self._cloned_items = []
        self._dashboard_mapping = dashboard_mapping
        self._dashboard_options = dashboard_options
        if dashboard_mapping is not None:
            for mapping in dashboard_mapping:
                self._clone_mapping["Item IDs"][
                    mapping.source_item_id
                ] = mapping.target_item_id
        self._create_graph()
    ...
    def _get_item_definitions(self, item):
        ...
        # If the item is an application or dashboard find the web map or group that the application referencing
            ...
            self._graph[item.id] = item_definition
            source_dash_api = (
                item._gis.properties["helperServices"]
                .get("dashboardsUtility", {})
                .get("url")
            )
            if source_dash_api:
                src_dbm = DashboardManager(item, item._gis)
                dep_results = src_dbm.get_dependencies()
                deps = [d.item_id for d in dep_results]
            else:
                deps = item.get_dependencies(out_format="id")

            for dep in deps:
                try:
                    dep_item = source.content.get(dep)
                    if dep_item:
                        item_definition.add_child(self._get_item_definitions(dep_item))
                except RuntimeError:
                    raise
        ...
        elif item["type"] in ["Web Experience", "Web Experience Template", "StoryMap"]:
            item_definition = self._get_item_definition(item)
            self._graph[item.id] = item_definition
            deps = item.get_dependencies(out_format="id")
    ...
    def _get_item_definition(self, item):
        ...
        if item["type"] in clone_registry():
            ...
            return _DashboardDefinition(
                ...
                owner=self.owner,
                preserve_item_id=self._preserve_item_id,
                dash_mapping=self._dashboard_mapping,
                dash_options=self._dashboard_options,
            )
...
class _FeatureServiceDefinition(_TextItemDefinition):
    ...
    def _add_features(
        ...
        for layer_id in layer_ids:
            ...
            if (
                "multiScaleGeometryInfo" in layers[layer_id].properties
                and self.target._portal.is_arcgisonline
            ):
                is_generalized = True
                ...
                if self._export and can_export:
                    ...
                    for layer in (
                        ...
                        if self.is_view:
                            ...
                            if (
                                ...
                                and layer.get("adminLayerInfo", None) is not None
                    ...
                    if len(layers_and_tables) > 0:
                        for o in layers_and_tables:
                            ...
                            if not self.target._portal.is_arcgisonline:
                                if (
                                    '"Shape_Area"' in definition
                                    and '"Shape__Area"' in definition
                                ):
                                    definition = definition.replace(
                                        '"Shape_Area"', '"ShapeArea"'
                                    )
                                if (
                                    '"Shape_Leng"' in definition
                                    and '"Shape__Length"' in definition
                                ):
                                    definition = definition.replace(
        ...
        try:
            ...
            if not new_item:
                ...
                for layer in layers:
                    ...
                    if (
                        feature_service_url
                        and updated_layer == False
                        and "itemId" in layer
                    ):
                        # living atlas cloning to Enterprise case
                        # remove itemId so it's referenced by URL only if going to Enterprise
                        if (
                            self.target.content.get(layer["itemId"]) == None
                            and layer["itemId"] not in self._clone_mapping["Item IDs"]
                        ):
                            layer.pop("itemId")
...
class _DashboardDefinition(_TextItemDefinition):
    ...
    def __init__(
        ...
        self._dash_mapping = dash_mapping
        self._dash_options = dash_options
    ...
    def clone(self):
        try:
            ...
            if not new_item:
                ...
                if target_dash_api and self._dash_mapping:
                    app_json_text = json.dumps(app_json)
                    item_properties["text"] = app_json_text
                    new_item = self._add_new_item(item_properties)
                    tgt_dbm = DashboardManager(new_item, self.target)
                    tgt_dbm.upgrade()
                    tgt_dbm.replace_dependencies(self._dash_mapping, self._dash_options)

                else:
                    if app_json and "version" in app_json:
                        target_dash_url = (
                            self.target.url + "/apps/dashboards/build.json"
                        )
                        target_version = (
                            self.target._session.get(target_dash_url)
                            .json()
                            .get("version", None)
                        )
                        source_version = app_json["version"]
                        can_clone = True
                        if isinstance(source_version, str) and isinstance(
                            target_version, str
                        ):
                            if source_version > target_version:
                                can_clone = False
                        elif isinstance(source_version, str) and isinstance(
                            target_version, (int, float)
                        ):
                            can_clone = False
                        elif isinstance(source_version, (int, float)) and isinstance(
                            target_version, (int, float)
                        ):
                            if source_version > target_version:
                                can_clone = False
                        if not can_clone:
                            raise _ItemCreateException(
                                "Dashboard version {} cannot be cloned to target organization with lower dashboard version {}".format(
                                    source_version, target_version
                                )
                            )
                        if isinstance(source_version, str) or source_version >= 24:
                    ...
                    map_dict = copy.deepcopy(self._clone_mapping["Item IDs"])
                    if "desktopView" in app_json:
                        selectors = _deep_get(
                            app_json, "desktopView", "header", "selectors"
                        )
                        if selectors:
                    ...
                    # Add the new item
                    new_item = self._add_new_item(item_properties)
                    new_item.remap_data(item_mapping=map_dict, force=True)
                    if target_dash_api:
                        tgt_dbm = DashboardManager(new_item, self.target)
                        tgt_dbm.upgrade()
            else:
        ...
        try:
            ...
            if not new_item:
                ...
                if original_item["url"] is not None:
                    ...
                    update_properties = {"url": url}
                    new_item.update(update_properties)
...
def _deep_get(dictionary, *keys):
    ...
    *keys - The keys used to fetch the desired value"""
    ...
    if isinstance(dictionary, PropertyMap):
