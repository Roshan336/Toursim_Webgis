...
class Query:
    ...
    def _get_url(self):
        """
        Get the query url for the layer or table.
        """
        if self.url:
            # already set in initializer
            return
    ...
    def _fetch_total_records_count(self):
        ...
        oid_field = self.get_oid_field_name(self.layer)
        if oid_field:
            count_params["outFields"] = oid_field
    ...
    @classmethod
    def get_oid_field_name(cls, layer):
        # Get OID field for use in feature count methods
        return [
            f for f in layer.properties.fields if f.get("type") == "esriFieldTypeOID"
        ][0].get("name")
