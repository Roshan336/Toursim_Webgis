...
def _get_arcgis_pipeline(
    ...
    # enterprise support will be added post 12.1 enterprise release.
    if gis._is_arcgisonline == False:
        raise Exception("Datapipelines is only supported on ArcGIS Online.")
    ...
    if "dataPipelines" in helper_service:
        ...
        not "dataPipelines" in helper_service
        and gis._is_kubernetes == False
        and gis._is_arcgisonline == False
    ):
        urls: list[str] = [
            server["url"]
            for server in gis.servers.get("servers", [])
            if server.get("serverType", None) == "ARCGIS_DATA_PIPELINES_SERVER"
        ]
        if len(urls) > 0:
            url: str = f"{urls[0]}/rest/"
        else:
            return
        return DataPipelines(url=url, gis=gis)
    return None
