...
class GPJob(object):
    ...
    **Parameter**      **Description**
    ...
    def __init__(self, future, gptool, jobid, task_url, gis, notify=False):
        ...
        self._jobid = jobid
        self._url = task_url
        if isinstance(gis, EsriSession):
            self.session = gis
            self._gis = env.active_gis or None
        elif isinstance(gis, GIS):

            self._gis = gis
            self.session = gis.session
        else:
            raise ValueError(f"`gis` only accepts a GIS or EsriSession")
    ...
    def _process_ortho(self, result):
        ...
        if hasattr(result, "_fields"):
            ...
            if isinstance(value, DataFile) and self.task != "GenerateReport":
                return self._gis.session.get(value.to_dict()["url"]).content
    ...
    def _process_reality(self, result):
        ...
        if hasattr(result, "_fields"):
            ...
            if isinstance(value, DataFile):
                return self.session.get(value.to_dict()["url"]).content
            if isinstance(value, (RasterData, LinearUnit)):
                return value
            elif isinstance(value, str) and value.lower().find("imageserver") > -1:
                return value
