...
def current_runtime_version(gis: GIS = None) -> str | None:
    """
    This helper returns the current runtime version for the GIS system.
    This is helpful when trying to add notebooks to the `GIS` as it is a
    required property to associate with the Notebook Item.

    `None` is returned when a notebook server cannot be found.
    ...
    if len(mgrs) > 0:
        if gis._portal.is_arcgisonline:
            mgr = mgrs[0]
            return mgr.runtime_manager.current_runtime_version
        elif gis._is_kubernetes:
            mgr = mgrs[0]
            nb = mgr.notebooks
            return nb.runtime_manager.current_runtime_version
        else:
            mgr = mgrs[0]
            nb = mgr.notebooks
            rt = nb.runtime_manager
            return rt.current_runtime_version
    return None
...
def list_runtimes(gis: GIS = None) -> list:
    ...
    if len(mgrs) > 0:
        if gis._portal.is_arcgisonline:
            mgr = mgrs[0]
            rt = mgr.runtime_manager
            return [r for r in rt.runtimes]
        elif gis._is_kubernetes:
            mgr = mgrs[0]
            nb = mgr.notebooks
            return [r for r in nb.runtime_manager.runtimes]
        else:
            mgr = mgrs[0]
            nb = mgr.notebooks
            rt = nb.runtime_manager
