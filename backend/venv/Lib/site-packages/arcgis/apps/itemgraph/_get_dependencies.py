...
def _parse_webmap(item):
    ...
    def process_op_layer(layer):
        if layer.get("layerType") == "GroupLayer":
            ...
            if "itemId" in layer:
                ...
                try:
                    sid = FeatureLayer(layer["url"], gis=item._gis).properties[
                        "serviceItemId"
                    ]
                    items.add(sid)
                except:
    ...
    items.update(_find_regex(webmap_json, _REGEX_GUID, [], scrub_urls=True))
    return list(items)
...
def _find_regex(i, regex, res=[], scrub_urls=False):
    """
    ...
    if isinstance(i, dict):
        for v in i.values():
            _find_regex(v, regex, res, scrub_urls)
    elif isinstance(i, list):
        for v in i:
            _find_regex(v, regex, res, scrub_urls)
    elif isinstance(i, str):
        if scrub_urls:
            i = re.sub(_REGEX_URL, "", i)
