...
def samples_extraction(path, window_size, max_num, min_num, training_class_map):
    ...
    results = []
    for res in progress_bar(all_chips):
...
def prepare_hyperspec_data(
    ...
    **kwargs,
    ...
    original_classes = {
        i["Value"]: i["Name"] for n, i in enumerate(emd_stats["Classes"])
    }
    if class_mapping:
    ...
    data.chipsize_height = trainChips_dl.__getitem__(0)[0].shape[1]
    data.chipsize_width = trainChips_dl.__getitem__(0)[0].shape[2]
    data._emd = emd_stats
