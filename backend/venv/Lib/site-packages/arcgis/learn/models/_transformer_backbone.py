...
from ._terramind_utils import TerraMindViT, TerraMindAdapter
import timm
import logging
import warnings
...
wavelengths_required_cfg = dict(
    ...
    clay_large=dict(
        ...
        item_id="2363f050d6a846be959fc25473c7d3e8",
    ),
    terramind_large=dict(
        backbone_name="terramind_large",
        patch_size=16,
        encoder_depth=24,
        dim=1024,
        num_heads=16,
...
vit_foundation_model_config = dict(
    ...
    prithvi_v2_300M=dict(
        ...
        pretrained_path="https://huggingface.co/ibm-nasa-geospatial/Prithvi-EO-2.0-300M/resolve/main/Prithvi_EO_V2_300M.pt",
        weight_name="Prithvi_EO_V2_300M.pt",
        item_id="10b8f77274914c54880a976d02956de7",
    ),
    prithvi_v2_600M=dict(
        backbone_name="prithvi",
        patch_size=14,
        embed_dim=1280,
        depth=32,
        ...
        pretrained_path="https://huggingface.co/ibm-nasa-geospatial/Prithvi-EO-2.0-600M/resolve/main/Prithvi_EO_V2_600M.pt",
        weight_name="Prithvi_EO_V2_600M.pt",
        item_id="60c74d12243d46dc88313678a02c0074",
    ),
...
def custom_backbone(
    ...
    with warnings.catch_warnings():
        ...
        if backbone_name in vit_config.keys():
            ...
            if "terramind" in backbone_name:
                merged_cfg.update({"in_chans": in_chans})
                backbone = TerraMindViT(**merged_cfg)
                if not kwargs.get("is_clf", False):
                    backbone = TerraMindAdapter(
                        backbone, in_chans, layer=-1, remove_cls_token=False
                    )
            else:
