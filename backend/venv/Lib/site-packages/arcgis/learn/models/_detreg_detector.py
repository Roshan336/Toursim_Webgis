...
class CustomDetReg:
    ...
    def on_batch_begin(self, learn, model_input_batch, model_target_batch, **kwargs):
        ...
        for bbox, label in zip(*model_target_batch):
            ...
            mask = (bbox[:, 2:] > (bbox[:, :2] + 1e-6)).all(
                1
            )  # added small epsilon to avoid zero area boxes.
            bbox = bbox[mask]
            label = label[mask]

            # If no boxes are left, DETR needs a dummy 'background'
            if len(bbox) == 0:
                # Create a tiny dummy box at 0,0 to handle empty image (no boxes in GT) for hungarian matcher and loss calculation.
                bbox = self.torch.tensor([[0.0, 0.0, 0.01, 0.01]]).to(learn.data.device)
