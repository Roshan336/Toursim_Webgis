...
def init_prithvi(model, pretrained_path):
    if os.path.exists(pretrained_path):
        raw_stdict = torch.load(pretrained_path)
    else:
    ...
    for k, v in raw_stdict.items():
        ...
        if "pos_embed" in k and "decoder" not in k and v.shape != model.pos_embed.shape:
            # prithiv1 there are 589(3 frame 196*3 + 1(cls_token)) poistion encoding but interpolation needed in grid_size*2 + 1(cls_token)
            # prithviv2 there are 785 poistion encoding, since they have taken 4 frame (196*4 + 1(cls_token)) in prithvi_eo_v2_300
            pretrained_num_token = (int(math.sqrt(v.shape[1])) ** 2) + 1
            num_token = min(pretrained_num_token, model.pos_embed.shape[1])
            v = v[:, :num_token]
