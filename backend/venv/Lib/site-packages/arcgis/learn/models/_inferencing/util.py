...
def update_pixels_img_trans(self, tlc, shape, props, **pixelBlocks):
    ...
    np_dtype = pixelBlocks["raster_pixels"].dtype
    torch_dtype = torch.from_numpy(np.zeros(0, dtype=np_dtype)).dtype
    ...
    if model_name in ["SuperResolution"]:
        merged_preds = merged_preds.to(torch_dtype)
