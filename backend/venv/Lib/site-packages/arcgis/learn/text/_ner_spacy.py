        ...
        **Parameter**            **Description**
        ...
        if self._trained:
            ...
            for (
                ...
                df.columns
            ):  # Filling all empty with empty string or np.nan based on the dtype of the column, to avoid errors in post processing steps.
                if isinstance(df[col].dtype, str):
                    dtype_na_mapping[col] = ""
                elif isinstance(df[col].dtype, float) or isinstance(df[col].dtype, int):
                    dtype_na_mapping[col] = np.nan
                else:
                    dtype_na_mapping[col] = ""
            df.fillna(dtype_na_mapping, inplace=True)
