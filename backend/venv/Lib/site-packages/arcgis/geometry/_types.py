...
def _is_valid(value) -> bool:
    if not value:
        ...
        from ._validator import _PointValidator as Validator
    elif isinstance(value, MultiPoint):
        from ._validator import _MultiPointValidator as Validator
    elif isinstance(value, Polygon):
        from ._validator import _PolygonValidator as Validator
    elif isinstance(value, Polyline):
        from ._validator import _PolylineValidator as Validator
    elif isinstance(value, SpatialReference):
        from ._validator import _SpatialReferenceValidator as Validator
    elif isinstance(value, Envelope):
        from ._validator import _EnvelopeValidator as Validator
    else:
        return False
    try:
        Validator(**value)
...
class GeometryFactory(type):
    ...
    def _from_wkb(iterable):
        if HAS_ARCPY:
            ...
            return Geometry(wkb.loads(iterable))
        return {}
...
class Geometry(BaseGeometry, metaclass=GeometryFactory):
    ...
    def has_curves(self) -> bool:
        ...
        if isinstance(self, Polygon) and "curveRings" in self:
            for part in self["curveRings"]:
                for g in part:
                    if "a" in g or "b" in g or "c" in g:
                        return True
        elif isinstance(self, Polyline) and "curvePaths" in self:
            for part in self["curvePaths"]:
                for g in part:
                    if "a" in g or "b" in g or "c" in g:
                        return True
        return False
    ...
    def is_multipart(self):
        ...
        :class:`geometry <arcgis.geometry.Geometry>` has more than one part.

        .. note::
            ArcPy provides an additional layer of Geometry checks than pure
        ...
        * **Polygon** - ``True`` if it contains more than one **exterior** ring.
        ...
        .. note::
            Relies on the ring orientation to determine whether it is
