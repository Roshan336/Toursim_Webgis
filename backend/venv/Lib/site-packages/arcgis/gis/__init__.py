...
dashboards = LazyLoader("arcgis.apps.dashboards")
...
class GIS(object):
    ...
    **Parameter**        **Description**
    ...
    profile             Optional string. the name of the *profile* that the user wishes to use
                        to use to authenticate. If argument is provided, the identified
                        *profile* will be used to login to the specified GIS.

                        See `Storing your credentials locally <https://developers.arcgis.com/python/latest/guide/working-with-different-authentication-schemes/#storing-your-credentials-locally>`_
                        for more details.
    ================    ===============================================================
    ...
    **kwargs**                **Description**
    ...
    .. code-block:: python
        ...
        # Usage Example 12: Creating a profile to use for storing credentials locally
        ...
        >>>         username="gis_user",
        >>>         password="secure_password",
        >>>         profile="gis_user_profile"
        >>>       )

        # Subsequent connections from that machine where the profile was created:
        >>> gis = GIS(profile="gis_user_profile")
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self._con._auth != "anon":
            ...
            me = self.users.me or {}
        logged_in_user: User | dict = self.users.me or {}
        if self._is_hosted_nb_home and logged_in_user.get("role", None) == "org_admin":
            import warnings
        ...
        self._is_agol = self._is_arcgisonline
    ...
    def admin(
        self,
    ) -> "PortalAdminManager" | "KubernetesAdmin" | "AGOLAdminManager" | None:
        """
        Returns the administrative object for the GIS object.
        If the user does not have permission, `None` is returned.
        """
        ...
        if self._adminPrivateServiceUrl:
            ...
            url: str = f"{self.url}/admin"
        elif self._is_kubernetes == False and self._is_arcgisonline == False:
            url: str = f"{self.url}/portaladmin"
        else:
            url: str | None = None  # AGOL
        # get the user and check permissions

        logged_in_user: User = self.users.me or {}
        is_admin: bool = True
        can_publish: bool = getattr(logged_in_user, "_can_publish", False)
        if logged_in_user is None or logged_in_user == {}:
            return None

        if logged_in_user.get("role", None) == "org_publisher":
            is_admin == False
        # create the admin URL
        if self._is_arcgisonline and (is_admin or can_publish):
            self._admin = AGOLAdminManager(gis=self)
        elif self._is_kubernetes and (is_admin or can_publish):
            self._admin = KubernetesAdmin(url=url, gis=self)
        elif (
            self._is_arcgisonline is False
            and self._is_kubernetes is False
            and (is_admin or can_publish)
        ):
            # enterprise
            self._admin = PortalAdminManager(url=url, gis=self, is_admin=is_admin)
        else:
            self._admin = None
        return self._admin
    ...
    @staticmethod
    def _get_proxy_decrypted(encrypted_proxy_dict, nbauth_file_path):
        from arcgis.gis._impl.nbauth import get_value

        proxy = {}
        for protocol, encrypted_proxy in encrypted_proxy_dict.items():
            encrypted_proxy_split = urlsplit(encrypted_proxy)
            if not encrypted_proxy_split.username or not encrypted_proxy_split.password:
                # skip decryption if username or password is not present in the proxy url
                proxy[protocol] = encrypted_proxy
                continue
            scheme = encrypted_proxy_split.scheme
            username = encrypted_proxy_split.username
            password = get_value(encrypted_proxy_split.password, nbauth_file_path)
            hostname = encrypted_proxy_split.hostname
            port = (
                f":{encrypted_proxy_split.port}" if encrypted_proxy_split.port else ""
            )
            path = encrypted_proxy_split.path
            query = (
                f"?{encrypted_proxy_split.query}" if encrypted_proxy_split.query else ""
            )
            fragment = (
                f"#{encrypted_proxy_split.fragment}"
                if encrypted_proxy_split.fragment
                else ""
            )
            decrypted_proxy = f"{scheme}://{username}:{password}@{hostname}{port}{path}{query}{fragment}"
            proxy[protocol] = decrypted_proxy
        return proxy

    @staticmethod
    def _get_hosted_nb_config(nbauth_dict, nbauth_file_path):
        try:
            required_json_keys = set(["privatePortalUrl", "publicPortalUrl", "referer"])
            assert required_json_keys.issubset(
                nbauth_dict
            ), f"Hosted notebook authentication file is missing required keys. Required keys are {list(required_json_keys)}"
            config = {
                "url": nbauth_dict["privatePortalUrl"],
                "restUrl": _create_base_url(nbauth_dict["privatePortalUrl"]),
                "publicPortalUrl": nbauth_dict["publicPortalUrl"],
                "adminPrivateServiceUrl": nbauth_dict.get("adminPrivateServiceUrl"),
                "referer": nbauth_dict["referer"],
            }
            if config["adminPrivateServiceUrl"] is None:
                config["adminPrivateServiceUrl"] = (
                    nbauth_dict["privatePortalUrl"] + "/portaladmin"
                )
            utoken = nbauth_dict.get("token")
            expiration = nbauth_dict.get("expiration")
            if expiration:
                config["expiration"] = expiration
            encrypted_token = nbauth_dict.get("encryptedToken")
            if encrypted_token:
                from arcgis.gis._impl.nbauth import get_token

                utoken = get_token(nbauth_file_path)
            if utoken:
                config["utoken"] = utoken
            if nbauth_dict.get("proxies"):
                config["proxies"] = GIS._get_proxy_decrypted(
                    nbauth_dict["proxies"], nbauth_file_path
                )
            return config

        # Catch errors and re-throw in with more human readable messages
        except AssertionError:
            GIS._raise_hosted_nb_error(
                "Authentication file doesn't contain "
                f"required keys {list(required_json_keys)}"
            )
        except Exception as e:
            GIS._raise_hosted_nb_error(
                "Unexpected exception when authenticating " f"through 'home' mode: {e}"
            )
    ...
    def _try_configure_for_hosted_nb(self):
        ...
        hosted_nb_config = self._get_hosted_nb_config(json_data, nb_auth_file_path)
        self._url = hosted_nb_config["url"]
        self.resturl = hosted_nb_config["restUrl"]
        self._public_portal_url = hosted_nb_config["publicPortalUrl"]
        self._adminPrivateServiceUrl = hosted_nb_config.get("adminPrivateServiceUrl")
        self._referer = hosted_nb_config.get("referer") or ""
        if "utoken" in hosted_nb_config:
            self._utoken = hosted_nb_config["utoken"]
        self._expiration = hosted_nb_config.get("expiration")
        if hosted_nb_config.get("proxies"):
            self._proxy = hosted_nb_config["proxies"]

    @staticmethod
    def _raise_hosted_nb_error(err_msg):
    ...
    def notebook_server(
        ...
        elif self._portal.is_arcgisonline == False and self._is_kubernetes == False:
    ...
    # ----------------------------------------------------------------------
    def __str__(self):
        return f"GIS @ {self.url} version:{self.version}"
    ...
    def map(
        ...
        """
        Creates a map widget centered at the declared location. If an address is
        provided, it is geocoded using the :class:`~arcgis.geocoding.Geocoder`
        configured with the :class:`~arcgis.gis.GIS`. Provided a match is found,
        the geographic extent of the matched address is used as the map extent.
        See the :class:`~arcgis.map.Map` documentation for more information.

        .. note::
            Visualization of the map widget is only supported within an ArcGIS
            notebook created with ArcGIS Pro, ArcGIS Online or a Jupyter
            server started within a local environment. Map and scene widgets
            can also be created in scripts, but cannot be visualized.

        ==================     ===========================================================
        **Parameter**           **Description**
        ------------------     -----------------------------------------------------------
        location               Optional string or :class:`~arcgis.geometry.Point` object.
                               The string can be a name or single-line address where the
                               map is to be centered.

                               .. note::
                                   A tuple of latitude-longitude coordinates can be entered
                                   in versions of the API prior to 2.4.0.  The ability to use
                                   *point* objects for this argument was introduced at 2.4.3.
        ------------------     -----------------------------------------------------------
        mode                   Optional string of either '2D' or '3D' to specify map mode.
                               Defaults to '2D'.
        ==================     ===========================================================

        :return:
          A :class:`map <arcgis.map.Map>` or :class:`scene <arcgis.map.scene_widget.Scene>` object.
        ...
        .. code-block:: python
            ...
            # Usage Example: Using a point geometry
            ...
            m = gis.map(location=pt_goem, mode="2D")
            type(m)

            >>> <class 'arcgis.map.map_widget.Map'>

            # Usage Example #2: Using a place name

            sc = gis.map(location="Durham,NC", mode="3D")
            type(sc)

            >>> <class 'arcgis.map.scene_widget.Scene'>

            # Usage Example #3: Using an address

            m2 = gis.map(location="location="5 Avenue Anatole France, 75007 Paris, France")
            m2.center

            >>> [48.85881899870472, 2.294597040067714]
    ...
    def export_items(
        ...
        **Parameter**       **Description**
        ...
        service_format      Optional string. The format of the data from any source hosted
                            ...
                            * *geoPackage*
                            * *Vector Tile Package*
                            * *Service Definition* (only available for ArcGIS Enterprise 11.3+)
        ---------------     --------------------------------------------------------------------
        update_fc           Optional boolean. If True, feature-class based Feature Layers will
                            be exported to the specified `service_format`, instead of
                            re-publishing the original data. This is useful when feature layer
                            data has been updated from the originally published source data.
                            Default is False.
        ===============     ====================================================================
        ...
        graph = create_dependency_graph(
            self._gis, items, outside_org=False, update_fc=update_fc
        )
...
class UserManager(object):
    ...
    def user_settings(self):
        """
        Gets/sets the new member defaults for an organization. See each
        deployment documentation for full explanations:
        ...
        * `ArcGIS Online <https://doc.arcgis.com/en/arcgis-online/administer/configure-new-member-defaults.htm>`_
        ...
        Allows administrators to get and update the default values for new
        built-in accounts created directly or through automatic account
        creation.

        To set the values, assign the property to a dictionary with key-value pairs
        specifying the default values.

        .. note::
            Passing in `None` to the property will delete all current settings.

        **Key/Value Pairs for setting the values**

        ================  ===============================================================================
        **Key**           **Value**
        ----------------  -------------------------------------------------------------------------------
        role              String/Role. A *role* name or :class:`~arcgis.gis.Role` object.

                          * To use default roles, the role name can be used:
                          ...
                          * To use a custom role, use a :class:`~arcgis.gis.Role` object
        ----------------  -------------------------------------------------------------------------------
        userLicenseType   String. The ID of a user type licensed with your organization. To see which
                          user types are included with your organization's licensing, as an administrator
                          query the :attr:`~arcgis.gis.UserManager.license_types` property for the
                          *id* of the user type options:

                          .. code-block:: python

                              # Usage Example: Query for available user types
                              ...
                              # Output below will vary by organization
                              ...
                              >>>                           }
        ----------------  -------------------------------------------------------------------------------
        groups            List of String (group ID values) or :class:`groups <arcgis.gis.Group>`
                          objects new members will be added to.
        ----------------  -------------------------------------------------------------------------------
        userType          String.

                          .. note::
                              This *key* only applies to `ArcGIS Online`.

                          Specify whether new members will have *Esri* access or not.

                          Options:

                          * *arcgisonly* - Default. No access to specific *Esri* resources
                          * *both* - *Esri* resource and Online platform access

        ----------------  -------------------------------------------------------------------------------
        apps              List of dictionaries speicifying the *itemID* value of *apps* new members
                          will be able to access, and *entitlements* if applicable to the *app*.

                          .. code-block:: python

                              # Usage Example: apps argument syntax
                              ...
                              >>>                            }
        ----------------  -------------------------------------------------------------------------------
        appBundles        List of dictionaries specifiying an *app* bundle's ID.

                          .. code-block:: python

                              # Usage Example: appBundles argument syntax
        ...
        .. code-block:: python
            ...
            # Usage Example: Setting role, user_type, and group values for new members to receive
            ...
            >>> gis.users.user_settings = {
            >>>                            "role": "publisher",
            >>>                            "userLicenseType": "GISProfessionalAdvUT",
            >>>                            "groups": ["2a841cae10498b26a40e0194bacd2a11"]
            >>>                           }
        """
...
<p style="color:gray;">This is an automated email. Please do not reply.</p>
    ...
    def _create20251plus(
        ...
        **Parameter**      **Description**
        ...
        if self._gis._is_arcgisonline or self._gis._is_kubernetes:
    ...
    def create(
        ...
        **Parameter**         **Description**
        ...
        if membership_access in ["org", "collaboration", None, "none"]:
            ...
            elif self._gis._is_arcgisonline and membership_access is None:
    ...
    def search(
        ...
        **Parameter**       **Description**
        ...
        outside_org         Optional boolean. Controls whether to search outside
                            your org. Default is False, do not search outside your org.
        ----------------    --------------------------------------------------------
...
class ContentManager(object):
    ...
    def create_empty_service(
        ...
        **Parameter**               **Description**
        -----------------------    -------------------------------------------------------------
        parameters                 Required :class:`~arcgis.gis._impl._dataclasses._contentds.CreateServiceParameter`
                                   object specifying properties for the empty service.  See the
                                   `createParameters properties </rest/users-groups-and-items/create-service/#createparameters-properties>`_
        ...
        :returns:
            Returns :class:`~arcgis.gis.Item` object if successful or a dictionary
            with details if unsuccessful.
        """
        regex = r"^[-a-zA-Z0-9_]*$"
    ...
    def advanced_search(
        ...
        **Parameter**        **Description**
        ...
        max_items           Optional Integer. The total number of items to return up to
                            ...
                            returned up to 10,000. The default is 100.
        ----------------    ---------------------------------------------------------------
        ...
        categories          Optional String. A comma separated list of up to 8 org content
                            ...
                            called.
        ----------------    ---------------------------------------------------------------
        category_filter     Optional String. A comma separated list of up to 3 category
                            terms to search items that have matching categories. Up to 2
                            `category_filter` parameter are allowed per request. It can
    ...
    def search(
        ...
        category_filter: Optional[Union[list[str], str]] = None,
        ...
        **Parameter**       **Description**
        ...
        sort_field          Optional string. Valid values can be title, uploaded, type, owner, modified,
                            avgRating, numRatings, numComments, and numViews. The default is "avgRating".
        ----------------    --------------------------------------------------------------------------
        sort_order          Optional string. Valid values are asc or desc. The default is "desc".
        ----------------    --------------------------------------------------------------------------
        max_items           Optional integer. The total number of items to return up to
                            10,000. When the value of -1 is given all aviable Items will be
                            returned up to 10,000.
        ----------------    --------------------------------------------------------------------------
        outside_org         Optional boolean. Controls whether to search outside your org (default is False, do not search outside your org).
        ----------------    --------------------------------------------------------------------------
        categories          Optional string or list. A string of category values.
        ----------------    --------------------------------------------------------------------------
        category_filter     Optional string. A comma separated list of up to 3 category terms to
                            search items that have matching categories.

                            Up to 2 category_filter parameter are allowed per request. It can not be
        ...
        sort_field = sort_field or "avgRating"
    ...
    def delete_items(
        ...
        **Parameter**      **Description**
        ...
        if (
            permanent
            and (self._gis._is_arcgisonline or self._gis.version > [2023, 2])
            and rsupport
            and renabled
        ):
        ...
        **Parameter**         **Description**
        ...
        if publish_parameters is None:
            if pp["type"] == "CSV":
                ...
                publish_parameters = {
                    "name": service_name.replace(".", "").replace(" ", "_"),
                    "maxRecordCount": 2000,
                    "hasStaticData": True,
    ...
    def clone_items(
        ...
        dashboard_mapping: Optional[list[dashboards.ItemMapping]] = None,
        dashboard_options: Optional[list[dashboards.DependencyOptions]] = None,
        **kwargs,
    ):
        """
        ...
        **Parameter**              **Description**
        ...
        ---------------------     --------------------------------------------------------------------
        dashboard_mapping         Optional list of :class:`~arcgis.apps.dashboards.ItemMapping`
                                  objects. When cloning an ArcGIS Dashboard, this allows users to
                                  specify remapping of item ids, layers, and fields contained within
                                  the Dashboard to existent ones in the target GIS.
        ---------------------     --------------------------------------------------------------------
        dashboard_options         Optional list of :class:`~arcgis.apps.dashboards.DependencyOptions`
                                  objects. Allows user to customize Dashboard dependency replacement.
        =====================     ====================================================================
    ...
    def bulk_update(
        ...
        """
        Enables an item owner or organization administrator to perform a bulk update of item content categories. Each request is limited to a maximum of 100 items.

        ================  ======================================================================
        **Parameter**      **Description**
        ----------------  ----------------------------------------------------------------------
        itemids           Required list of Item IDs (strings) or :class:`~arcgis.gis.Item` objects.
        ----------------  ----------------------------------------------------------------------
        properties        Required dictionary containing the *categories* key and a list of category strings
                          as the value. See the :attr:`~arcgis.gis.CategorySchemaManager.schema_paths`
                          property to return all options for appropriately formatted category strings.
        ================  ======================================================================

        :return:
            A list of results specifying whether the update was successful for each itemId. If an update fails, the associated error is also provided.
        ...
        .. code-block:: python
            ...
            from arcgis.gis import GIS
            gis = GIS(profile="your_online_admin_profile")
            category_mgr = gis.content.categories
            type(category_mgr)

            >>> <class 'arcgis.gis.CategorySchemaManager'>

            category_paths = category_mgr.schema_paths
            ...
            >>>  .
            >>>  .
            >>> /Themes/Airports, Ports, and Rails

            flyr_item1 = gis.content.get("<flyr_item1_id>")
            flyr_item2 = gis.content.get("<flyr_item2_id>")

            res_bu = gis.content.bulk_update(
                itemids=[
                  flyr_item1.id,
                  flyr_item2.id
                ],
            ...
            >>>    'results':
            >>>       [
            >>>         {'itemId': '<flyr_item1_id>',
            >>>          'success': True},
            >>>         {'itemId': '<flyr_item2_id>',
            >>>          'success': False,
            >>>          'error': {
            >>>             'code': 400,
            ...
            flyr_item1_updated = gis.content.get('<caf_flyr_item_id>')

            flyr_item1_updated.categories
            >>> ['/Themes/Region/World']
...
class ResourceManager(object):
    """
    A helper class for managing resource files of an :class:`item <arcgis.gis.Item>`
    or :class:`user <arcgis.gis.User>`. An instance of this class is available as the
    *resources* property on each respective object.
    ...
    .. code-block:: python
        ...
        # Usage Example: Initializing ResourceManager object from an Item
    ...
    def add(
        ...
        """
        Adds file resources to an *item* or *user*. For example, an image that
        is used as custom logo for a *Report Template* item. All the files are
        ...
        **Parameter**      **Description**
        ...
        file              Optional string. The path to the file that needs to be added.
                          ...
                          .. note::
                              For a *user* object must be a binary file if no *text*
                              argument.
        ----------------  ---------------------------------------------------------------
        folder_name       Optional string. Provide a folder name if the file has to be
                          ...
                          .. note::
                              **Only** applicable to :class:`~arcgis.gis.Item` resources.
        ----------------  ---------------------------------------------------------------
        file_name         Optional string, **Required** on *user* resource manager. The
                          ...
                          .. note::
                              Extension must be `.json` when added to a
                              :class:`~arcgis.gis.User`.
        ----------------  ---------------------------------------------------------------
        text              Optional string. Text input to be added as a file resource,
                          ...
                          .. note::
                              If argument is provided on an *item* resource manager, then
                              the *file_name* argument becomes **required.**
        ----------------  ---------------------------------------------------------------
        archive           Optional boolean. Default is *False*.  If *True*, file resources
                          added are extracted and uploaded to respective folders.
        ...
        access            Optional String.
                          ...
                          * If *private* - sets file resource to private regardless of
                            the item access level
                          * If *inherit* - sets the file resource to inherit acces value
                            from the :class:`~arcgis.gis.Item`

                          Supported values for :class:`~arcgis.gis.User` resources:

                          * *userappprivate* - available only through the app from which
                            which the resource was uploaded
                          * *allorgusersprivateapp* - available to all members of the organization
                            to which the resource owner belongs, and through the app where the
                            from which the resource was uploaded
        ...
        properties        Optional Dictionary. Set the properties for the resources such
                          ...
                          .. note::
                              Only applicable for :class:`~arcgis.gis.Item` resources.
        ================  ===============================================================
        ...
        * For Both:
        ...
        .. code-block:: python
            ...
            >>>        file_name="aggregated_till_report.json"
            >>> )
        """
        if self._item:
...
class User(dict):
    ...
    **Property**             **Details**
    ...
    def _can_publish(self) -> bool:
        """determines if the user can publish by see if 1 or more privileges exist"""
        privs = [
        ...
        me: User = self
        user_privs = me.privileges
        diff: int = len(set(user_privs) - set(privs))
        if len(user_privs) > diff:
            return True
        return False
    ...
    # ----------------------------------------------------------------------
    def _check_report_status(self, ret):
        """Internal method to check the status of a reports.
        ...
        **Parameter**        **Description**
        ...
        item_id: str = ret.get("itemId")
        gis: GIS = self._gis
        if gis._is_arcgisonline:
            url: str = f"{gis.resturl}portals/self/itemsstatus"
        elif gis._is_arcgisonline is False:
            # enterprise/kubernetes
            url: str = f"{gis.resturl}portals/self/itemsStatus"

        session: EsriSession = gis.session
        results: dict[str, dict] = {}
        for key, params in payloads.items():
            resp: requests.Response = session.post(url, data=params)
            resp.raise_for_status()
            results[key] = resp.json()
            for result in results[key].get("results", []):
                if item_id == result.get("itemId", -1):
                    return key
    ...
    def report(
        ...
        **Parameter**      **Description**
        ...
        res = self._gis._con.post(url, params)
        i: int = 1
        status = self._check_report_status(res)
        while status in ["partial", "processing"]:
            status = self._check_report_status(res)
            ...
            if status in ["completed"]:
                ...
                raise Exception(f"An error was encountered: {status}")
            else:
                time.sleep(i)
                if i < 10:
                    i += 0.5
        if status == "failed":
            raise Exception(
                "The report generation failed, please check the logs for more information."
            )
        else:
            try:
                item = Item(self._gis, res["itemId"])
    ...
    def update_license_type(self, user_type: str):
        ...
        **Parameter**           **Description**
        ...
        :return: A boolean indicating success (True) or failure message if not successful. Failure returns a dict.
    ...
    def delete_thumbnail(self):
        ...
        :return:
            A boolean indicating success (True) or failure message if not successful. Failure returns a dict.
    ...
    def categories(self) -> list[str]:
        """
        Return a list of categories assigned to the user.
...
class Item(dict):
    """
    Represents an *item* in the :class:`~arcgis.gis.GIS` organization. An item is
    considered the fundamental unit of content in the *GIS*, and can represent a
    wide range of information. There are many different *types* of items, and each
    can have varied properties and capabilities. Each item has a unique identifier and
    a well-known URL that is independent of the user owning the item. The
    :class:`~arcgis.gis.Item` class inherits from internal Python structures such that
    not all *properties* for every item can be documented in this reference documentation.
    There may be additional *properties* available on an *item* than what are listed in
    the table of contents of the reference documentation.

    .. note::
        For a comprehensive list of properties available for an *item* object, please
        see the `Response properties <https://developers.arcgis.com/rest/users-groups-and-items/item/#response-properties>`_
        page.

        You can also get a full list of properties using Python's built in
        `dir() <https://docs.python.org/3/library/functions.html#dir>`_ method:

        .. code-block:: python

            from arcgis.gis import GIS
            gis = GiS(profile="you_organization_profile")

            my_item = gis.content.get("<item_id>")

            [d for d in dir(my_item) if not d.startswith("_")]

    Additionally, each item can have associated binary or textual data that's available
    via the item data resource. For example, an item of type `Map Package` returns the
    actual bits corresponding to the map package via the item data resource.

    Items that have layers (for example, :class:`~arcgis.features.FeatureLayerCollection`,
    :class:`~arcgis.features.FeatureLayer`, and :class:`~arcgis.raster._layer.ImageryLayer`
    items amongst others) and tables also have the dynamic ``layers`` and ``tables``
    properties to get to the individual layers/tables of those items.

    .. code-block:: python

        # Usage Example: Examining item properties

        from arcgis.gis import GIS
        gis = GIS(profile="your_organization_profile)

        city_item = gis.content.search("City *")[0]

        print(city_item.title)

        >>> 'City_Administrative_parcels'

        print(city_item.layers)

        >>> [<FeatureLayer url:"https://example.server.com/JAr...e3a/arcgis/rest/services/City_Administrative_parcels/FeatureServer/3">,
             <FeatureLayer url:"https://example.server.com/JAr...e3a/arcgis/rest/services/City_Administrative_parcels/FeatureServer/4">]

        print(city_item.size)

        >>> 958464

        print(city_item.type)
    ...
    def favorite(self) -> bool:
        ...
        try:
            ...
            if user is None:
                raise Exception("You must be logged in inorder to set a favorite.")
            query = f'(group:"{user.favGroupId}" AND id:"{self.itemid}")'
            if self._gis.content.advanced_search(query, return_count=True) > 0:
                return True
            return False
        except Exception as ex:
    ...
    def content_status(self):
        """
        Get or set (by assigning a value) the status of an :class:`~arcgis.gis.Item`
        to authoritative (either *org_authoritative* or *public_authoritative*)
        or deprecated. Owners and administrators of the *item* can use these
        designations to recommend whether an *item* should be used for specific
        purposes.

        ==================     ====================================================================
        **Parameter**           **Description**
        ------------------     --------------------------------------------------------------------
        value                  Optional string or None.  Defines whether an :class:`~arcgis.gis.Item`
                               is deprecated or authoritative.

                               .. note::
                                  If the property of *None* is assigned to the property, then all
                                  previous designations are cleared and status is reset.
                               ...
                               * *None*
                               ...
                               .. note::
                                   ...
                                   Also, `authoritative` will be converted to `org_authoritative`
                                   status.
        ==================     ====================================================================

        .. code-block:: python

            # Usage Example 1: Setting status to org_authoritative:

            gis = GIS(profile="your_organization_profile")

            authoritative_item = gis.content.get("<item_id>")
            authoritative_item.content_status = "org_authoritative"
            print(authoritative_item.content_status)

            >>> 'org_authoritative'

            # Usage Example 2: Clearing previous designations and resetting

            org_item.content_status

            >>> `org_authoritative`

            org_item.content_status = None
            org_item.content_status
        ...
        **Parameter**        **Description**
        ...
        if dry_run:
            ...
            if permanent:
                ...
                if (
                    (self._gis._is_arcgisonline or self._gis.version > [2023, 2])
                    and rsupport
                    and renabled
                ):
    ...
    def update(
        ...
        .. note::
            ...
            If passing a file path to the data parameter, the name of the replacement file
            can be different from the name of the file which is being replaced on the portal,
            because this function maintains the original file name internally.
        ...
        **Parameter**        **Description**
        ...
        item_properties     Required dictionary. See table below for the keys and values.
        ---------------     --------------------------------------------------------------------
        data                Optional pathlib.Path, string, io.StringIO, or io.BytesIO. Either a path or URL to
                            the data or an instance of `StringIO` or `BytesIO` objects.
        ---------------     --------------------------------------------------------------------
        thumbnail           Optional string. Either a path or URL to a thumbnail image.
        ---------------     --------------------------------------------------------------------
        ...
        **Key**            **Value**
        ...
        -----------------  ---------------------------------------------------------------------
        tags               Optional string. Tags listed as comma-separated values, or a list of strings.
                           Used for searches on items. To remove tags, pass in an empty string or list.
        -----------------  ---------------------------------------------------------------------
        text               Optional string. For text based items such as Feature Collections & WebMaps
        -----------------  ---------------------------------------------------------------------
        snippet            Optional string. Provide a short summary (limit to max 250 characters) of the what the item is.
        ...
        if isinstance(data, pathlib.Path):
            from arcgis.gis._impl._content_manager.folder._util import normalize_path

            data: str = normalize_path(data)
        ...
        for k, v in ranges.items():
            ...
            if as_df:
                ...
                if "data" not in res or len(res["data"]) == 0:
                    ...
                    res.Date = pd.to_datetime(res["Date"].astype(int), unit="ms")
        ...
        try:
            ...
            if as_df:
                ...
                if "data" not in res or len(res["data"]) == 0:
                    ...
                    df.Date = pd.to_datetime(arg=df["Date"].astype(int), unit="ms")
    ...
    def copy_item(
        ...
        *,
        title: Optional[str] = None,
        tags: Optional[Union[list[str], str]] = None,
        folder: Optional[Union[str, Folder]] = None,
        include_resources: bool = False,
        include_private: bool = False,
    ):
        """
        Creates a new :class:`~arcgis.gis.Item` that is a copy of the original
        :class:`~arcgis.gis.Item` on the server side. It is similiar to the
        :meth:`~arcgis.gis.Item.copy` method, but only creates a new
        :class:`~arcgis.gis.Item`.

        One of the below must be true to run the operation:

        * Original *item* is owned by the *user* calling *copy_item()*
        * The :class:`~arcgis.gis.User` calling *copy_item()* is an administrator
        * The :class:`~arcgis.gis.User` calling *copy_item()* has the
          *itemControl* update capability.

        Additionally, there are several caveats to the ``copy_item`` method.

        * The newly created *item* ID value is system-generated
        * Hosted services are copied as reference only
        * Type keywords, ratings, views, comments and listing properties are
          reset for the new item
        * Sharing access of the new *item* is set to private. Original sharing
          properties are not preserved.
        * Neither *Item* relationships nor dependencies of the original are maintained

        =======================    =============================================================
        **Parameter**               **Description**
        -----------------------    -------------------------------------------------------------
        title                      Optional string. The title of the destination
                                   :class:`~arcgis.gis.Item`. If not specified, title of the
                                   original item is used.
        -----------------------    -------------------------------------------------------------
        tags                       Optional String. New set of tags (comma separated) for the
                                   destination :class:`~arcgis.gis.Item`.
        -----------------------    -------------------------------------------------------------
        folder                     Optional String or :class:`~arcgis.gis.Folder` instance.
                                   The *folder* id, name, or *folder* object for destination item.

                                   If not specified, then the *item* remains in the same folder.

                                   .. note::
                                       If an administrator copies an *item* belonging to another
                                       user with no *folder* argument, the resulting copy is
                                       placed in the administratores Root Folder.
        -----------------------    -------------------------------------------------------------
        include_resources          Optional boolean.

                                   * If *True*,  original *item* file resources are copied to the
                                     new item. Private file resources will not be copied
                                     unless *include_private* is also set to *True*
                                   * If *False* (the *default*), original *item* file resources
                                     are not copied to new item
        -----------------------    -------------------------------------------------------------
        include_private            Option boolean.

                                   * If *True* **and** *include_resources=True*, private
                                     resources are copied to the new item
                                   * If *False* (the *default*),  private file resources are not
                                     copied to the new item.
        =======================    =============================================================

        :return:
            An :class:`~arcgis.gis.Item` object
        """
        ...
        if folder:
            params["folder"] = folder.properties["id"]
        res = self._portal.con.post(url, params)
        if "itemId" in res:
...
class ViewManager:
    ...
    def create_join_layer(
        ...
        **Parameter**             **Description**
        --------------------     --------------------------------------------------------------------
        join_name                Required string. Name of the new view item.
        --------------------     --------------------------------------------------------------------
        ...
        --------------------     --------------------------------------------------------------------
        join_type                Required JoinType. The type of table join to perform. Options are
                                 JoinType.INNER, JoinType.LEFT, JoinType.RIGHT, and JoinType.FULL.
        --------------------     --------------------------------------------------------------------
        ...
        --------------------     --------------------------------------------------------------------
        layer_id                 Optional int. If the target item has multiple layers, the ID of the
                                 layer to perform the join on.  The default is 0.
        ====================     ====================================================================
        """
        ...
        if isinstance(target, Item):
            target: features.FeatureLayer = features.FeatureLayer.fromitem(
                item=target, id_property=layer_id
            )
                        ...
                        "viewLayerDefinition": {
                            "table": {
                                ...
                                "sourceLayerFields": [
                                    ...
                                    for fld in target.properties["fields"]
