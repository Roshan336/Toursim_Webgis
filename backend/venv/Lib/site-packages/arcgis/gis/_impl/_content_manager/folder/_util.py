...
@cache
def normalize_path(file_path: str | Path) -> str | None:
    """converts a string location to the absolute path"""
    if file_path is None:
        return None
    if not isinstance(file_path, (str, Path)):
        raise ValueError(
            f"The input must be of type string or pathlib.Path, not {type(file_path)}"
        )
...
def create_upload_tuple(file: str | io.StringIO | io.BytesIO, **kwargs) -> tuple:
    ...
    elif isinstance(file, Path) and file.is_file():
        file: str = normalize_path(file_path=file)
        return create_upload_tuple(file=file)
    raise ValueError(
