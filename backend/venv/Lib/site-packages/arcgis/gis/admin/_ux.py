...
import requests
from ._basemaps import basemap_definition
...
class UX(object):
    ...
    _homepage_settings: HomePageSettings | None = None
    _map_settings: MapSettings | None = None
    _utility_services_settings: UtilityServicesSettings | None = None
    _item_settings: ItemSettings | None = None
    ...
    # ----------------------------------------------------------------------
    def __repr__(self):
        return f"UX(gis={self._gis.url})"

    # ----------------------------------------------------------------------
    def _check_resource(self, key: str) -> bool:
        """Helper to check if resource exists because get will return error in the file. Need to check list method first."""
        resources = self._portal_resources.list()
    ...
    def name(self, name: str):
        ...
        """
        if self._gis.properties.get("name") == name:
            return
        rps = []
        if self._gis.properties.get("rotatorPanels"):
            # update in rotator panels if any
            rps = [dict(r) for r in self._gis.properties.get("rotatorPanels")]
            for r in rps:
                r["innerHTML"] = r["innerHTML"].replace(
                    self._gis.properties.get("name"), name
                )

        # update in gis properties
        self._gis.update_properties({"name": name, "rotatorPanels": json.dumps(rps)})

        # Add localizedOrgProperties resource
        params = {
            "key": "localizedOrgProperties",
            "text": json.dumps({"default": {"name": name, "description": None}}),
            "f": "json",
        }
        url = f"{self._gis._portal.resturl}portals/self/add_resource"
    ...
    def summary(self):
        ...
        **Parameter**      **Description**
        ...
        :return: string
        """
        if self._check_resource("localizedOrgProperties") is False:
            self.summary = ""
            return self.summary
        local_org_props = self._portal_resources.get("localizedOrgProperties", None)
        if local_org_props:
            with open(local_org_props, "r") as reader:
                res = json.loads(reader.read())
                return res["default"]["description"] or ""
        ...
        for lng in languages:
            if lng["language"].lower() == language.lower():
                ...
                if "cultureFormats" in lng:
                    ...
                    culture = lng["culture"] + "-" + culture_format
                    break
    ...
    def contact_link(self):
        ...
        return props.get("portalProperties", {}).get("links", None)
    ...
    def admin_contacts(self):
        ...
        """
        return self._gis.properties.get("contacts", [])
    ...
    def help_source(self):
        ...
        """
        return self._gis.properties.get("helpBase", None)
    ...
    # ----------------------------------------------------------------------
    @deprecated(
        deprecated_in="2.4.3", details="Use homepage_settings.set_logo() instead."
    )
    ...
    # ----------------------------------------------------------------------
    @deprecated(
        deprecated_in="2.4.3", details="Use homepage_settings.get_logo() instead."
    )
    ...
    def description_visibility(self):
        ...
        **Parameter**      **Description**
        ...
        """
        return self._gis.properties.get(
    ...
    def description_visibility(self, visibility: bool):
        ...
        if (
            ...
            raise ValueError("This property no longer exists for your organization.")
    ...
    def description(self):
        ...
        **Parameter**      **Description**
        ...
        :return: dictionary
        """
        return self._gis.properties.get("description", None)
    ...
    def featured_content(self) -> dict:
        ...
        **Parameter**      **Description**
        ...
        .. code-block:: python
            *Usage Example*
            >>> group = gis.groups.search("My Featured Group")[0]
            >>> ux.featured_content = group
            True
    ...
    def featured_content(self, contents: dict):
        ...
        for content in contents:
            if (
                isinstance(contents, dict) and isinstance(contents.get("id"), str)
            ) or isinstance(contents, str):
                if isinstance(contents, dict):
                    contents = contents["id"]
                content = Group(self._gis, contents)

            # Get group details
            parsed_group_details = {
                "id": content.get("group").id,
                "title": content.get("group").title,
                "owner": content.get("group").owner,
            }
            # append the Group to the featured groups list
            featured_groups.append(parsed_group_details)
    ...
    def navigation_bar(
        ...
        **Parameter**        **Description**
        ...
        """
        portal_properties = self._gis.properties.get("portalProperties", {})

        # Use a dict to map argument names to their values
        updates = {
            "gallery": gallery,
            "map": map,
            "scene": scene,
        ...
        top_nav = portal_properties.get("topNav") or {
            "gallery": "all",
            "map": "all",
            "scene": "all",
            "groups": "all",
        ...
        # update only the keys that have a value passed in
    ...
    def shared_theme(
        ...
        **Parameter**        **Description**
        ...
        """
        portal_properties = self._gis.properties["portalProperties"]
        shared_theme = portal_properties.get(
            "sharedTheme",
            {
    ...
    def homepage_settings(self):
        ...
        title, logo, etc.
        """
        if self._homepage_settings is None:
            self._homepage_settings = HomePageSettings(gis=self._gis)
        return self._homepage_settings
    ...
    def map_settings(self):
        ...
        """
        if self._map_settings is None:
            self._map_settings = MapSettings(gis=self._gis)
    ...
    def utility_services_settings(self):
        """
        Get an instance of the :class:`~arcgis.gis.admin.UtilityServicesSettings`
        class to manage the set of utility services that can be configured to use
        ArcGIS Online services.
        """
        if self._utility_services_settings is None:
            self._utility_services_settings = UtilityServicesSettings(gis=self._gis)
        return self._utility_services_settings
    ...
    def item_settings(self):
        ...
        if self._item_settings is None:
            self._item_settings = ItemSettings(gis=self._gis)
        return self._item_settings
    ...
    def security_settings(self):
        ...
        if self._security_settings is None:
            self._security_settings = SecuritySettings(gis=self._gis)
...
class HomePageSettings(object):
    ...
    _hp_cache: dict[str, Any] | None = None
    ...
    def __init__(self, gis):
        ...
        self._portal = gis._portal
        self._portal_resources = gis.admin.resources
        self._hp_cache = self._load_homepage()

    # ----------------------------------------------------------------------
    def __repr__(self):
        return f"HomePageSettings(gis={self._gis.url})"

    # ----------------------------------------------------------------------
    def _load_homepage(self, force_reload: bool = False) -> dict[str, Any]:
        """reads the homepage settings as a dictionary."""
        if self._hp_cache is None or force_reload:
            resources = self._portal_resources.list()
            for res in resources:
                if res["key"] == "home.page.json":
                    with open(
                        ...
                        content = reader.read()
                        self._hp_cache = json.loads(content)
                        break
                else:
                    self._hp_cache = {}
    ...
    # ----------------------------------------------------------------------
    def _add_image_file(self, src_file: str | None, background=True):
        # find image extension
        fpath = Path(src_file)
        f_splits = fpath.name.split(".")
        if len(f_splits) > 1 and f_splits[1] == "png":
            key_val = "background.png" if background else "thumbnail.png"
        elif len(f_splits) > 1 and f_splits[1] == "jpg":
            key_val = "background.jpg" if background else "thumbnail.jpg"
        elif len(f_splits) > 1 and f_splits[1] == "gif":
            key_val = "background.gif" if background else "thumbnail.gif"
        else:
            raise RuntimeError("Invalid image extension")

        add_result = self._portal_resources.add(key_val, src_file)
        if not add_result:
    ...
    def set_background(
        ...
        a single, non repeating background image, the image should be 1,920 pixels wide (or smaller if your users are on
        ...
        **Parameter**        **Description**
        ...
        # determine which background to set
        if background_file:
            background_update_val = self._add_image_file(
                background_file, background=True
            )
            cover_type = "custom"
            cover_image_stock = ""
        ...
        res = self._portal_resources.add(
            key="home.page.json", text=json.dumps(params["text"])
        )
        self._load_homepage(force_reload=True)
        return res
    ...
    def get_background(self, download_path: str | None = None):
        ...
        **Parameter**      **Description**
        ...
        # Return depending on if stock or custom
        ...
        if download_path is None:
            raise ValueError(
                "A download path is needed since the background is a custom image."
            )
    ...
    def set_logo(
        ...
        **Parameter**        **Description**
        ...
        if logo_file is not None and os.path.isfile(logo_file):
            # add new logo
            key_val = self._add_image_file(logo_file, background=False)
    ...
    def get_logo(self, download_path: str):
        ...
        **Parameter**      **Description**
        ...
        """
        resource = self._hp_cache["header"]["logo"]
    ...
    def set_title(
        ...
        **Parameter**        **Description**
        ...
        if color:
            hp["header"]["backgroundColor"] = color
        if alignment and alignment in ["center", "left"]:
            hp["header"]["titleLogoAlign"] = alignment
        if position and position in [
            "middle",
    ...
    def get_title(self):
        ...
        hp = self._hp_cache
        title = {
            "title": hp["header"].get("title"),
            "show_title": hp["header"].get("showTitle"),
            "color": hp["header"].get("titleColor")
            or hp["header"].get("backgroundColor"),
            "position": hp["header"].get("titleLogoPos"),
            "alignment": hp["header"].get("titleLogoAlign"),
        }
        return title
    ...
    def set_contact_email(
        ...
        res = self._portal_resources.add(key="home.page.json", text=json.dumps(hp))
        self._load_homepage(force_reload=True)
        return res
    ...
    def get_contact_email(self):
        ...
        contact = {
            "email": self._hp_cache["footer"]["contact"],
            "show_email": self._hp_cache["footer"]["showContact"],
        }
        return contact
    ...
    def get_typography(self):
        """Get the footer of the homepage"""
        if self._hp_cache.get("useCustomTypography"):
            return self._hp_cache["customTypography"]
        return self._hp_cache.get("typography")
    ...
    def get_base_color(self):
        ...
        return self._hp_cache.get("baseColor", None)
...
class MapSettings(object):
    ...
    # ----------------------------------------------------------------------
    def __repr__(self):
        return f"MapSettings(gis={self._gis.url})"
    ...
    def default_extent(self):
        ...
        **Parameter**      **Description**
        ...
        """
        return self._gis.properties.get("defaultExtent", {})

    # ----------------------------------------------------------------------
    @default_extent.setter
    def default_extent(self, extent: dict | None):
        """
    ...
    def default_basemap(self):
        ...
        **Parameter**      **Description**
        ...
        """
        return self._gis.properties.get("defaultBasemap", {})
    ...
    def default_basemap(self, value: str):
        ...
        """
        if value not in basemap_definition.basemap_dict:
            raise ValueError(
        ...
        basemap = {
            "baseMapLayers": basemap_definition.basemap_dict[value],
    ...
    def use_vector_basemap(self):
        ...
        """
        return self._gis.properties.get("useVectorBasemaps")
    ...
    def vector_basemap(self):
        ...
        **Parameter**      **Description**
        ...
        """
        return self._gis.properties.get("defaultVectorBasemap")
    ...
    def vector_basemap(self, basemap: dict):
        ...
        """
        value = self._gis.properties.get("defaultVectorBasemap", {})
        if "title" in basemap:
    ...
    def basemap_gallery_group(self):
        ...
        """
        group = None
        if self._gis.properties.get("useVectorBasemaps"):
            group = self._gis.properties.get("vectorBasemapGalleryGroupQuery")
        else:
            group = self._gis.properties.get("basemapGalleryGroupQuery")
        if "id:" in group:
            # must use [3::] to slice string since format of: "id:123abc"
    ...
    def basemap_gallery_3d_group(self):
        ...
        :return: An instance of Group if a group is set, else the default or None
        """
        group_id = self._gis.properties.get("3DBasemapGalleryGroupQuery", None)
        if group_id:
            if "id:" in group_id:
                # must use [3::] to slice string since format of: "id:123abc"
    ...
    def use_3D_basemaps(self, value: bool) -> bool:
        ...
        reference in a web scene.
        """
        if self._gis.properties.get("use3dBasemaps", False) != value and value in [
            True,
            False,
        ]:
    ...
    def default_mapviewer(self):
        ...
        .. note::
            ...
            * If you are using ArcGIS Enterprise 11.4 or earlier, the value can be
              set to "classic" or "modern".
            * If you are using ArcGIS Enteprise 11.5, the value can be set to
              "modernOnly".
            * For ArcGIS Enterprise 12.0 and later, support for Map Viewer Classic
              has been removed and there is no default Map Viewer property.
            * For ArcGIS Online the value can only be "modern".

        """
        if (
            not self._gis._is_arcgisonline
            and self._gis.properties["enterpriseVersion"] >= "12.0.0"
        ):
            import warnings

            warnings.warn(
                "Default map viewer property is None. Only modern Map Viewer supported."
            )
        return self._gis.properties["portalProperties"].get("mapViewer")

    # ----------------------------------------------------------------------
    @default_mapviewer.setter
    def default_mapviewer(self, value: str):
        if value not in ["modern"]:
            raise ValueError("The only accepted value is 'modern'")

        portal_properties = self._gis.properties["portalProperties"]
        portal_properties["mapViewer"] = (
            value if self._gis._is_arcgisonline else "modernOnly"
        )
        self._gis.update_properties({"portalProperties": portal_properties})
    ...
    def web_styles(
        ...
        **Parameter**                **Description**
        ...
        return {
            "2DStyles": self._gis.properties.get("2DStylesGroupQuery"),
            "3DStyles": self._gis.properties.get("stylesGroupQuery"),
        }
...
class ItemSettings(object):
    ...
    # ----------------------------------------------------------------------
    def __repr__(self):
        return f"ItemSettings(gis={self._gis.url})"
    ...
    def enable_comments(self):
        ...
        **Parameter**      **Description**
        ...
        """
        return self._gis.properties.get("commentsEnabled")
    ...
    def enable_metadata_edit(self):
        ...
        **Parameter**      **Description**
        ...
        ================  ===============================================================
        """
        return self._gis.properties.get("metadataEditable")
...
class SecuritySettings(object):
    ...
    # ----------------------------------------------------------------------
    def __repr__(self):
        return f"SecuritySettings(gis={self._gis.url})"
    ...
    def set_informational_banner(
        ...
        **Parameter**        **Description**
        ...
        current_info_banner = self._gis.org_settings.get(
            "informationalBanner",
            {
                "text": "",
                "bgColor": "white",
        ...
        if text is not None or text == "":
            # Set to empty or provided text
            text = text
        else:
            text = current_info_banner["text"]
        if enabled in [True, False]:
            # set to provided enabled value
            enabled = enabled
        else:
        ...
        informational_banner = {
            "text": text,
            "bgColor": bg_color or current_info_banner["bgColor"],
            "fontColor": font_color or current_info_banner["fontColor"],
            "enabled": enabled,
        }
    ...
    def get_informational_banner(self):
        ...
        """
        return self._gis.org_settings.get("informationalBanner")

    # ----------------------------------------------------------------------
    @property
    def enable_https(self):
        """Allow access to the portal through HTTPS only (True)."""
        return self._gis.properties.get("allSSL")
    ...
    def anonymous_access(self):
        ...
        return self._gis.properties.get("access")
    ...
    def enable_update_user_profile(self):
        ...
        return self._gis.properties.get("updateUserProfileDisabled")
    ...
    def share_public(self):
        ...
        return self._gis.properties.get("canSharePublic")
    ...
    def show_social_media(self):
        ...
        """
        return self._gis.properties.get("portalProperties", {}).get(
    ...
    def get_password_policy(self):
        ...
        params = {"f": "json"}
        res = self._gis.session.post(url, params)
        resp = res.json()
    ...
    def allowed_origins(self):
        ...
        """
        return self._gis.properties.get("allowedOrigins", [])
    ...
    def trusted_servers(self):
        ...
        """
        return self._gis.properties.get("authorizedCrossOriginDomains", [])
    ...
    def set_anonymous_access_notice(
        ...
        **Parameter**                **Description**
        ...
        """
        org_settings = self._gis.org_settings
        button_type = button_type or "acceptAndDecline"
        notice = {"title": title, "text": text, "buttons": button_type}
        if title and text:
    ...
    def get_anonymous_access_notice(self):
        ...
        :return: The dict representation of the notice or if none set then None.
        """
        return self._gis.org_settings.get("anonymousAccessNotice", None)
    ...
    def set_email_settings(
        ...
        **Parameter**                    **Description**
        ...
        return self._gis.session.post(url, params).json()
    ...
    def get_email_settings(self):
        ...
        return self._gis.session.post(url, {"f": "json"}).json()
    ...
    def test_email_settings(self, mail_to: str):
        ...
        **Parameter**                    **Description**
        ...
        return self._gis.session.post(url, {"f": "json", "mailTo": mail_to}).json()
...
class UtilityServicesSettings:
    """
    Helper class to manage the set of `Utility services <https://enterprise.arcgis.com/en/portal/latest/administer/windows/about-utility-services.htm>`_
    residing in ArcGIS Online that can be configured with *ArcGIS Enterprise*. Objects
    of this class are not designed to be created directly, but instead are
    available by accessing the :attr:`~arcgis.gis.admin.UX.utility_services_settings`
    property of the :class:`~arcgis.gis.admin.UX` class. A *UX* object is available
    with the :attr:`~arcgis.gis.admin.PortalAdminManager.ux` property.

    See `Configure ArcGIS Online Utility Services <https://enterprise.arcgis.com/en/portal/latest/administer/windows/configure-arcgis-online-utility-services.htm>`_
    for full details.

    .. code-block:: python

        # Initializing a UtilityServicesSettings object

        from arcgis.gis import GIS

        ent_gis = GIS(profile="your_enterprise_admin_profile)

        utility_svcs_mgr = ent_gis.admin.ux.utility_services_settings
        utility_svcs_mgr

        >>> UtilityServicesSettings(gis=https://example_url.com/web_adaptor_name)
    ...
    def __repr__(self):
        return f"UtilityServicesSettings(gis={self._gis.url})"

    # ----------------------------------------------------------------------
    def add_from_online(self, services: list[str], gis: _arcgis_gis.GIS, folder=None):
        """
        Set the Enterprise utility services to ArcGIS Online services.

        .. note::
            Requires an ArcGIS Online account.

        ==================      ================================================
        **Parameter**            **Description**
        ------------------      ------------------------------------------------
        services                List of utility services to configure with ArcGIS
                                Online services. The services are accessed through
                                the ArcGIS Online credentials provided in the *gis*
                                argument.
                                ...
                                * *Traffic Data*
        ------------------      ------------------------------------------------
        gis                     A :class:`~arcgis.gis.GIS` object that is
                                initialized to your ArcGIS Online organization
                                where the services will be accessed from.
        ------------------      ------------------------------------------------
        folder                  Optional string or
                                :class:`~arcgis.gis._impl._content_manager.Folder`
                                object. The folder in the *ArcGIS Enterprise
                                organization* where the proxy
                                :class:`items <arcgis.gis.Item>` for the ArcGIS
                                Online Service(s) will be added. If the *folder*
                                cannot be found, a new one is created.
        ==================      ================================================

        :return:
           A dictionary object indicating *success* or *failure*.

        .. note::
            An :class:`~arcgis.gis.Item` is created in the Enterprise portal that
            stores the Online account credentials with it.

        .. code-block:: python

            # Usage example: Configure an utility service from an ArcGIS Online Service

            from arcgis.gis import GIS

            ago_gis = GIS(
                        url="https://www.arcgis.com",
                        username="<your_online_username>",
                        password="<your_online_password>"
            )
            ent_gis = GIS(profile="your_enterprise_admin_profile")

            service_list = ["Elevation", "GeoEnrichment"]

            proxy_folder = ent_gis.content.folders.create(
                                  "utility_service_items
                           )

            utility_mgr = ent_gis.admin.ux.utility_services_settings

            result = utility_mgr.add_from_online(
        ...
        for svc in services:  # replace with config utilityServices
            for helper_svc in helper_services[svc]:
                ...
                elif helper_svc == "orthomappingElevation":
                    url = agol_helper_svcs[helper_svc]["url"]
                    proxy_urls[helper_svc + "Service"] = {"url": url}
                else:
                    try:
                        url = agol_helper_svcs[helper_svc]["url"]
                    except:
                ...
                item_props = {
                    ...
                    "title": f"AGO World {helper_svc.capitalize()} ({username})",
                    "type": svc_types.get(svc_type, ""),
                }
                if helper_svc == "geocode":
                    item_props["tags"] += ",Locator Service,Geocoding Service"

                # Add item
                search_result = self._gis.content.search(
                    query=f"{item_props['title'].startswith('AGO World')} {helper_svc[:7]} *",
                ...
                this_item = next(
                    (
                        ...
                        if x.title.startswith(f"{item_props['title'][:15]}")
                    ),
                    "",
                )
                if not this_item:
                    if helper_svc == "orthomappingElevation":
                        ...
                        try:
                            svc_item_job = folder_item.add(item_properties=item_props)
                            svc_item = svc_item_job.result()
                    ...
                    if helper_svc == "geocode":
                        ...
                        geocoders = [
                            dict(val) for val in gis.properties.helperServices.geocode
                        ]  # pylint:disable=no-member
                        geocoder = deepcopy(geocoders[0])
                        ...
                        geocoders_list = deepcopy(
                            self._gis.properties.helperServices["geocode"]
                        )
                        geocoders_list.insert(0, geocoder)
                        proxy_urls[helper_svc + "Service"] = geocoders_list
                    # elif helper_svc == "route":
                    else:
                        if helper_svc == "route":
                            proxy_urls[helper_svc + "ServiceLayer"] = {
                                "url": svc_item_url
                            }
                        else:
                    ...
                    if helper_svc == "geocode":
                        ...
                        geocoders_list.append(gcoder_item_dict)
                        proxy_urls[helper_svc + "Service"] = geocoders_list
                    else:
                        if helper_svc == "route":
                            proxy_urls[helper_svc + "ServiceLayer"] = {
                            ...
                            proxy_urls[helper_svc + "Service"] = {"url": this_item.url}
    ...
    def reset_services(self, services: list[str]):
        """
        Resets the specified utility services to the default setting for the
        service. The method does *not* delete the *items* for the services.

        .. note::
            For all utility services except, *Geocoding*, the service is set to
            no value. For *Geocoding*, it is set to the `ArcGIS World Geocoding
            Service <https://enterprise.arcgis.com/en/portal/latest/administer/windows/configure-services.htm#ESRI_SECTION2_05FEF01AA0964C949177D248F9B783A1>`_.


        ==================      ================================================
        **Parameter**            **Description**
        ------------------      ------------------------------------------------
        services                List of utility services to configure to the
                                default settings.
        ...
        :return:
            A dictionary response indicating success or failure.
        ...
        for service in services:
            ...
            for helper_svc in helper_services[service]:
                ...
                full_url = None
                if helper_svc != "orthomappingElevation":
                    service_info = current_services.get(helper_svc, "")
                ...
                if full_url:
                    ...
                    # Add except clause to deal with default geocoder url
