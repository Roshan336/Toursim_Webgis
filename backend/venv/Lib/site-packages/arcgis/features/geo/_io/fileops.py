...
def read_feather(
    ...
    if spatial_column and spatial_column in sdf.columns:
        ...
        if "SHAPE" in sdf.columns:
            sdf["SHAPE"] = [Geometry(g) for g in sdf["SHAPE"]]
        sdf.spatial.name
    return sdf
...
def _arcpy_workflow(filename, **kwargs):
    ...
    if fields is None:
        ...
        afields = set(fields)
        fields_set = set(
            [fld.name for fld in desc["fields"] if fld.type != "Geometry"]
        )  # all fields
        fields = list(afields - (afields - fields_set))
...
def to_featureclass(
    ...
    **Parameter**        **Description**
    ...
    if Path(location).is_dir() and Path(location).parent.as_uri().endswith(
        ".geodatabase"
    ):
        # drops the table lock in the mobile gdb
...
def _gdal_to_sedf(file_path, **kwargs):
    ...
    try:
        ...
        sr_code = spatial_ref.ExportToWkt()
    ...
    for feature in out_layer:
        ...
        if geom is not None:
            ...
            # Geometry({'wkt': sr_code,})
            if isinstance(sr_code, str):
                esri_geom.spatialReference = Geometry(
                    {
                        "wkt": sr_code,
                    }
                )
            elif isinstance(sr_code, int):
    ...
    df.spatial.sr = Geometry(esri_geom.spatialReference)
